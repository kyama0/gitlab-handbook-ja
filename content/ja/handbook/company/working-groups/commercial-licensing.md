---
title: "コマーシャル & ライセンス ワーキンググループ"
description: "将来にわたってコマーシャルおよびライセンス取引を処理するシステムのビジネス & 顧客体験要件を定義します。"
upstream_path: /handbook/company/working-groups/commercial-licensing/
upstream_sha: b4eeb07f0d5f46e2fc5f8572be1a2547261aed89
translated_at: "2026-04-25T00:00:00Z"
translator: claude
stale: false
---

## 属性

| プロパティ        | 値          |
|-----------------|----------------|
| 作成日           | 2020年9月1日   |
| 終了日           | 2020年10月30日   |
| Slack           | [#commercial_steerco](https://gitlab.slack.com/archives/C019UNKBH7B)（社内からのみアクセス可能）|
| Google Doc      | [Commercial & Licensing Working Group](https://docs.google.com/document/d/1MWC3oT4DqCrJ6bFQFsW9C_PsWMIyDB_ygsMwcGyDS6s/edit)（社内からのみアクセス可能）|

## 状況

状況：請求・ライセンスワークフローに関するビジネス成果と顧客体験のオーナーシップが、Fulfillment、Growth、BizOps、SalesOps、Marketing Ops、Sales と非常に分散しています。この領域に単一のビジネスオーナーが存在しないため、担当チームに統一した方向性を提供するためのクロスファンクショナルなワーキンググループを設置することにしました。

## 目的

将来にわたってコマーシャルおよびライセンス取引を処理するシステムのビジネス & 顧客体験要件を定義します。

## 終了条件

- TBD

## メンバー

| 担当者                | 役職                                  |
|-----------------------|----------------------------------------|
| Scott Williamson      | Chief Product Officer                         |
| Bryan Wise            | VP of IT                               |
| Jack Brennan          | Director, Sales Systems                |
| Dara Warde            | Director of Marketing Operations       |
| Christopher Nelson    | Sr. Director of Enterprise Applications|
| Mike Karampalas       | Principal Product Manager, Retention   |
| Amanda Rueda          | Product Manager, Fulfillment           |
| Bartek Marnane        | Director of Engineering, Growth        |
| Kristie Thomas        | EBA                                    |
| Jacki Bauer           | Product Design Manager, Growth         |
