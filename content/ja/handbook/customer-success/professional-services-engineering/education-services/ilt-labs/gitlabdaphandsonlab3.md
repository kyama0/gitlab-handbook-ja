...
1. **Configuration** で、Tools ドロップダウンで **Create Issue** を検索して選択します。
...
1. **System prompt** セクションに次のように入力します。
...
1. エージェントを作成したページと同じページから、右上隅の **Enable** をクリックします。
...
1. エージェントドロップダウンリストから **Swag Shop Onboarding Agent** を選択します。
...
1. 次に、Issue を作成するようエージェントに依頼します。次のプロンプトを入力します。

   ```prompt
   Create an issue for me to help onboard and start contributing to help the team.
   ```

1. エージェントがツール呼び出しの承認を求めます。タイトル、説明、ラベルをレビューして **Approve** をクリックします。

1. **Plan > Work items** に移動して、新しく作成された Issue を開きます。
