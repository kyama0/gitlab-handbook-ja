---
title: "CSM 開発"
description: "GitLab の Customer Success Manager が利用できる開発パスとリソース。"
upstream_path: /handbook/customer-success/csm/csm-development/
upstream_sha: b4eeb07f0d5f46e2fc5f8572be1a2547261aed89
translated_at: "2026-04-26T00:00:00Z"
translator: claude
stale: false
---

GitLab の Customer Success Manager として、[成長のための 70-20-10 ルール](/handbook/people-group/learning-and-development/career-development/#70-20-10-rule-for-development)を活かし、自己投資やキャリアへの投資に時間をかけることが推奨されています。

継続的な改善へのコミットメントを意図的に行うため、毎月最初の金曜日の午後は個人・職業的開発に使用することが強く推奨されています。個人で行うことも、他の GitLab チームメンバーと協力して行うことも可能です。

そのため、個人・職業的開発に利用できるリソースを理解することが不可欠です。このページの残りの部分は、リーダーシップと CSM が CSM チームの進歩・開発に利用できるリソースに貢献するための場所です。

## イネーブルメントのための主要なハンドブックセクション、ロードマップ、L&D プラットフォーム

1. [FY24 CSM 計画済みイネーブルメントロードマップ](https://docs.google.com/presentation/d/1maP9grsRJTlEsPsvQSw3d2jxw6BB-tgnURmA2lvOC_I/edit#slide=id.g23e6c497812_0_432)
1. [CSM イネーブルメントコーナー](https://docs.google.com/document/d/1xyaxc37iCDtfeabo2NXCRV31pNOdvieXoVDxJw_aXoM/edit?usp=sharing)（主要な日程と CSM が必要なアクションで四半期ごとに更新）

<iframe src="https://docs.google.com/presentation/d/e/2PACX-1vTB9tOQWZ771-M2nrIQkqIpx3XF7f9q1xXbBRkW5D-_UAyksak626FVn0Ovrj0j7rHDurML4N8f83IY/embed?start=false&loop=false&delayms=3000&slide=id.g23e582b58af_0_829" frameborder="0" width="960" height="569" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>

## カスタマーサクセスマネジメント - 学習・開発リソース

コンテンツはコンピテンシーの 3 つの主要カテゴリーに分けられています。[LevelUp](https://levelup.edcast.com/home) は GitLab の認定、学習パスなどのための学習体験プラットフォームです！開発したいコースについてアイデアがある場合は、#learninganddevelopment チャンネルで連絡してください。L&D チームがセットアップを支援します。

**💡 [カスタマーサクセスマネジメント](#general-csm)     💬 [ソフトスキル](#soft-skills)     🚀 [テクノロジーとドメイン知識](#tech-domain)**

---

<a name="general-csm"></a>

### 💡 カスタマーサクセスマネジメント

#### GitLab における CSM のベストプラクティスと事例

- [Chorus プレイリスト](https://chorus.ai/playlists/741604)では最高の CSM コールの録音にアクセスできます。録音は継続的に更新されるため、ベストプラクティスのためにこのプレイリストをブックマークしてください。

#### CSM インターンシップ

- [学習のための CSM インターンシップ](/handbook/customer-success/csm/csm-internship/)

#### カスタマーサクセスマネジメント

- [Customer Success Management Fundamentals](https://www.linkedin.com/learning/customer-success-management-fundamentals)（LinkedIn Learning）
- [Business Fundamentals for Customer Success Managers](https://www.linkedin.com/learning/business-fundamentals-for-customer-success-managers)（LinkedIn Learning）
- [Avoiding Common Pitfalls in Customer Success Management](https://www.linkedin.com/learning/avoiding-common-pitfalls-in-customer-success-management)（LinkedIn Learning）
- [Onboarding and Adoption Best Practices for Customer Success Management](https://www.linkedin.com/learning/onboarding-and-adoption-best-practices-for-customer-success-management)（LinkedIn Learning）
- [Engagement Evaluation Best Practices for Customer Success Management](https://www.linkedin.com/learning/engagement-evaluation-best-practices-for-customer-success-management)（LinkedIn Learning）
- [Value Realization Best Practices for Customer Success Management](https://www.linkedin.com/learning/value-realization-best-practices-for-customer-success-management)（LinkedIn Learning）
- [Building Rapport with Customers](https://www.linkedin.com/learning/building-rapport-with-customers-21402583)（LinkedIn Learning）
- [Customer Experience: Journey Mapping](https://www.linkedin.com/learning/customer-experience-journey-mapping)（LinkedIn Learning）
- [Advanced Business Development Communication & Negotiation](https://www.linkedin.com/learning/advanced-business-development-communication-and-negotiation/taking-it-to-the-next-level)（LinkedIn Learning）
- [Sales: Customer Success](https://www.linkedin.com/learning/customer-success-foundations)（LinkedIn Learning）
- [Customer Success by Nick Mehta, Dan Steinman, Lincoln Murphy, Maria Martinez](https://learning.oreilly.com/library/view/customer-success/9781119167969/)（O'Reilly Books）
- [The Customer Success Professional's Handbook](https://learning.oreilly.com/library/view/the-customer-success/9781119624615/)（O'Reilly Books）
- [SQS の Sales & Customer Success 学習パス](/handbook/sales/onboarding/sales-learning-path/): [セールスクイックスタート（SQS）](/handbook/sales/onboarding/)でスケジュールされたトレーニングセッションと並行して、[ロールベースのコンテンツ](/handbook/sales/onboarding/sales-learning-path/#customer-success-quick-start---role-based-curriculum)あり（インストラクター主導のセッション）

#### リーダーシップ

- [CSM アスパイアリングリーダーズプログラム](/handbook/customer-success/csm/aspiring-leaders-program/)

#### エグゼクティブコミュニケーション

CSM がより戦略的なポジションに就き、ディレクター、VP、CISO のペルソナにリーチするようになるにつれて、コミュニケーションスタイルを調整し、エグゼクティブに合わせた方法を考える必要があります。これらのリソースは CSM のエグゼクティブプレゼンスとコミュニケーションを支援します。

- L&D には [Crucial Conversations](/handbook/leadership/crucial-conversations/) の認定を持つ 2 名のチームメンバーがいます。四半期に 1〜2 回のセッションがあります。
- [Connecting with Executives](https://www.linkedin.com/learning/communicating-with-executives)
- [Issue: Executive Communication Best Practices](https://gitlab.com/gitlab-com/customer-success/csm/-/issues/712)
- [Google ドキュメント（積極的に更新中）](https://docs.google.com/document/d/1lkk_25XwOOw9eN-NqxuupZsDWRdWPbmFYD8RP8cWXz8/) - Lee Faus、Brian Wald、Francis Ofungwu が GitLab のプラットフォームに関するエグゼクティブから聞いているテーマ（4 つの柱）を共有
- Sandler 記事
  - [How to Use DISC Assessments to Appropriately Customize Workplace Communication](https://www.sandler.com/blog/how-use-disc-assessments-appropriately-customize-workplace-communication/)（記事）
  - [Up Front Contracts](https://www.youtube.com/watch?v=WTZwyUeAI3o)（記事）
- [Pyramid Principle for Slide Presentation](https://www.myconsultingoffer.org/case-study-interview-prep/pyramid-principle/)（記事）
- [Tim Ferris - how I prepare for keynotes](https://tim.blog/2010/04/11/public-speaking-how-i-prepare-every-time/)（記事）

#### パブリックスピーキングとイネーブルメント

- [Top Facilitation Techniques and Tactics For Great Remote Workshops](https://www.youtube.com/watch?v=JsRxF0I3eaI) - 16 分、質問ごとにタイムスタンプ付き（YouTube 動画）
- [How to Create and Run a Brilliant Remote Workshop](https://www.linkedin.com/learning/how-to-create-and-run-a-brilliant-remote-workshop/how-to-run-a-brilliant-remote-workshop?autoplay=true&resume=false&u=2255073) - 48 分、パート 2 を聴くことをお勧めします（GitLab がワークショップで使用するツールについてはアドバイスがあります）（LinkedIn Learning）
- [Tips & Tricks for an Engaging Remote Workshop](https://global.hitachi-solutions.com/blog/tips-tricks-for-remote-workshops/)（記事）
- [How to Run an Engaging Remote Workshop: Tips for Stress-Free Collaboration](https://dscout.com/people-nerds/remote-workshops)（記事）
- [10 tips on how to deliver an engaging remote workshop](https://excopartners.com/2021/10/09/10-tips-on-how-to-deliver-an-engaging-remote-workshop)（記事）
- [Communicating with Confidence](https://www.linkedin.com/learning/communicating-with-confidence-23450131)（LinkedIn Learning）
- [Dynamic Public Speaking by University of Washington](https://www.coursera.org/specializations/public-speaking)（Coursera）
- [Storytelling and influencing: Communicate with impact](https://www.coursera.org/learn/communicate-with-impact)（Coursera）

<a name="soft-skills"></a>

### 💬 ソフトスキル

- [Professional Soft Skills Learning Pathway](https://www.linkedin.com/learning/paths/professional-soft-skills-learning-pathway)（LinkedIn Learning）
- [Kind Engineering: How to Engineer Kindness](https://www.youtube.com/watch?v=OGFsM2lcF60)（YouTube）
- [Secrets to Optimal Client Service](https://www.youtube.com/watch?v=hJbwyN4ZoCg)（YouTube）
- [Selling Solutions vs Solving Problems](https://www.youtube.com/watch?v=9ZuWLs7ZNpY)（YouTube）
- [Psychological Safety: Clear Blocks to Innovation, Collaboration, and Risk-Taking](https://www.linkedin.com/learning/psychological-safety-clear-blocks-to-innovation-collaboration-and-risk-taking/psychological-safety-clear-blocks-to-problem-solving-and-innovation)（LinkedIn Learning）
- [Improving Your Listening Skills](https://www.linkedin.com/learning/improving-your-listening-skills-19238090)（LinkedIn Learning）
- [The Six Morning Habits of High Performers](https://www.linkedin.com/learning/the-six-morning-habits-of-high-performers/six-practices-to-get-back-on-track)（LinkedIn Learning）
- [Unconscious Bias](https://www.linkedin.com/learning/unconscious-bias-14822310)（LinkedIn Learning）
- [How to Make Strategic Thinking a Habit](https://www.linkedin.com/learning/how-to-make-strategic-thinking-a-habit-22614178/why-make-strategic-thinking-a-habit)（LinkedIn Learning）
- [Leading with Emotional Intelligence](https://www.linkedin.com/learning/leading-with-emotional-intelligence-19884915)（LinkedIn Learning）

<a name="tech-domain"></a>

### 🚀 テクノロジーとドメイン知識

#### CSM のための GitLab

- [カスタマーサクセス教育・イネーブルメント（全 CS）](/handbook/customer-success/education-enablement/)
- セルフ認定は [GitLab University](https://university.gitlab.com/pages/certifications) で利用可能です。各試験には対応する無料の自己ペースコースがあります:
  - [GitLab Certified Git Associate](https://university.gitlab.com/courses/gitlab-with-git-essentials-certification-exam)
  - [GitLab Certified Project Management Associate](https://university.gitlab.com/courses/gitlab-project-management-certification-exam)
  - [GitLab Certified CI/CD Associate](https://university.gitlab.com/courses/gitlab-ci-cd-certification-exam)
  - [GitLab Certified Security Specialist](https://university.gitlab.com/courses/gitlab-security-essentials-certification-exam)
- [Learning GitLab](https://www.linkedin.com/learning/learning-gitlab-14539757)（LinkedIn Learning）
- [Continuous Delivery with GitLab](https://www.linkedin.com/learning/continuous-integration-and-continuous-delivery-with-gitlab)（LinkedIn Learning）
- [Introduction to GitLab CI & DevOps with AWS](https://www.youtube.com/watch?v=PGyhBwLyK2U)（YouTube）
  - [Introduction to GitLab CI & DevOps with AWS - コースノート](https://gitlab.com/gitlab-course-public/freecodecamp-gitlab-ci/-/blob/main/docs/course-notes.md)（Git リポジトリ）
- [GitLab CI CD Tutorial for Beginners](https://www.youtube.com/watch?v=qP8kir2GUgo)（YouTube）

#### DevOps

- [DevOps Foundations](https://www.linkedin.com/learning/devops-foundations-23454205)（LinkedIn Learning）

#### ソフトウェア開発

- [Software Development Life Cycle (SDLC)](https://www.linkedin.com/learning/software-development-life-cycle-sdlc)（LinkedIn Learning）

#### DevSecOps

- [GitLab アプリケーションセキュリティの開始](https://docs.gitlab.com/ee/user/application_security/get-started-security.html)
- [Application Security in DevSecOps](https://www.linkedin.com/learning/application-security-in-devsecops)（LinkedIn Learning）

#### Git

- [Git Basics Episode 1 - What is Version Control?](https://git-scm.com/video/what-is-version-control)（外部動画）
- [Git Basics Episode 2 - What is Git?](https://git-scm.com/video/what-is-git)（外部動画）
- [Git Basics Episode 3 - Get Going with Git](https://git-scm.com/video/get-going)（外部動画）
- [Git Basics Episode 4 - Quick Wins with Git](https://git-scm.com/video/quick-wins)（外部動画）

#### Markdown

- [ハンドブック Markdown ガイド](https://handbook.gitlab.com/docs/markdown-guide/)
- [GitLab フレーバード Markdown（GLFM）](https://docs.gitlab.com/ee/user/markdown.html)
- [ハンドブックスタイルガイド](/handbook/about/style-guide/)
- [Markdown チュートリアル](https://about.gitlab.com/blog/2018/08/17/gitlab-markdown-tutorial/)

#### Docker

- [Docker Crash Course for Absolute Beginners](https://www.youtube.com/watch?v=pg19Z8LL06w)（YouTube）
- [Mac での Docker Desktop](/handbook/tools-and-tips/mac/#docker-desktop)

#### Kubernetes

- [What is Kubernetes - Kubernetes explained in 15 mins](https://www.youtube.com/watch?v=VnvRFRk_51k)（YouTube）
- [Kubernetes Crash Course for Absolute Beginners](https://www.youtube.com/watch?v=s_o8dwzRlu4)（YouTube）

## PS テクニカルインストラクターが技術的な概念を教える方法

これらは GitLab の Education Services 部門が顧客に対して実際に行った 8 時間コースのライブ録画です。技術的な概念を体系的な形式でプレゼンテーションする方法の良い例となっています。

> Education Services チームは、これらのクラス動画の短い編集バージョンに取り組んでいます。利用可能になったら、これらのリンクを編集バージョンに置き換えます。今のところ、これらの未編集のクラス録画の中でラボや休憩はスキップしてください。

アクセスコードと録画リンクはこの[内部ドキュメント](https://docs.google.com/document/d/1WNwz_2TbavLgEVG86eENu4Mf_StjYt77h-d-364l3bM/edit?usp=sharing)に記載されています。

## キャリア開発

### CSM ジョブファミリー内

- [CSM ジョブファミリーページ](/job-description-library/sales/customer-success-management/)は CSM ジョブファミリー内の各レベルのロールと職責を最新の状態に保っています。
- [CSM スキルマトリックス](https://docs.google.com/spreadsheets/d/1_UEke64Qkz8wSyqfr_E9qqeAF6rX77w4vIH84Ckm_ts/edit#gid=0)により、CSM は次のシニアリティレベルに向けて取り組む機会の領域を特定できます。

### CS 組織内

- [SA ジョブファミリーページ](/job-description-library/sales/solutions-architect/)は SA のロール、職責、技術的期待値を最新の状態に保っています。SA ロールへの移行に興味のある CSM は、マネージャーと協力して必要な技術・営業スキルを開発するための学習パスに取り組めます。

### GitLab 内、CS 以外

- 他のチームのロールへの移動には「公式な方程式」はありませんが、GitLab 内の別の分野を探索したい CSM の成長を歓迎しています。CS 以外の異なるグループへの移動を希望する CSM は、まずジョブファミリーの要件を確認し、マネージャーにパスを決めるための相談をするのが第一歩です。マネージャーは対象グループのチームリーダーと連携して、成功の可能性が最も高い開発計画を作成することが推奨されています。
