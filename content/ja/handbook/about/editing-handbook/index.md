...
1. 編集したいハンドブックページに移動します。下部または右側にある `Edit this page` の URL をクリックして、新しい Web IDE を開きます。
   - あるいは、[公開ハンドブック](https://gitlab.com/gitlab-com/content-sites/handbook)プロジェクトを開き、編集アクションから `Web IDE` を選択します。これには、ハンドブックファイルがあるソースツリーに移動する必要があります。

   ![ハンドブックページの下部、アクション: Web IDE で編集](/images/handbook/about/editing-handbook/practical_handbook_edits_web_ide_vs_code_bottom_handbook_action.png)
