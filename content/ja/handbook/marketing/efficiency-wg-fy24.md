---
title: "マーケティング部門 効率化ワーキンググループ - FY24"
description: "効率化ワーキンググループは、マーケティング部門内における効率改善の主要 3〜5 領域を特定し、解決策を策定することを目指します。"
upstream_path: /handbook/marketing/efficiency-wg-fy24/
upstream_sha: 2d678e92f3fbc59843a2973bbfa95041c6aef07f
translated_at: "2026-04-30T12:00:00Z"
translator: claude
stale: false
---

## 属性

| プロパティ | 値 |
|-|-|
| 作成日 | 2023-07-18 |
| 終了日 | 2023-10-31 |
| Slack | [#wg_marketing_efficiency_fy24](https://gitlab.slack.com/archives/C05J8PTGF6K) |
| Google Doc | [Working Group Doc](https://docs.google.com/document/d/1-vZTwstweeJa_73nld_ABl8ar1h3ulVgylalZKtlp5g/edit#heading=h.mz0r2cgw3bv3)  |
| 概要・ステータス | [Epic](https://gitlab.com/groups/gitlab-com/marketing/-/epics/4199) |

## 概要

FY'24 Pulse Engagement サーベイは 2023-05-16 から 2023-06-27 にかけて実施されました。

効率はマーケティング部門のフォーカス領域として特定されました。このワーキンググループは、効率改善の主要 3〜5 領域を特定し、解決策を策定するために設置されました。

## 目標

1. 効率改善の主要 3〜5 領域に合意します。チームとして、WHY ＆ ROI を特定し、改善のための解決策を策定します。
2. コミュニケーションおよびチェンジマネジメント計画を策定します。
3. 変更を実施します。

## ビジネス成果と終了基準

TBD

## リスクと相互依存関係

TBD

## タイムライン

FY24 Q3: 2023-08-01 - 2023-10-31

## 役割と責任

- DRI: [Nikki Silverberg](https://gitlab.com/nikkiroth)
- スポンサー: [Denise Parsonage](https://gitlab.com/dparsonage)
- ワーキンググループメンバー: [Allison Graban](https://gitlab.com/agraban)、[Amruta Kulkarni](https://gitlab.com/akulks)、[Brian Mason](https://gitlab.com/brianmason)、[Eirini Panagiotopoulou](https://gitlab.com/eirinipan)、[Ginny Reib](https://gitlab.com/Greib)、[Indre Kryzeviciene](https://gitlab.com/ikryzeviciene)、[Katie Rogel](https://gitlab.com/krogel)、[Mao Nogales Lazarini](https://gitlab.com/mnogales)、[Michael Friedrich](https://gitlab.com/dnsmichi)
