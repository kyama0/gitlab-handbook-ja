...
- フレーキーテスト: 失敗するが、テストを実行している CI ジョブをリトライすると成功するテスト。
- Broken master:
...
1. 特定
   ...
   - `master` が**フレーキーな理由で失敗している**と特定し、信頼性を持って再現できない場合（つまり、失敗したスペックをローカルで実行するか、失敗したジョブをリトライする）:
      ...
      - インシデントをクローズします
   - エラーのスタックトレースをインシデントに追加します（gitlab-bot によってまだ投稿されていない場合）。ジョブアーティファクトで利用可能な場合は Capybara スクリーンショットも追加します。
     - スクリーンショットを見つけるには: ジョブアーティファクトをダウンロードし、利用可能な場合は `artifacts/tmp/capybara` のスクリーンショットをインシデントにコピーします。
...
1. 失敗の原因の初期評価として、[このドキュメント](https://docs.gitlab.com/ee/user/gitlab_duo_chat/examples.html#troubleshoot-failed-cicd-jobs-with-root-cause-analysis)に従って実験的な AI 支援[根本原因分析](https://docs.gitlab.com/ee/user/gitlab_duo/index.html#root-cause-analysis)機能を試すことができます。
