...
Issue を選ぶ際、エンジニアは Issue の所有権を取ることを示すシグナルとして自分をアサインし、開発の開始を示すために "workflow::in dev" に移します。
...
この例では、次のマイルストーンのキャパシティはチーム全体で 45 ウェイトです。見積もりもこの計算も正確な科学ではないことに留意してください。キャパシティ計画は EM と PM が社内外で成果物に関する現実的な期待を設定するのを助けるためのものです。予測されたウェイトの正確な量を達成することは期待していません。
