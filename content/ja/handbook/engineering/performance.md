...
まず上記の[ウェブリクエストのステップ](#web-request)を読み、その後ここから続けてください。
...
ユーザーがブラウザーを開き、`GitLab.com` のお気に入り URL を表示するシナリオを考えてみてください。ステップは[「ウェブリクエスト」](#web-request)のセクションに説明があります。このテーブルでは、ステップが測定され、改善の目標が設定されています。
