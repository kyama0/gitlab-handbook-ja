...
プランニングミーティングはマイルストーム開始前に月に1回開催されます。プロダクトマネージャーがスケジュールの DRI です。
...
成果として、ミーティングではマイルストームを推定した Issue のリストが作成される場合があります。

#### 非同期ファースト

ほとんどのプランニングは非同期で行われます。これをより効率的にするために、いくつかのツールとプロセスが観察されています。

Issue には1つのマイルストームしか添付できないため、`~"Next Up"` ラベルを使って、マイルストームがあるかどうかに関係なく、今後のマイルストームの項目をマークします。PM と EM は、プランニングプロセス中の現在のマイルストームを slipする可能性のある Item やプロスペクティブな Issue に追加する前に、このラベルをすべての Issue から削除する必要があります。

このラベルを使用することで、今後のマイルストームを簡単に分析できます。[プランニングボード](https://gitlab.com/groups/gitlab-org/-/boards/7109724)は、現在のマイルストームではなくこのラベルにスコープされているビルドボードを模倣しています。以下のために使用します:

- 提案されたすべての Issue の現在のワークフロー状態を表示する。
- 各リストのウェイト値を合計してキャパシティを計画する。
- マイルストームが始まる前に解決できる可能性のあるブロッキング関係を理解する。

新しいマイルストームが始まると、一括アクションで `~"Next Up"` ラベルの付いたすべての Issue にマイルストームを追加し、ラベル自体を削除できます。
...
|---    | --- | ---               | --- |
| ~workflow::* | Issue の現在のワークフロー状態を伝えます。進捗を理解し、マイルストーム期間中のリスクを定量化するために重要です。 | 開発全体を通じた Issue の更新 | エンジニア |
...
| ~Deliverable/~Stretch | ~Deliverable は、アサインされたマイルストーン内に Issue を届ける意図があることを顧客とステークホルダーに伝えます。~Stretch は、マイルストーム中に着手する可能性はあるが完了は期待されないことを示します。 | [リリーススコーピングラベル](https://docs.gitlab.com/ee/development/labels/#release-scoping-labels) | エンジニアリングマネージャー |
...
`~Knowledge::P1/P2/P3` ラベルを使用して、`~workflow::*` ステップとマイルストーム内の Issue の優先度を示します。

- プロダクトマネージャーがこれらのラベルの DRI ですが、チームの全員がアサイン/調整できます。
- マイルストームの開始前に、PM と EM はそのマイルストームに含まれるすべての Issue の優先度を確認します。期待値は以下の通りです:
      - Issue の 40% が `~Knowledge::P1`
      - Issue の 30% が `~Knowledge::P2`
      - Issue の 30% が `~Knowledge::P3` で、これらの Issue は `~Deliverable` にできません
- これらのラベルはマイルストーム外でも最高優先事項を追跡するために使用します。
...
- [Plan:Knowledge](https://gitlab.com/groups/gitlab-org/-/boards/1569369?scope=all&utf8=%E2%9C%93&state=opened&label_name[]=group%3A%3Aknowledge) - マイルストームフィルターを適用して現在のリリースの作業を確認する
