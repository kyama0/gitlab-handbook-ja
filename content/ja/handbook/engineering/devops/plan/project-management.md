...
- フィーチャー（Epic）- 対応するフィーチャーフラグをデフォルトで「on」にするために必要なすべての垂直フィーチャースライスを含みます。フィーチャー Epic はまた、対応するリリースポスト項目 MR を生成する場所としても機能します。フィーチャー Epic は[顧客価値を提供する最小限の機能](/handbook/product/product-principles/#the-minimal-valuable-change-mvc)にスコープされるべきです。将来の拡張のために計画された追加スコープはフォローアップ Epic に格納する必要があります。
  ...
  - 垂直フィーチャースライス（Issue）- 単一のマイルストーム内で完了でき、本番環境の `plan-stage` グループ内でテストおよび検証できるフィーチャーのサブセット。
...
技術的負債、セキュリティ Issue、バグのバックログに製品コミットメントと並行して一貫して対処できるようにするために、マイルストームベースのローテーションシステムを実施しています。各マイルストームで、1人のバックエンドと1人のフロントエンドエンジニアが、フィーチャー開発の代わりにエンジニアリングに特化した作業に専念します。
...
- 一度に1マイルストーム
- ローテーション期間中は 100% 専念
- マイルストームごとに1人のバックエンドエンジニアと1人のフロントエンドエンジニア
...
効率を向上させるための継続的な取り組みの一環として、[レガシー Issue のワークアイテムへの移行プロジェクト](https://gitlab.com/gitlab-org/gitlab/-/issues/461855)で従来のウェイト/ストーリーポイントの代わりに[キューイング技術](https://www.brightball.com/articles/story-points-are-pointless-measure-queues)を実験します。マイルストーム 17.3 から始まり、以下を行います:
...
GitLab のマイルストームは**毎月第3木曜日**に終了し、そのスケジュールに合わせた非同期の回顧を実施しています。回顧は3つのフェーズで構成されています:

#### フェーズ1: 収集

**期間:**
開始: **マイルストーム終了の3月曜日前**
終了: **リリース週の終わり（マイルストームの木曜日）**
...
#### タイムラインの例（マイルストーム 18.3 終了: 2025-07-17）
