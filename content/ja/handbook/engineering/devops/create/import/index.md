...
エンジニアリングマネージャーが Issue をスケジュールし、それがマイルストーム計画 Issue に含まれます。

各エンジニアのマイルストーム割り当てに基づいて、`Ready for Development` ステータスでない Issue を特定してください。これらは通常 `Refinement` または `Planning breakdown` ステータスを持ちますが、任意の[プロダクト開発フロー](/handbook/product-development/how-we-work/product-development-flow/)ステータスを持つ場合もあります。
...
セカンダリドクターは、プライマリが OOO（オフィス外）またはキャパシティオーバーの場合に介入し、同じ責任を担います。それ以外の場合は、マイルストームに計画された作業に取り組みます。プライマリドクターは OOO またはキャパシティの問題がある場合はセカンダリに知らせてください。

プライマリドクターのマイルストームキャパシティは100%役割の責任に割り当てられます。残っていない場合、エンジニアは `~type::maintenance` または `~type::bug` Issue に取り組み、コードレビューのキャパシティを増やすことを検討してください。
...
- マイルストームドクターが問題を正常に診断した方法に関するチームのランブックドキュメントを維持する
...
- マイルストーム終了時に [`@gitlab-com/create-team/import/reaction-rotation` のメンバーシップ](https://gitlab.com/groups/gitlab-com/create-team/import/reaction-rotation/-/group_members?with_inherited_permissions=exclude)を更新する

##### インポーター依存関係

各マイルストームで、各インポーターの依存関係の変更履歴を確認し、今後の破壊的変更や API の非推奨化がないかをチェックしてください。[GitLab Duo Chat](https://docs.gitlab.com/user/gitlab_duo_chat/) を使って影響を評価し、更新が必要な変更には関連する `~"Importer:"` ラベルを付けた Issue を作成してください。
...
1. この時点で機能はマイルストーム内でリリースされたとみなされ、セルフマネージドの顧客に届くためリリースポストで発表できます。
...
定期的にスケジュールされた「マイルストームごと」の振り返りが1回あり、「プロジェクトごと」の振り返りは随時行うことができます。

#### マイルストームごと

Import グループは [GitLab Issue でマイルストーム振り返り](https://gitlab.com/gl-retrospectives/manage-stage/import/-/work_items)を実施しています。これにはエンジニア、UX、PM、そのマイルストームでチームと協力したすべてのステーブルカウンターパートが含まれます。

チームメンバーの参加はすべてのマイルストームで強く奨励されています。
...
- [マイルストーム振り返り](https://gitlab.com/gl-retrospectives/manage-stage/import/-/work_items)
- Slack チャンネル
...
- Issue ボード
  - [現在のマイルストームボード](https://gitlab.com/groups/gitlab-org/-/boards/1459244?milestone_title=Upcoming&label_name[]=group%3A%3Aimport%20and%20integrate)
- コントリビューションガイド
