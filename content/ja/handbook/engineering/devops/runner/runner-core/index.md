...
現在のマイルストームで進行中またはスケジュールされている Issue は [マイルストームボード](https://gitlab.com/groups/gitlab-org/-/boards/9726167) で追跡できます。

このボードにはチームのワークフローを追跡するために必要なすべてのカラムが含まれており、以下が表示されます。

- 現在のマイルストームのすべての Issue
- GitLab のネイティブ Issue ステータス（"Ready for Development"・"In dev"・"In review"・"Closed" など）ごとに整理された Issue

チームメンバーがマイルストームボード上の Issue を自己割り当てしたら、現在の作業状態を反映するために GitLab のネイティブ Issue ステータスに従って更新する必要があります。

#### 次のマイルストームの作業

次のリリースに計画された Issue は以下で確認できます。

- [次のマイルストーム](https://gitlab.com/groups/gitlab-org/-/issues?sort=updated_desc&state=opened&milestone_title=Upcoming&label_name%5B%5D=group%3A%3Arunner%20core&first_page_size=50)
