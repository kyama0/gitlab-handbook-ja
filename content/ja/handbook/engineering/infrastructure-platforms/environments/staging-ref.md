...
1. `gitlab-staging-ref` プロジェクトの [`gitlab-toolbox`](https://console.cloud.google.com/kubernetes/deployment/us-east1/staging-ref-3k-hybrid-us/default/gitlab-toolbox/overview?project=gitlab-staging-ref) ワークロードにアクセスする
1. **管理対象 Pod** セクションで、実行中の Pod の **名前** をクリックします。例えば、**名前** は `gitlab-toolbox-5955db475c-ng2xr` のようになります。
1. 上部付近にある **Kubectl** ドロップダウンをクリックする
1. **Exec** にカーソルを合わせてサブメニューを表示する
1. **toolbox** をクリックする
...
1. **Connect** をクリックする
1. コマンドをコピーし、ローカルで実行して `kubeconfig` を取得する
1. [Kubernetes チートシート](https://docs.gitlab.com/charts/troubleshooting/kubernetes_cheat_sheet.html#gitlab-specific-kubernetes-information) に従う
...
1. [Admin エリア](https://docs.gitlab.com/ee/administration/) に移動する。
1. アップグレードしたいエンティティに応じて、Users または Groups セクションを選択する。
1. 名前でユーザーまたはグループを検索し、「Edit」をクリックする。
1. 「Plan」で必要な有料プランを選択する。
1. 「Save changes」をクリックする。
...
1. [管理者](#admin-access) として、[https://staging-ref.gitlab.com/groups/saml-sso-group](https://staging-ref.gitlab.com/groups/saml-sso-group) にグループが存在しない場合は作成する。
1. このグループの[料金プランをアップグレード](#upgrade-paid-plans)して「Premium」にする。
1. [https://staging-ref.gitlab.com/groups/saml-sso-group/-/saml](https://staging-ref.gitlab.com/groups/saml-sso-group/-/saml) にアクセスして：
   ...
   - 「Enforce SSO-only authentication for web activity for this group」にチェックを入れる
   - 「Identity provider single sign-on URL」の値を 1Password に保存されている値に更新する
   - 「Certificate fingerprint」の値を 1Password に保存されている値に更新する
