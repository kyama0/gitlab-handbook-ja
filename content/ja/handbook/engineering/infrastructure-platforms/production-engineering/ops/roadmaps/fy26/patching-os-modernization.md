...
### Ubuntu 20.04 のデプロイ停止 {#stop-deploying-ubuntu-2004}
...
### 仮想マシン上で動作する必要があるサービスの文書化 {#document-services-that-must-run-on-virtual-machines}
...
### VM 以外のインフラワークロードにおけるパッチのギャップへの対処 {#address-patch-gaps-in-non-vm-infrastructure-workloads}
...
### 共有ランナーの COS イメージの更新サイクルを確立する {#establish-update-cadence-for-shared-runner-cos-images}

1. CI ジョブ VM 上の Container Optimized OS に対して標準化された更新スケジュールを実装し、一貫したセキュリティパッチ適用を確保します

### Dedicated/Cells とのプロセス統一 {#align-processes-with-dedicatedcells}
