...
以下に挙げるライブラリは初期候補であり、最終選択ではありません。フォローアップ MR でこれらや他の選択肢を調査して、各コンポーネントの最適な選択肢を特定し、本 ADR は適宜更新されます。

### 1. バックグラウンドジョブ処理: Redis ベースのキュー

Artifact Registry は、ライフサイクルポリシー実行、ガベージコレクション、イベント処理、アナリティクス集計、クリーンアップ操作のためにバックグラウンドジョブ処理を必要とします。これには、信頼性のあるスケジューリング（cron スタイルおよび一回限り）、リトライと失敗処理、優先度キュー、モニタリングと可観測性が含まれます。

バックグラウンドジョブ処理ライブラリは、サテライトサービス全体での一貫した使用のため、LabKit 抽象でラップされます。

**候補ライブラリ: [asynq](https://github.com/hibiken/asynq)**:

- MIT ライセンス、活発にメンテナンスされており、設計上 Redis のみ。追加のインフラ依存関係なし。
- GitLab エンジニアに馴染みの Sidekiq に似たセマンティクス（キュー、リトライ、デッドレターキュー）
- 優先度キュー、cron スタイルスケジューリング、ユニークジョブ重複排除をサポート
- 内蔵の Prometheus メトリクスエクスポーターと web UI（[asynqmon](https://github.com/hibiken/asynqmon)）

**補完オプション: [River](https://github.com/riverqueue/river)**:

- トランザクショナルなエンキュー: ジョブはデータ変更と同じ PostgreSQL トランザクション内でエンキューされ、ジョブが起源トランザクションのコミット前に実行されたり、ロールバック後に実行されたりする競合状態を排除する。
- メタデータ書き込みとバックグラウンドジョブスケジューリング間の原子性が必要な操作に関連する可能性がある。
- 優先度キュー、cron スタイルスケジューリング、ユニークジョブ、スヌーズをサポート
- 強く型付けされたジョブ引数のためにジェネリクスを使用する Go ネイティブ設計
...
Redis プロトコル互換サービス（例: Redis、Valkey）はバックグラウンドジョブ処理にすでに必要であるため、追加のインフラ依存関係は導入されません。これはキャッシング、レート制限、分散ロック、エフェメラルな共有アプリケーション状態をカバーします。
