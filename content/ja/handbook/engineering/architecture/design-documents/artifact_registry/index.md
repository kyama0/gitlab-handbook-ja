...
<td class="px-3 py-2 border border-gray-300"><a href="https://gitlab.com/jdrpereira" class="text-blue-600 hover:underline">@jdrpereira</a>, <a href="https://gitlab.com/10io" class="text-blue-600 hover:underline">@10io</a>, <a href="https://gitlab.com/dmeshcharakou" class="text-blue-600 hover:underline">@dmeshcharakou</a></td>
...
<td class="px-3 py-2 border border-gray-300">2025-12-15</td>
</tr>
</tbody>
</table>
</div>
...
GitLab の現在のプロジェクトレベルのパッケージレジストリやコンテナレジストリとは異なり、Artifact Registry は、すべてのアーティファクト（コンテナ、パッケージ、ML モデル）に対する集中的な制御、可視性、管理機能をエンタープライズに提供します。
...
- **Organization**: GitLab の最上位コンテナエンティティで、主要な分離およびシャーディング境界として機能します。ワークスペース、リポジトリ、すべてのアーティファクトを含みます。各 Organization は独立したストレージ、コスト按分、重複排除スコープを持ちます。
- **Workspace**: 組織内のリポジトリを論理的にグループ化するもので、チーム、セキュリティドメイン、または製品ラインごとにアーティファクトを整理します。組織構造のマッピングおよびワークスペーススコープのアクセス制御を可能にします。（MVP 後の機能。名称は UX 検証次第で変更される可能性があります。）
...
- **RBAC (Role-Based Access Control)**: 組織、ワークスペース、またはリポジトリレベルでロールに基づいて割り当てられる権限。
...
- **開発速度のギャップ**: コンテナレジストリは Rails モノリスのツール（マイグレーション、機能フラグ、自動リリース）を限定的にしか利用できず、機能提供が遅くなっています <!-- (see [ADR-005](decisions/005_implementation_architecture.md)) -->
...
1. プロジェクトレベルの断片化を排除する**組織レベルのアーティファクト管理**を提供する
2. ツール、検証、安全制御を備えた JFrog Artifactory および Sonatype Nexus からの**マイグレーションを可能にする**
3. **AI 駆動の最適化**を実現する: 構成支援、コスト推奨、予測インサイト
...
8. **マルチフォーマットアーティファクト**をサポートする: コンテナ（Docker、OCI）、パッケージ（Maven、npm）、および将来のフォーマット
...
- アーリーアダプタの顧客が本番ワークロードでプラットフォームを検証する
...
- ワークスペースベースの構造により、大規模エンタープライズが自社のチーム階層をマッピング可能
...
<!--
1. [ADR-005: Implementation Architecture](decisions/005_implementation_architecture.md) - Rails monolith vs satellite service decision
-->
...
1. [ADR-023: Code Structure and Enforcement](decisions/023_code_structure_and_enforcement.md) - 機能ごとのパッケージ構成を伴う Go の `cmd/` + `internal/` レイアウト
