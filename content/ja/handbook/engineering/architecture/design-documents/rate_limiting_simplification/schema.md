...
- このスキーマで定義された設定を管理するためのユーザーインターフェースの構築（[フェーズ 3](_index.md#phase-3-implement-a-rate-limit-interface)）
- 特定のレート制限値の定義（スキーマは構造を定義し、実際の制限値ではありません）
- 集中型レート制限サービスの作成（[フェーズ 3](_index.md#phase-3-implement-a-rate-limit-interface)）
