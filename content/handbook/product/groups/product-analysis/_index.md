...
私たちは、2週間のイテレーション内で完了できると確信できる作業のみにコミットすることを目指します。そのため、最近のベロシティ *未満* にコミットし、緊急の Issue と中断を考慮するバッファを残します。最初は、各アナリストが約2日分の作業のバッファを残します（最近の未計画作業のボリュームに基づく推定）。割り当てられたバッファを超える高優先 Issue は、計画された作業を完了する能力に大きな影響を及ぼすため、Product Data Insights チームの支援が必要になることが分かっている場合は事前に計画してください。
