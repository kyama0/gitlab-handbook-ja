...
SFDC のバックログを回避またはクリアするために、Marketo ではマルチスレッド同期が利用可能です。これにより、SFDC から Marketo にデータが流れる複数のレーンを使用できるようになり、Marketo サーバーで利用可能なリソースに応じて同期速度が大幅に向上します。Marketo の各レコードには、メールアドレスの最初の文字に基づいて `Marketo Thread ID` フィールドに `1-9` の値がマークされます。レコードは `Marketo Thread ID` の値に基づいてスレッド上で同期および分散されます。
...
**注意: この機能は常時稼働ではなく、Marketo サポートのみが有効化できます。SFDC からの大規模な更新を計画する 7 日前、または重大なバックログがある任意のタイミングで、サポートチケットを開く必要があります**

#### マルチスレッド同期の有効化

有効化するには、私たちがチケットを Marketo と開けるよう、7 日前にマーケティングオペレーションチームに [Issue を作成](https://gitlab.com/gitlab-com/marketing/marketing-operations/-/issues/new?issuable_template=request-marketo-multithread)し、以下を含めてください。

1. 大規模更新の予定日
1. ビジネスへの影響
1. バックログを引き起こすことになる更新フィールド

### サンドボックス

Marketo にはサンドボックスがあります。サンドボックスはトレーニング、API リンクの作成、本番に移す前の全体的なテストに使用されます。サンドボックスから本番にプログラムを `昇格` する方法はないので、最初にサンドボックスでプログラムを構築することは常に必要ではありません。サンドボックスで構築する際のガイドラインは TBD ですが、カスタム API および Webhook 統合では強く推奨されます。
...
FY25 開始時に行われたリードスコアリングへのデータ駆動型変更の詳細については、データセットの調査結果をレビューするために、[このスライドデッキ](https://docs.google.com/presentation/d/14bTq_KOyG7jHJR7vghcUVAZ_tMSOgbAN7hI8auI49eU/edit#slide=id.g1d24c3e4ddd_5_252)と[このスライドデッキ](https://docs.google.com/presentation/d/1B8Q3Rq6O7-641rFoGOJOG6TxKHM_uAU29Ejbzqr2KMI/edit#slide=id.g1d24c3e4ddd_5_252)を参照してください。

#### スコアリングロジックドキュメント

スコアに**直接**影響するすべてのスマートキャンペーンが文書化されており、[この Google Sheet](https://docs.google.com/spreadsheets/d/1A8ip-_e7CODnZjkigwy6zHV9uT6sqzAoeornixaK8IY/edit?gid=0#gid=0) で確認できます。これは社内 GitLab チームメンバーのみが利用できることに注意してください。
...
|*Trial - Default | SaaS  |    +20| 1日1回    |
|*Trial - Personal | 一般的なメールドメインでサインアップ: <br> SaaS,<br>Self-Managed|    +10|1日1回    |
|*Trial | SaaS,<br>Self-Managed  |    +30| 1日1回    |
