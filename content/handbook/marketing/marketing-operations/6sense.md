...
1. Sales Intelligence ダッシュボードで、購買者の intent シグナルを示しているアカウントの `Profile Fit`、`Account Reach`、`Buying Stage` を確認してください。
2. Persona Map で、強い `Engagement Grade` を持つ理想的な顧客プロファイルに合致するリードを特定してください。
3. リードを選択して `add to Outreach` をクリックしてください。
4. Outreach から Outreach Sequence のプロンプトが表示されるので、リードを追加したいシーケンスを選択してください。
