...
## エンリッチメント（RingLead Enrich Premium）
 
GitLab は RingLead Enrich Premium を使用して、ZoomInfo 経由で Salesforce の Lead および Contact レコードをエンリッチします。日次、週次、月次のケイデンスにわたって約 719,456 件のレコードをカバーする 10 件のアクティブなエンリッチメントジョブがあります。
 
認識しておくべきアクティブな問題がいくつかあります:

1. 新しいエンリッチメントデータが利用できない場合でも多くのレコードが再処理されており、それでもエンリッチメントの日付やフィールドが更新され、Salesforce のデータ変更のボリュームが不必要に膨らんでいます。
2. 月次エンリッチメントジョブの日付も毎月手動更新が必要です。RingLead は相対的な週（例: 「第 2 木曜日」）ではなく、特定のカレンダー日付でのスケジューリングしかサポートしていないためです。
 
### エンリッチメントの概要
 
| ジョブ | オブジェクト | ケイデンス (PT) | 
|---|---|---|
| [Leads Created in the Last 7 Days](https://gitlab.ringlead.com/enrichment/152606/general/) | Lead | 日次 – 7 PM | 
| [Lead Enrichment w/ Company Unknown](https://gitlab.ringlead.com/enrichment/154809/general/) | Lead | 週次 – 木 7 PM | 
| [Lead State Enrichment](https://gitlab.ringlead.com/enrichment/154997/general/) | Lead | 週次 – 土 12 AM | 
| [Contact State Enrichment](https://gitlab.ringlead.com/enrichment/154998/general/) | Contact | 週次 – 水 12 AM | 
| [Inquiry Leads – This FY](https://gitlab.ringlead.com/enrichment/154970/general/) | Lead | 月次 – 第 1 金曜日 2 PM | 
| [MQL / Accepted / Qualifying Leads](https://gitlab.ringlead.com/enrichment/155047/general/) | Lead | 月次 – 第 2 木曜日 5 PM | 
| [Inquiry Leads – Last FY](https://gitlab.ringlead.com/enrichment/155031/general/) | Lead | 月次 – 第 2 金曜日 2 PM | 
| [MQL / Accepted / Qualifying Contacts](https://gitlab.ringlead.com/enrichment/155047/general/) | Contact | 月次 – 第 3 木曜日 7 PM | 
| [Raw Leads](https://gitlab.ringlead.com/enrichment/154973/general/) | Lead | 月次 – 第 3 金曜日 5 PM | 
| [Raw / Inquiry Contacts](https://gitlab.ringlead.com/enrichment/155046/general/) | Contact | 月次 – 第 4 金曜日 7 PM | 
 
### エンリッチメントジョブ
 
#### Leads Created in the Last 7 Days
 
エンリッチメントは Marketing Ops によって管理され、毎日 7 PM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
...
エンリッチメントは Marketing Ops によって管理され、毎週木曜日 7 PM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
 
- Company contains `unknown`

**注意:** シンプルなフィルター、高度なロジックなし。
 
#### Lead State Enrichment
 
エンリッチメントは Marketing Ops によって管理され、毎週土曜日 12 AM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
...
エンリッチメントは Marketing Ops によって管理され、毎週水曜日 12 AM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
...
エンリッチメントは Marketing Ops によって管理され、毎月第 1 金曜日 2 PM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
...
エンリッチメントは Marketing Ops によって管理され、毎月第 2 木曜日 5 PM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
...
エンリッチメントは Marketing Ops によって管理され、毎月第 2 金曜日 2 PM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
...
**注意:** 高ボリューム。MQL Leads と同じ週に実行されます — システム負荷を監視してください。
 
#### MQL / Accepted / Qualifying Contacts
 
エンリッチメントは Marketing Ops によって管理され、毎月第 3 木曜日 7 PM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
...
エンリッチメントは Marketing Ops によって管理され、毎月第 3 金曜日 5 PM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
...
エンリッチメントは Marketing Ops によって管理され、毎月第 4 金曜日 7 PM PT に実行されます。レコードフィルター — 以下の場合にレコードがエンリッチされます:
