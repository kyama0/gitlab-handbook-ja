...
3. Campaign Owner／Operations（Fund Requester は Campaign Owner も兼ねることが可能）
   ...
   2. GitLab Hyperscaler Funds Issue を作成
   3. Hyperscaler Funds Issue リンクを [スプレッドシート](/handbook/marketing/channel-marketing/hyperscalers/#application-process) に追加
   4. Issue 同期をアクティブ化して Asana プロジェクトを作成しキャンペーンを管理
   5. Marketo と Salesforce の実行・セットアップを管理
   6. Zip リクエストをオープン
   7. Proof of Performance のために Program Owner に領収書を送付
...
GitLab Hyperscaler Funded Campaign Issue は、Asana に同期する前の中間ステップとして使用されます。
...
3. GitLab Hyperscaler Funds Issue の詳細が完了したら、Campaign Owner は GitLab Hyperscaler Funds Issue に `Asana-Sync-Project` を追加し、FMM と FMC を追加して [Plan to WIP プロセス](/handbook/marketing/field-marketing/#process-for-moving-events-from-plan-to-wip) を開始します。
4. Campaign Owner は、GitLab Hyperscaler Funded Campaign Issue の Resources セクションに Asana プロジェクトリンクを追加します。

#### Asana プロジェクト

Asana の Regional Marketing プロセスに対応するため、Hyperscaler Funded Campaign のステップを [Plan to WIP プロセス](/handbook/marketing/field-marketing/#process-for-moving-events-from-plan-to-wip) に統合しています。Regional Marketing Asana テンプレートには、適切なタスクにハイパースケーラープロセスの詳細が組み込まれています。
...
20250307_GCP_CR_ActivityType_ActivityName_Region（詳細は Regional Marketing Asana テンプレートのプログラムトラッキングタスクで提供）
   ...
   3. Asana Project Overview の Marketo および Salesforce へのリンクを更新

2. Zip リクエストをオープン（詳細は Regional Marketing Asana テンプレートの contract request タスクで提供）
   ...
   6. Asana Project Overview の Zip リクエストへのリンクを更新

##### イベント後

詳細は Regional Marketing Asana テンプレートの Ecosystem セクションで提供

1. Asana プロジェクトに Proof of Payment（POP）を添付し、Francine をタグ付け
...
1. Field Marketing がマーケティング活動を管理する場合、Hyperscaler MDF GitLab Issue を使用し、トラッキングと実行のために Plan to WIP Asana Project に接続します。
2. Channel Marketing がマーケティング活動を管理する場合、Channel Marketing MDF GitLab Issue を使用します:
    ...
    2. Issue とプロジェクトのセットアップ:
       ...
       2. 標準のチャネルマーケティングワークフローのために MDF Asana Project にリンク
