...
GitLab が関与する今後のイベントについては、`TBD new Marketing calendar` をご覧ください。
...
- **Know Before & After You Go ドキュメント:** `Glean エージェントが近日公開予定！`
- **Plan to WIP:** チームは [Plan to WIP プロセス](/handbook/marketing/field-marketing/#process-for-moving-events-from-plan-to-wip) において DAP に大きく依存しており、これによってグローバルイベントすべてのバックエンドのプロセス／セットアップ／計画をチームがナビゲートします。
...
以下は、Regional Marketing のイベントが draft／plan フェーズから最終化されて、GitLab でアクティブに管理される（WIP）状態になるときのプロセスです。
...
- **Plan to WIP** - GitLab イベント Issue で `Operations Checklist` が完了し、FMC にメンションされてから 3 営業日
- **Marketing Operations Requests** - [7 営業日](/handbook/marketing/marketing-operations/campaign-operations/#slas)

### 重要な GitLab アクセス手順

すべてのユーザーが Duo Agentic を利用する際に無制限のクレジットを受け取るために、以下の手順に従う必要があります。

1. **GitLab** で、右上のアバターを選択します。
1. **Preferences** を選択します。
1. **Behavior** セクションまでスクロールします。
1. **Default GitLab Duo** namespace ドロップダウンから、自分の namespace を選択します。
...
- FMM はイベントを Allocadia の適切なプランに追加し、すべてのサブカテゴリーパネルフィールドを記入し、最初のラインアイテムを作成する
- FMM はサブカテゴリーに戻り、パネルの一番下までスクロールして `Create/Update Marketing Issue` をクリックし、GitLab Issue を作成する
- FMM は `Operations Checklist` を記入し（すべて完全に記入する必要があります）、その後 Issue で FMC をメンションして、Issue を WIP に移行するよう依頼する
...
- FMC は Allocadia のサブカテゴリーおよびラインアイテムのパネル詳細をレビューし、その施策の予測コストが正しい月（[prepaid ポリシー](/handbook/finance/accounting/#prepaid-expense-policy) に従って）になっていることを確認し、必要な調整を行う
- FMC は `Operations Checklist` のすべての詳細が記入されていることを確認する
- FMC は GitLab 画面の右上で Duo を開き、`Planner` エージェントを選択する
  - FMC は `Agentic` が `ON` に切り替わっていることも確認する
...
- Duo によってすべての関連するエピックと Issue が作成されたら:
  ...
  - FMC はプログラムトラッキングとコピー作成（write copy）の Issue を完了する
- 次に FMC は GitLab Issue で `mktg-status::plan` ラベルを `mktg-status::wip` に変更し、イベントが WIP に移行されたことを FMM にメンションする

#### GitLab Issue のクローズ

イベントが終了し、リードが正常に更新され、すべてのタスクが完了したら、FMM は GitLab Issue をクローズします（Issue の下部にスクロールして `Close Issue` をクリックするか、Issue 右上の三点メニューに移動して `Close Issue` をクリックします）。
...
- 該当する場合、FMC は Zip リクエストで調達担当者とファイナンスをメンションして、イベントがキャンセルされたことを通知する。
...
- 注文が発送されると、Boundless は Zip を通じて請求書を直接提出します。
...
- Pizzatime で注文を行うには、支払いに [Mastercard Virtual Card](https://gitlab.com/groups/gitlab-com/Finance-Division/procurement-team/-/wikis/home/Virtual-Card-Requests) のリクエストが必要であることに注意してください。
- イベントに関連するすべての必要な詳細を含む仮想カードの契約リクエストを FMC に提出してください。
- **注**: すべてのコスト見積もりが Allocadia に追加されていることを確認してください。最終的な Pizzatime 注文コストがリクエストされた仮想カードの金額より 10% 多い場合は、FMC に通知してください。FMC は Zip 経由で PO Change Request を提出する必要があります。
...
1. FMC に [契約リクエスト](/handbook/marketing/field-marketing/#regional-marketing-contract-requests) を提出します。注文が承認されてカードが発行されると、仮想カード情報を提供します。
1. FMC からの仮想カード情報を取得したら、Pizzatime ウェブサイトで Pizzatime 注文の予約/支払いを進めます。
...
- パートナーが合意書に署名したら、FMM/FMC は GitLab 署名のために Issue で調達パートナーをメンションする
...
Regional Marketing では、FMC が自分の地域のすべての [Zip 申請](/handbook/business-technology/enterprise-applications/guides/zip-guide) を管理します。承認のために提出する準備ができている契約または請求書については、FMM は（[Plan から WIP へのプロセス](/handbook/marketing/field-marketing/#moving-from-plan-to-wip)で作成された）[Contract Request](https://gitlab.com/gitlab-com/marketing/field-marketing/-/blob/master/.gitlab/issue_templates/Contract_Request.md) Issue を利用し、必要な重要な詳細を提供する手順に従います。

#### SLA

FMC が Zip へのリクエストを提出する SLA は、契約リクエスト Issue で割り当て／メンションされてから **3 営業日** です。調達プロセス中にすべての承認や署名を取得するのに通常 1 週間以上かかることに注意してください（legal からの redline が必要な複雑な契約の場合はさらに長くなる）。それに従って計画してください。

#### ドキュメントレビュー

契約リクエスト Issue を完了する *前に*、ドキュメントを慎重にレビューし、合意書に必要な契約詳細を事前に記入していることを確認してください。FMM と FMC の両方が契約をレビューする際に留意すべき項目をいくつか以下に示します...
...
**2026 年 6 月 1 日**より開始される Brex/ZIP を通じた新しい Mastercard 仮想カード（Coupa 仮想カードを置き換えるもの）に関する詳細は [こちら](https://gitlab.com/groups/gitlab-com/Finance-Division/procurement-team/-/wikis/home/Virtual-Card-Requests) を参照してください。以下の詳細は新しい Mastercard プロセスに基づいて更新されています。

#### Mastercard 仮想カードの領収書

各仮想カードの支払いについて、領収書と支払いのレビューが必須になりました。この新しいプロセスの詳細は、**ステップ 9** の下にある [こちら](https://gitlab.com/groups/gitlab-com/Finance-Division/procurement-team/-/wikis/home/Virtual-Card-Requests#steps-after-the-virtual-card-is-generated) を参照してください。
...
いかなる種類のスワッグ購入にも Navan カードを使用しないでください。いかなる種類のスワッグ購入にも、仮想カードまたは調達プロセスを通じた請求書支払いのみが許可されます。

#### 仮想カードの制限

仮想カードの制限は、承認された PO 金額に直接関連していることに注意してください。仮想カードの支出は $25k に制限されなくなりました。カードには引き続き 10% のオーバーがあり、PO 金額の 10% 以上を請求しようとすると、カードは拒否されます。カードの制限に追加の資金を追加する必要がある場合は、FMC に [PO Change Request](/handbook/business-technology/enterprise-applications/guides/zip-guide/#how-to-do-a-request-change) を提出して PO の合計とカードの制限を増やすようリクエストする必要があります。

#### Mastercard の受け入れ - 新しいベンダーのオンボーディング

契約リクエストを提出する **前に**、ベンダーが Mastercard を利用しているかを必ず確認してください。利用していない場合は、ACH/小切手による支払いが必要となり、これにはベンダーが調達システムに登録されている必要があります。まだ登録されていない場合は、契約承認の前にオンボーディングする必要があります。FMC は [Zip リクエスト](/handbook/business-technology/enterprise-applications/guides/zip-guide/#how-to-request-a-new-vendor) でベンダーのオンボーディングをリクエストします。

#### 仮想カード 1 枚につきベンダー 1 社

新しい Mastercard 仮想カードでは、1 回の ZIP 提出で複数ベンダーのリクエストを提出することができなくなりました。これまで AV、グラフィック、カーペット、リードスキャナーなどを 1 つの ZIP で提出していたブース支払いについては、各ベンダー/注文がそれぞれ独自の ZIP リクエストを持つように分割する必要があります。
...
イベントの正確なヘッドカウントを事前に持っていないことが多いので、追加コスト（AV、F&B など）は初期の調達承認のために見積もることができます。追加コストが初期の会場契約に含まれている場合は、会場に契約での見積もりヘッドカウントの価格設定（特に F&B コストの場合、F&B 最低料金だけを提出していないようにするため）を求めてください。これにより、承認のためのベースラインコスト見積もりが得られ、FMC は最終コストに基づいて必要に応じて PO 金額を調整します（PO 変更リクエストは、元の PO 金額より 10% 以上の料金に対して発行されます）。追加コスト（ケータリング、AV など）が会場と異なるベンダーから提供される場合は、新しいベンダーの注文をメインの契約とは別に、それ自身の ZIP で提出していることを確認してください（上記の `仮想カード 1 枚につきベンダー 1 社` セクションを参照）。ベンダーが契約を必要とせず、料金が $5,000 未満の場合は、Navan カードで支払うことができます。
...
- 新しいチームメンバーのマネージャーは、[このテンプレート](https://gitlab.com/gitlab-com/marketing/field-marketing/-/blob/master/.gitlab/issue_templates/Regional_Mktg_Onboarding.md) から新しいチームメンバーの Regional Marketing オンボーディング Issue を作成する責任があります。
