...
upstream_sha: bad07717330a36b6f62aee0e67888a18c4cafdfd
translated_at: "2026-06-21T21:06:16Z"
translator: claude
stale: false
...
Click-Through Demo Generator は、GitLab デベロッパーアドボカシーチームが構築したエージェント型ツールで、スクリーンショットのフォルダを、ホスト型のインタラクティブな製品ウォークスルーに変換します。エディタも録画もビルドステップも不要です。[サンプルデモ](https://click-through-demo-generator-v-2-d63870.gitlab.io/demos/demo-20260506-094904/)をご覧ください。

#### 仕組み {#how-it-works}

スクリーンショットをフォルダにドロップして、AI エージェントにプロンプトを与えます。エージェントが画面を並べ替え、ナレーションを書き、マージリクエストをオープンします。マージリクエストがマージされると、デモが GitLab Pages に公開されます。後で変更したい場合は、エージェントに画面の並べ替えやナレーションの書き直しを依頼すると、デモが再ビルドされます。

#### メリット {#benefits}

1. 高速かつ効率的 — 手作業で何時間もかけて組み立てて編集する代わりに、プロンプト 1 つでスクリーンショットから公開済みデモへと進めます。
1. 動画作成より速い — 動画を録画することなく、製品フローをシンプルに案内する方法が必要なときにクリックスルーデモを使用します。動画とは異なり、ナレーションと画面の順序を後から修正するのが容易です。
1. ベンダー不要、コスト不要 — 外部のデモプラットフォームを GitLab ネイティブのワークフローに置き換えます。すべてが GitLab プロジェクト内に存在し、追加でライセンスが必要なものはありません。
1. レビューとメンテナンスが容易 — ナレーションが Git 内に存在するため、ツール間でコピー＆ペーストするのではなく、通常のマージリクエストを通じて更新が行われます。これによりレビューが容易になり、製品の進化に合わせてデモを最新に保ちやすくなります。

誰が使えるのか? 全員です — [リポジトリをフォーク](https://gitlab.com/gitlab-da/projects/click-through-demo-generator-v-2/-/forks/new)して、すぐにテストを始められます。
