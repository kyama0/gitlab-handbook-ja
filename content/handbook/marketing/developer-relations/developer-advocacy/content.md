...
注: Narrative ボックスのオン／オフを切り替えるには、Navattic デモでは `ctrl+p`、エージェント型デモでは `h` を使用します。どのプラットフォームで作成されたデモかを確認するには、デモ URL を参照してください（Navattic の URL には `navattic.com` が含まれます）。ナビゲーションボックスがない場合は、左右の矢印キーで移動してください。
