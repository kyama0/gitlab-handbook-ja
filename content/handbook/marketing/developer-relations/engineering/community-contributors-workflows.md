...
`gitlab-corporate-cla` 下のサブグループは、[`gitlab-corporate-cla/config`](https://gitlab.com/gitlab-corporate-cla/config) プロジェクトで OpenTofu を介して宣言的に管理されています。すべての標準的なグループ設定（可視性、説明、プロジェクト/サブグループ作成レベル、wiki 無効化、メンション無効化、share-with-group ロックなど）は、パイプラインによって自動的に適用されます。

1. [`groups.yaml`](https://gitlab.com/gitlab-corporate-cla/config/-/blob/main/groups.yaml) にエントリを追加するマージリクエストを [`gitlab-corporate-cla/config`](https://gitlab.com/gitlab-corporate-cla/config) に対して開きます:
    ...
    1. `name` — 組織の正式名称（グループ表示名および標準説明で使用されます）。
    1. `path` — サブグループの URL セーフなスラッグ（小文字、スペースはハイフン）。
    1. `description` — *(任意)* 標準の `"Approved contributors for {name} under the GitLab Corporate CLA"` テンプレートの上書き。組織が説明に追加のコンテキストを必要とする場合に使用します。

1. MR パイプラインは `tofu plan` を実行し、差分を投稿します。プランが期待どおりの単一の新規グループを示したら、レビューしてマージします。
1. `main` ブランチのパイプラインは `tofu apply` を実行し、サブグループを作成します。安全策として、ドリフト検出が `main` に対してスケジュールで実行されます。
1. apply 後の手動手順を実行します（これらの設定は [GitLab Terraform プロバイダー](https://registry.terraform.io/providers/gitlabhq/gitlab/latest/docs/resources/group) ではまだサポートされていないため、手動で構成する必要があります）:
    1. 新しいサブグループの **Settings > General > Permissions and group features** で、次を設定します:
        1. **Customer relations is enabled** — 無効化
        1. **GitLab Duo availability** — `Always off`
    1. 組織の指定されたユーザーマネージャーのユーザーアカウントを `Owner` としてグループに追加します（**Group information > Members** の下）。
    1. ご自身のユーザーアカウントをサブグループの直接メンバーから削除します（**Members** の下、`Membership = Direct` でフィルタし、'kebab メニュー' をクリックして 'Leave Group' を選択）。
