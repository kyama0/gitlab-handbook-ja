...
translator: claude
stale: false
lastmod: "2026-05-21T13:18:55+01:00"
---
...
GitLab for Education の申請プロセスのインタラクティブなステップバイステップのウォークスルーは、[Navattic](https://gitlab.navattic.com/g3x05ub) から利用できます。
...
* "initialized" ステータスは、ユーザーが検証プロセスを開始したものの、何かを送信したかどうかが不明な状態を示します。このステータスは、ユーザーの提出が不完全で Proxi.id の Web サイトに何も情報を入力していないことを意味する場合**もあれば**、Proxi.id の Web サイトに送信したものの、まだ検証を完了していないことを意味する場合もあることに注意してください。
* "rejected" ステータスは、ユーザーの提出が却下されたことを示します。"Feedback" 列には却下理由が記載されており、対象外の学生からの申請、または機関がプログラムの基準を満たさないことのいずれかとなります。

GitLab チームメンバーは、ユーザートラッキング ID を添えて gitlab-support@proxi.id にメールすることで、現在のステータスについて Proxi.id チームにフォローアップできます。Proxi.id は Web サイトを通じて送信されたユーザーデータを定期的に削除するため、数週間前に行われた提出は削除されている場合があることに注意してください。
