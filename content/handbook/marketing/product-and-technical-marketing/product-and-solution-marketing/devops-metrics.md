...
upstream_path: /handbook/marketing/product-and-technical-marketing/product-and-solution-marketing/devops-metrics/
...
リードタイムは素晴らしいメトリクスです。これは何が実現可能かを直感的に把握できる優れた指標だからです。何らかの形で DevOps プロセスを管理している場合、これを全体的なヘルスチェックとして見ることができます。リードタイムが数ヶ月の場合、システムやプロセスを合理化・自動化する機会の領域がおそらくいくつかあります。
...
### 平均復旧時間 (MTTR) {#mean-time-to-recover-mttr}
...
DevOps チームがこれを完全に管理し、障害から数分で復旧できるなら、ビジネスにより多くの（許容可能な）リスクを取るよう促し、実験にも前向きになってもらえます。最終的には、収益と利益の増加につながります。

### 変更失敗率 (CFR) {#change-failure-rate-cfr}
...
Flow Load はしばしば Work in Progress (WIP) として測定されます。直感的なレベルでは、コンテキストスイッチや同時に多くの仕事を抱えすぎることにはコストがあると誰もが知っていますが、進行中の項目の数を追跡し、結果と相関付けることで、そのコストを定量化し、どれだけ作業を引き受けるかについてデータ駆動の意思決定を行えます。
