...
このページでは、GitLab の Developer Advocacy チームが使用するクラウドリソースとワークフローの概要を説明します。リソースを効果的に管理・割り当てるためのガイダンスを提供し、長期的な本番環境と個人の短期的なリソースの両方でベストプラクティスを維持しながら、チームメンバーがコミュニティプロジェクトにクラウドインフラストラクチャーを活用できるようにします。

## クラウドリソース {#cloud-resources}

DRI は Developer Advocacy（GCP 管理者）であり、Co-Create & Community Engineering と共有しています。
...
| Contributors Success | ストレージ/トラフィック | 本番環境 | Co-Create & Community Engineering | ストレージ | [GDK-in-a-box 仮想イメージ](https://gitlab.com/gitlab-org/gitlab-development-kit/-/blob/main/doc/gdk_in_a_box.md) |
