...
1. CMM はパートナーとの定例の機会に、イベントの日程、イベント／キャンペーンの詳細、申請金額について話し合います。
     1. パートナーが Impartner のファンドリクエストフォームに入力する情報を、PowerPoint でレビューします。
...
1. 完了したら、運用サポートの DRI に依頼して Issue を [Plan から WIP](/handbook/marketing/growth-marketing/regional-marketing/#process-for-moving-events-from-plan-to-wip) へ移動してもらいます。
...
#### ステップ 3: Salesforce Campaign に MDF Request を追加 {#step-3-add-mdf-request-on-the-salesforce-campaign}
...
1. MDF Request Details の Status を `Cancelled` に更新します - こうすることで、最初に送信されたファンドリクエストがキャンセルまたは却下されたことを通知する自動メールがパートナーに届きます。
...
パートナーが MDF ページにアクセスしようとして 404 エラーに遭遇する場合、これは権限が不足していることを示しています。この問題を解決するには、次の要件が満たされていることを確認してください。
アカウントレベル:
