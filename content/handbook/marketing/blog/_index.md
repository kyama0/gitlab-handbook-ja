...
[GitLab では誰もが貢献できます](/handbook/company/mission/#mission)。ブログにとって、これは皆さんのブログの提案、アイデア、ドラフトを歓迎することを意味します。ただし、[メインブログ](https://about.gitlab.com/blog/)は GitLab の多くの公式の声の 1 つであり、つまりそこに公開されるコンテンツは、私たちが GitLab（会社と製品の両方）を正確に表現していることを確実にするために、慎重に精査されなければなりません。Blog Managing Editor および Director of Global Content Marketing が [公式 GitLab ブログの directly responsible individuals（DRI）](/handbook/people-group/directly-responsible-individuals/)であり、この責任を担っています。

GitLab Blog についてご質問がある場合は、[sgittlen@gitlab.com](mailto:sgittlen@gitlab.com) までご連絡ください。
...
**ニュースレターコンテンツのリクエストについて:** [この Issue](https://gitlab.com/gitlab-com/marketing/brand-product-marketing/content-strategy-and-ops/content-ops/-/issues/902) にリクエストを追加してください。

### 外部からの貢献

**注:** GitLab は、依頼していないブログの投稿を受け付けません。GitLab パートナーで、検討のためにブログ投稿のアイデアをピッチしたい場合は、[sgittlen@gitlab.com にメール](mailto:sgittlen@gitlab.com)してください。GitLab コミュニティメンバーで、検討のためにブログ投稿のアイデアをピッチしたい場合は、[contributors@gitlab.com にメール](mailto:contributors@gitlab.com)してください。ピッチとともにブログのドラフトは送らないでください。
...
外部からの貢献は、[External Blog Submissions Terms](https://about.gitlab.com/externalblogsubmissionsterms/) の対象となります。GitLab に資料を提出する前に、このドキュメントを読み、条件に同意してください。

## アイデアが承認された後にブログのドラフトを提出する方法

**ブログのピッチが承認されたら、** 執筆者は以下のいずれかのオプションを使ってドラフトを提出します。

- GitLab でマージリクエストを通じて提出する。これを行うには、[リポジトリからこの手順に従ってください](/handbook/marketing/digital-experience/decap-cms/#creating-a-blog-post-in-the-aboutgitlabcom-repository)、または [IDE 向けはこちら](/handbook/marketing/digital-experience/decap-cms/#creating-a-blog-post-using-the-web-ide)。注意深く従ってください! ご質問があれば @sgittlen に連絡してください。
...
一部のブログ投稿は、私たちの [Materials Legal Review Process/SAFE プログラム](/handbook/legal/materials-legal-review-process/) に従って、法務によるレビューを受ける必要があります。執筆者は、Google ドキュメントを Blog チームと共有する前に、SAFE ガイドラインを確認し、Legal の承認を得る責任があります。このプロセスには時間がかかる場合があるため、それに応じて目標公開日を計画してください。
...
[SAFE ガイドラインについてハンドブックページを読んで](/handbook/legal/safe-framework/)、[Materials Legal Review Process](/handbook/legal/materials-legal-review-process/) に従うことで、詳しく学んでください。
...
- GitLab の外部の方は、提案する変更の詳細を添えて Sandra Gittlen（`sgittlen@gitlab.com`）にメールするか、または公開ページの一番下に移動して「Edit this page」をクリックして必要な変更を含む MR を作成してください。これにより MR が開始されます。
...
- あなたのブログ投稿はバイアスがかかっていますか? 私たちには皆、無意識のバイアスがあります。[DIB ハンドブックページの無意識のバイアスのリスト](/handbook/company/culture/inclusion/unconscious-bias)を確認し、自分のバイアスを認識し管理するための戦略を実践しましょう。
- まだ質問がある場合は、ためらわずに編集チームリードの @sgittlen、ブログ Issue 内の該当する [team member resource group（TMRG）](/handbook/company/culture/inclusion/tmrg-tmag/)のリードをタグ付けするか、質問を添えて diversityinclusionandbelonging@gitlab.com にメールしてください。

### ブログのカテゴリーとタグ

#### カテゴリー

投稿ごとに以下のカテゴリーの **1 つだけ** を使用してください。
大文字小文字、スペル、その他何も **変更しないでください**。
そうしないと別のカテゴリーが作成されてしまい、これは誤って行いたくないことです。
...
[mermaid チャートを MR に埋め込む方法](https://gitlab.com/gitlab-com/marketing/digital-experience/gitlab-blog/-/merge_requests/211#usage-notes)。チャートが正しくレンダリングされない原因となるニュアンスがあるので、ぜひ読んでください。
...
- Preview でサイズ変更するには、`Tools`、`Adjust size` に移動し、`Resolution` フィールドの値を調整します。Preview は、`OK` をクリックして確定する前に、結果として得られる画像サイズを推定します。
- すべての画像の幅を同じに保ってください。

#### スクリーンショット

技術的/チュートリアルの投稿では、例を [コードブロック](#adding-code-blocks)またはスクリーンショットで図示してください。例には一貫性を持たせてください。例えば、手順を例示するために汎用 URL `domain.com` を使う場合は、一貫して投稿全体で `domain.com` のままにしてください。
...
アニメーション GIF は、この種のリソースがあれば理解しやすくなるような、短い動的プロセスを図示するのに非常に役立ちます。
アニメーション GIF を作成する方法はいくつかありますが、その 1 つが Mac 向けの軽量アプリ [Giphy Capture] を使うことです。
...
- `Name` フィールド
    これは必須フィールドです。また、一意のフィールドでもあります。

    1. 執筆者の名前は、合成名を含め、姓名のみの組み合わせにすべきです。このフィールドに職務の説明（これには `Role` フィールドを使ってください）や英数字の組み合わせを追加しないでください。
    1. 共著者なし（例: `/authors/<author1>-<author2>`）。複数の単独の執筆者をブログ投稿に追加できます。個々の執筆者を作成し、それから一対多の関係として他のコンテンツタイプにそれら全員を追加すべきです。
    1. 上記と同様、単一のブログ投稿に複数の執筆者なし（例: `/authors/<author1>-<author2>-<author3>-and-<author4>`）。
...
| タグ | 定義 | 使い方 |
|----|----|----|
| translation-in-progress | ブログ投稿が現在翻訳・レビュー中であることを示す | 翻訳リクエストが開かれたときに適用する。投稿が公開されたら削除する |
| language_de-DE | ドイツ語のエントリーをマークする | ドイツ語に翻訳されたブログ投稿に適用する |
| language_fr-FR | フランス語のエントリーをマークする | フランス語に翻訳されたブログ投稿に適用する |
| language_ja-JP | 日本語のエントリーをマークする | 日本語に翻訳されたブログ投稿に適用する |
