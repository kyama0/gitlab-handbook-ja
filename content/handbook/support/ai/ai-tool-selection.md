...
| [duo-helper-image](#duo-helper-image) | ガバナンス管理された `glab` + Duo 環境、セマンティックチケット検索 | ✅ 安全 | GitLab MCP、Glean、Semantic Search | ✅ 承認済み (RED) |
| [Request for Help (RFH) Agent](#request-for-help-rfh-agent---search-and-create-issues) | RFH Issue の検索と作成 | ✅ 安全 | RFH プロジェクト | ✅ 承認済み (RED) |
| [internal-soslab](#internal-soslab) | SOS バンドルの調査、ログ／設定の分析 | ✅ 安全 | Glean、27 以上の MCP ツール | ✅ 承認済み (RED) |
...
通常、[Duo CLI](https://docs.gitlab.com/cli/duo/cli/) または `[OpenCode](https://opencode.ai/)` を [Model Context Protocol (MCP)](https://docs.gitlab.com/user/gitlab_duo/model_context_protocol/) を介して GitLab やその他のシステムに接続しており、チケット業務、調査、ワークフロー自動化向けに調整されています。

### duo-helper-image

**承認状況:** 承認済み (Red)

**参考:** [duo-helper-image](https://gitlab.com/gitlab-com/support/toolbox/duo-helper-image)

`glab` + Duo に加え、Glean、GitLab MCP、Semantic Search、サニタイズ済みの過去の Zendesk チケットを対象とする Ticket Search をパッケージ化した、ガバナンス管理されたコンテナ化サポートツールキットです。

**よくある用途:**

1. すぐに使える、ガバナンス管理された環境で `glab` と Duo を実行する。
1. セマンティック検索とチケット検索でサニタイズ済みの過去チケットを検索する。

### Request for Help (RFH) Agent - Issue の検索と作成

**承認状況:** 承認済み (Red)

**参考:** [Request for Help](https://gitlab.com/gitlab-com/request-for-help/)

RFH プロジェクトに存在する、Issue の検索と作成に使う AI エージェントです。Duo カスタムエージェントとして、Red データの承認に基づいて動作します。

**よくある用途:**

1. 既存の RFH Issue を見つける。
1. RFH プロジェクトから直接、新しい RFH Issue を作成する。

### internal-soslab

**承認状況:** 承認済み (Red)

**参考:** [internal-soslab](https://gitlab.com/gitlab-com/support/toolbox/internal-soslab)

SOS バンドル、ログ、設定、スクリーンショット、チケット会話のためのエージェント型調査ハーネスです。Glean へのアクセスと、目的に特化した 27 以上の MCP ツールを提供します。

> [!note]
> ハーネス自体は Red データをローカルで扱いますが、その Glean 統合は **Orange データのみに** 制限されています。

**よくある用途:**

1. SOS バンドルを調査し、ログと設定を関連付ける。
1. 詳細なトラブルシューティング中にスクリーンショットとチケット会話をレビューする。
