...
translator: claude
stale: false
lastmod: "2026-05-01T16:20:30+01:00"
---
...
DAP で使用する大規模言語モデル (LLM) を選択できます。選択はレスポンスの品質、速度、推論の深さに影響します。

| モデル | 速度 | 推論の深さ | 最適な用途 |
|-------|-------|-----------------|----------|
| Claude Haiku 4.5 | 最速 | 基本 | 素早い検索、シンプルな要約、テキストの再フォーマット |
| Claude Sonnet 4.6 | 高速 | 強力 | ほとんどの日常的なサポート業務: 診断、ドラフト作成、リサーチ |
| Claude Opus 4.6 | 低速 | 最強 | 詳細調査、あいまいな問題、長い多段階推論 |

**推奨アプローチ:**

1. ほとんどのチケットで **Sonnet 4.6** から始めてください。診断、解決策のリサーチ、ドラフト作成を上手く扱います。
1. レスポンスに深さが欠けるか重要な詳細が抜けている場合、同じプロンプトで **Opus 4.6** を再試行してください。
1. 高ボリュームで複雑性の低いタスク (チケットの一括要約や箇条書きから散文への変換など) で速度が重要な場合は **Haiku 4.5** を使用してください。
1. コード、Issue、MR、ドキュメントを相互参照する詳細調査では、**Opus 4.6** が最も徹底した結果を生み出します。

選択したモデルは、ツールがアクセスできるデータには影響しません。DAP 内のすべてのモデルは同じ Red データ承認の下で動作します。
