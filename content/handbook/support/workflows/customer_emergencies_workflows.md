...
1. **サポートマネージャー**: 状況を評価します。状況が必要とする場合、[バックアッププールへのページを開始](/handbook/support/workflows/support-leader-on-the-hook) して、バックアップエンジニアからの支援を要請することが可能です。
...
サポートマネージャーオンコールは常にあなたを支援するために対応可能です。[これらのステップ](/handbook/support/on-call/#engaging-the-on-call-manager) に従って [Support Leader on the Hook (SLOTH)](/handbook/support/workflows/support-leader-on-the-hook) を支援に関与させてください。
...
平日のライセンス緊急対応では、[`#support_licensing-subscription`](https://gitlab.slack.com/archives/C018C623KBJ) に連絡を取り、そこの専門家にケースを処理するよう依頼してください。確実に取り上げられるよう、リクエストで現在の [Support Leader on the Hook (SLOTH)](/handbook/support/workflows/support-leader-on-the-hook) に ping してください。ping されたら、サポートマネージャーオンコールが緊急対応が処理されることを保証する DRI となります。
