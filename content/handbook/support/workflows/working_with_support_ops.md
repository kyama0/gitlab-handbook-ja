...
- **Support Team YAML Files** - サポートチームの設定ファイルへの変更は、直接マージリクエストで行えます。Customer Support Operations はマージするための承認を行う必要があります。参照: [Support Team YAML Files](/handbook/eta/css/gitlab/support-team-yaml-files)
...
[Customer Support Operations との協働](/handbook/eta/css/#working-with-us)について詳しく見る。

[Customer Support Operations が進捗を表すために使用するステージ](/handbook/eta/css/gitlab/issues/working-issues)について詳しく見る。
...
3. 影響を受けるサービスとして "Customer Support Operations" を指定して [新規 PagerDuty インシデントを作成](https://gitlab.pagerduty.com/incidents/create)します（[Support Operations On-Call ワークフロー](/handbook/eta/css/pagerduty/oncall) を参照）
