...
このワークフローの一環として、休暇に入る Support エンジニアは、現在 Open、Pending、On-Hold のチケットにマクロを使ってメモを残します。このマクロはチケットのサマリーを提供し、Support エンジニアをチケットの CC リストに追加し、チケットに `ooo_summary` サマリータグを追加します。すべての高優先度チケット、または 3日以上の PTO を取る場合に、このワークフローに従うことを推奨します。Pending チケットは手動で割り当てを解除する必要はありません。マクロが `ooo_summary` タグを追加するため、顧客が応答した際に自動的に割り当てが解除されます。
