...
1. email アドレスを使って Customers Portal を検索します。
...
自動チェックでユーザーアカウントが見つからない場合、システムが生成する email メッセージでユーザーに通知されます。
...
- ユーザー名（存在する必要があります）
- ユーザー名と email が同一アカウントで一致する必要があります
- アカウントが Enterprise Account でないこと

注意: フォームでは、email アドレスを以下の優先順位で尋ねています。

1) アカウント上のプライマリまたはセカンダリの email アドレス（権限を有する代理人がアカウントオーナーの email にアクセスできる場合に使用）
2) Account Success の email アドレス（権限を有する代理人が Designated Account Successor として追加された場合に使用）
3) 権限を有する代理人の email（権限を有する代理人が、Designated Account Successor として追加された際にどの email が含まれていたか分からない場合、またはアカウントに紐づく email アドレスが分からない場合に使用）。
...
自動チェックでユーザーアカウントが見つからない場合、すべてのシステムタスクが完了した後、システムが生成する email メッセージで依頼者に通知されます。
...
データ主体は、自身の GitLab forum アカウントの削除を追加で要求できます。私たちは GitLab forum からのユーザー削除のみが可能であり、Discourse プラットフォームに対する制御権はありません。このリクエスト種別は現時点では Privacy Center 経由では利用できませんが、サポートチケットを通じてリクエストが来ることがあります。forum ユーザーを削除するには:

1. ユーザーに、ユーザープロフィールのリンク、forum のユーザー名、email アドレスを尋ねます。送信元の email アドレスはユーザー名のものと一致している必要があり、ユーザーは少なくとも一度は返信している必要があります。
1. Issue 上の内部コメントで [forum admins](https://forum.gitlab.com/about) の 1 人をタグ付けし、そのユーザーと関連する投稿を削除するよう依頼します。
1. ユーザーが削除された旨を返信します。
