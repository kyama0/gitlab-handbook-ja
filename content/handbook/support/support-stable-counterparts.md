...
translator: codex
stale: false
lastmod: "2026-09-01T12:00:09-04:00"
---
...
- タイムゾーンが許可する場合、プロダクトマネージャーと自己紹介のコーヒーチャットを行う。
  - グループへの興味と、なぜ SSC になることを選んだかを話す。チームと製品チームの日常について尋ねる。
  - SSC への期待について製品チームに尋ねる。それらを管理し整合させる。
- 重ならないタイムゾーンにいる場合は、Slack で握手をする。
- 製品チームとのコミュニケーションのための定期的なケイデンスを確立する。
  - 特に定期的な非同期コミュニケーションモデルを維持する場合、生きている Google Docs を使うと役立ちます。それには [このテンプレート](https://docs.google.com/document/d/1m9t-sxPzwie2D40cXTwpjnQ2VxYJb3GoEB5ig38Nkmw/edit?usp=sharing) を活用できます。
- 最初の自己紹介で、
  - 製品チームのチームシンクに自分を追加してもらう - シンクコールに参加できない場合はアジェンダドキュメントを読むことができます！
  - 製品チームの Slack チャンネルに参加する。
...
- 製品グループの優先事項と課題を認識する。
- 製品チームがカバーする機能の使用における主題専門家になることを目指す。
...
- チケットからの顧客フィードバックを Product チームと共有する。
  - 関連する Issue、チケット、Slack スレッドに製品チームをループする。
...
- SSC でもあるサポートエンジニアを管理している場合は、月に少なくとも 1 回はメンバーとの 1:1 で会話のトピックにすべきです。
  - このアクティビティの優先順位付けと時間の確保ができるようにする。
  - 懸念事項があれば解決に取り組む。
...
- SSC が長期間不在になる場合や、グループに関連する重要なイベント中の場合は、製品チームのバックフィルとして機能してください。
...
- 各 SSC-PM の関係は異なります。あなたと製品チームに最適な方法を見つけることに取り組んでください！
...
> SSC ロールは初めてですか？まず [SSC Onboarding テンプレート](https://gitlab.com/gitlab-com/support/support-training/-/blob/main/.gitlab/issue_templates/Support%20Stable%20Counterparts%20Basics.md) から始めてください。

**SSC Helper Agent** は、GitLab Support Stable Counterparts 向けの [Glean](https://app.glean.com) エージェントです。最近の製品変更、次の GitLab メジャーリリースに向けた進捗、サポート準備作業、関連する Slack の議論を簡潔に把握できます。SSC が複数の情報源を手動で確認する必要はありません。

このエージェントは、[SSC Helper Agent プロジェクトリクエスト](https://gitlab.com/gitlab-com/support/support-project-portfolio/-/work_items/24)を受けて作成されました。このリクエストでは、GitLab の Issue、マージリクエスト、Slack の変更について、SSC が別の視点からも確認できるようにする必要性が明らかになりました。

### アクセス

[Glean で SSC Helper Agent を開きます](https://app.glean.com/chat/agents/7458777e185a47568377228e6b6ab117)。

### 想定ユーザー

主なユーザーは、担当する製品領域の変更を常に把握する必要がある GitLab Support Stable Counterparts です。

### エージェントの機能

エージェントは実行ごとに、次の処理を行います。

- 選択したプロジェクトとラベルについて、最近の Issue とマージリクエストを確認します。
- 次の GitLab メジャーリリースに関連する進捗と未完了の作業を特定します。
- 関連する Slack チャンネルから、計画、ロールアウト、リスク、ブロッカー、インシデント、サポート準備に関する議論を検索します。
- スコープと影響に基づいて作業を分類します。
- Support のレビューが必要になる可能性のある項目を強調します。
- 関連する GitLab の Issue、マージリクエスト、Slack スレッドへのリンクを含む、簡潔で Slack 向けのステータス更新を作成します。
- Glean エージェントから Slack の DM として結果を送信します。
- SSC は、レポートを実行して Slack に送信するタイミングを選択できます。

### 入力

| 入力 | 説明 | デフォルト |
|---|---|---|
| `projects` | カンマ区切りの GitLab プロジェクトパス | `gitlab-org/gitlab` |
| `labels` | 製品領域またはトピックを定義するカンマ区切りのラベル | エージェントで設定されたラベル |
| `days_back` | 確認する最近のアクティビティの日数 | `7` |
| `slack_channels` | 任意のカンマ区切りの Slack チャンネル名 | 可能な場合はワークフローによって導出 |
| `next_major_version` | 月次リリーススケジュールのメジャーバージョン | ワークフローによって導出 |

### 期待される出力

エージェントは、コンテンツが存在する場合に次のセクションで構成された、Slack 向けのステータス更新を作成します。

- **今週のハイライト** — 注目すべき最近の変更
- **次のメジャーリリースに向けて** — 今後のメジャーバージョンに関する未完了の作業と進捗
- **Support の準備状況** — 顧客への回答やトラブルシューティングに影響する可能性がある項目
- **Slack から** — 設定されたチャンネルでの関連する議論

各セクションには、情報源となる GitLab の Issue、マージリクエスト、Slack スレッドへのリンクが含まれます。出力に基づいて行動する前に、リンク先の情報源を開いて確認してください。

### 制限事項

- Glean 検索、GitLab ドキュメント、ハンドブック、および該当する場合は Slack を使用して、GitLab のコンテキスト内で動作します。
- 名前、メールアドレス、本人確認情報を要求したり使用したりしません。
- Issue、MR、ユーザー名、ラベル、日付、チャンネル、URL を捏造してはいけません。不確かなフィールドは空白のままにします。
- 特定されたチャンネルが空の場合は Slack のアクティビティを報告できません。自動検出されたチャンネルは、明示的に文書化されているか、ユーザーが指定したものでなければなりません。
- 出力は Support が状況を把握するための概要であり、リンク先の情報源に代わるものではありません。
- Issue や MR を自動的に変更しません。

### 推奨される用途

SSC Helper Agent は次の用途に使用します。

- 製品領域に関する定期的な更新。
- SSC 同期や製品に関する議論の準備。
- Support の同僚に最新情報を伝える前に、最近の変更を確認する。
- 顧客への回答やトラブルシューティングへの影響を特定する。
- リリースのリスクと未解決の疑問を明らかにする。
- ドキュメント、ランブック、サポート準備のギャップを特定する。

### エージェントを使用した新しい SSC のオンボーディング

1. 担当プロジェクトのパス（通常は `gitlab-org/gitlab`）、製品領域のラベル、正確に把握している Slack チャンネル名、確認対象期間（通常は `7` 日）を収集します。不明な場合は、SSC、マネージャー、Product、Engineering の連絡先に確認してください。チャンネル名を推測してはいけません。
1. [SSC Helper Agent を開きます](https://app.glean.com/chat/agents/7458777e185a47568377228e6b6ab117)。
1. フォームに用意された選択肢を使用して、プロジェクト、ラベル、確認対象期間、正確なチャンネル名をエージェントフォームに入力します。ローカルへのインストールやカスタムプロンプトは不要です。
1. 通常、ワークフローによって次のメジャーバージョンが自動的に導出されます。
1. 出力されたセクションを確認し、リンク先の情報源を開いて検証します。
1. 出力を Product、Engineering、Technical Writing とのフォローアップ、SSC 同期、Support の更新に使用します。
1. 出力が空の場合は、プロジェクトパスとラベルを検証し、`days_back` を増やし、正確なチャンネル名を指定して、情報源となる項目を直接調べます。

### メンテナンス

SSC のワークフローまたはレポート、リリースプロセス、製品階層またはチャンネルマッピング、利用可能な検索動作、エージェントのフィールド、デフォルト、出力形式、分類ロジックのいずれかが変更された場合は、このドキュメントをレビューします。エージェントを更新するときは、変更内容を説明する短い注記とともに、このドキュメントも同じ変更で更新してください。
