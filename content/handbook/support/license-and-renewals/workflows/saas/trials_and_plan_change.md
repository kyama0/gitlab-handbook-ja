...
1. 既存の GitLab.com サブスクリプションとネームスペースを持たない新規のお客様は、次の [フォーム](https://gitlab.com/-/trial_registrations/new?glm_source=about.gitlab.com/&glm_content=default-saas-trial) に記入することで、30 日間の GitLab Ultimate サブスクリプションを申請できます。
1. 既存のネームスペースを持っているがアクティブな GitLab.com サブスクリプションを持っていない GitLab.com ユーザーは、グループネームスペースの billing セクションに移動し `Start Trial` ボタンを押すことができます。そのネームスペースで以前トライアルが存在した場合、このボタンは表示されません。その場合、ユーザーは GitLab セールスに連絡して新しいトライアルまたはトライアル延長を要求する必要があります。
...
1. [GitLab の公式トライアルページ](https://about.gitlab.com/free-trial/?hosted=sass) から Ultimate トライアルをリクエストします。これには、トライアルを適用するために新しいネームスペースをセットアップする必要があります。セールスやサポートのアクションは不要です。
...
#### 以前に Ultimate トライアルを使用したネームスペース

これらのネームスペースは、グループが複数のトライアルを取得できないようにする通常のガードレールを回避して、Premium サブスクリプションの上で Ultimate トライアルを開始することが許可されています。
...
1. リクエストを確認し、リクエストを実行するのに十分な情報が提供されていることを確認します。これを行うには、次のことを確認します:
   1. `Namespace:` フィールドに有効な GitLab ネームスペースが含まれており、それがトライアルプラン（アクティブまたは期限切れ）を保持していること。これは Salesforce のリンクやメールアドレスであってはなりません。
...
1. ネームスペースを手動で調整する必要がある場合は、詳細と `~Console Escalation::Customers` ラベルを付けた新しい内部 Issue を作成します。
...
**注:** お客様がネームスペースでトライアルを開始していない場合、トライアルを延長することはできません。お客様がトライアルを開始していない場合は開始する必要があります。または期限切れのものを使用する必要があります。
...
有料の非トライアルネームスペースでのプラン変更は、サブスクリプション購入を通じて行うべきです。
