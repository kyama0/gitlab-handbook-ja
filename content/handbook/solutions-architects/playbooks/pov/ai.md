...
5. `gitlab-org/fulfillment/meta` の `trial_credit_requests` テンプレートから GitLab の Issue を作成し、タイトルを `[A-XXXXXX] [Subscription Name], [Customer Name], [Total Credits Requested], [Date Requested For]` とします。次のフィールドを含めます。サブスクリプション名 / ID、顧客名、アカウントチーム（AE \+ SA）、Salesforce のアカウントリンク、Salesforce の DAP 商談リンク、販売先担当者のメールアドレス、サブスクリプションの種類、合計シート数、申請クレジット総数（6.4 のユーザーベースのモデルに基づいて範囲を設定）、オフラインフラグ、評価文書へのリンク、および再トライアルまたは延長の場合は、前回のトライアル Issue のリンク、理由コード、残りのクレジット、妥当性を検証した SA の氏名。
...
### 1.4 DAP 申請内容の基準 {#14-dap-request-content-standard}

申請は次の質問に答える必要があります。

* 顧客は誰で、どのサブスクリプションにクレジットを付与しますか？
* トライアルはどの商談と商業上の意思決定を支援しますか？
* 上位 3 〜 5 つのユースケースは何ですか？
* 測定可能な成功基準は何ですか？
* 誰が評価し、誰がトライアルの責任を持ち、誰が意思決定を承認しますか？
* ユーザー、シート、プロジェクト、クレジットはそれぞれいくつ必要で、その理由は何ですか？
* 開始日 / 終了日はいつで、その期間で十分なのはなぜですか？
* 対象となるデプロイモデル、GitLab のバージョン、Runner のモデル、AI Gateway / モデルの経路、インテグレーションは何ですか？
* 何が評価を妨げる可能性があり、すでにどの対策を実施しましたか？
* 再トライアルまたは延長ですか？その場合、評価を妨げた正当な阻害要因は何で、何が変わりましたか？

### 1.5 SA manager による評価と承認のガイドライン {#15-sa-manager-evaluation-and-approval-guidelines}

2026 年 8 月 28 日より、DAP トライアルクレジットの承認と付与は、Deal Desk ではなく SA Leadership が担当します。初回申請を承認する前に、SA Manager は次を確認します。
...
| 必須の通過条件 | 承認前に満たすべき状態 | エビデンス / 記録 |
| :---- | :---- | :---- |
| POV オブジェクト | Salesforce に POV オブジェクトを作成し、商談、評価計画、コラボレーションの場所に関連付けている。 | Salesforce の POV オブジェクトへのリンク |
| 商談の種類 | 更新商談ではない。 | Salesforce の商談の種類 |
| 契約に至る道筋 | 購買担当者 / 承認者、推進者、意思決定日、商業上の次のステップを含め、商談と意思決定に至る道筋に信頼性がある。 | 商談メモと評価計画 |
| 顧客固有の評価文書 | 評価文書はテンプレートをコピーしただけのものではなく、顧客固有の目標、ユースケース、ステークホルダー、リスク、スコープ、スケジュール、エビデンスを含む。 | リンクされた評価文書 |
| エビデンスを収集できるユースケース | ユースケースが、責任者、ワークフローの手順、前提条件、成功の兆候を備え、エビデンスを生み出せるほど具体的である。 | ユースケースのプレイブックまたは計画 |
| 参加者とユーザー数 | 顧客が人員、時間、アクセス、技術責任者を確保すると確約している。トライアルに参加するユーザー数を明記している。 | 参加者一覧、ユーザー数、責任者、対応可能な時間 |
| クレジットと活動モデル | 申請クレジット数が、トライアルのユーザー数、ユースケース、期間、想定される活動に結び付いている。 | クレジットの計算と申請 |
| 技術的な準備 | SA が技術的な準備状況を検証済み、または評価期間開始前に完了する日付入りの計画がある。スモークテストと設定作業を明示的に計画している。 | 準備状況チェックリスト、スモークテストの結果または日付入りの計画 |
| 双方のスケジュール | アカウントチームがキックオフ、定期的な進捗確認、中間確認、最終報告を予定に設定している。 | カレンダー招待と評価計画 |
| 顧客の認識 | アカウントチームに、目標、ユースケース、役割、スケジュール、前提条件、成功基準、クレジット、責任を顧客に提示するスライドがあり、顧客が計画を認識して合意している。 | 顧客向け POV 計画スライドと了承の記録 |
| 測定と記録 | 使用量、利用定着、フィードバック、技術的な結果の記録方法についてチームが合意している。 | 測定計画、ダッシュボード、アンケート、正式な記録システムのフィールド |
...
| 規則 | 運用ガイダンス |
| :---- | :---- |
| SA manager の承認 | SA Managers は、SA manager の承認チェックリストが完了している場合、**1,000 クレジット以下**の DAP トライアル申請を承認できます。 |
| ユーザー数とクレジットの上限 | トライアルごとに最大 50 ユーザーとし、評価 1 回あたりの初回上限を 2,000 クレジットに設定します。延長は引き続き SA Leadership を通します。合計の上限（初回 \+ 延長）は 5,000 クレジットです。 |
| Deal Desk 承認後の付与 | Deal Desk が 1,000 クレジットを超える数量を承認した場合、SA Manager は、承認された上限までの範囲で、Deal Desk が承認した総数を下回る数量を付与できます。承認された数量を超えて付与しないでください。 |
| 標準の規模算定式 | **トライアルのユーザー数 × ユーザーあたり 40 クレジット**でトライアルの規模を算定します。ユーザー数、期間、ユースケース、算定式に対する例外を申請に記録します。 |
| 使用量の管理 | 評価クレジットは、定義した評価期間と顧客範囲に対してのみ使用します。顧客が商業上の負担を明示的に了承しない限り、オンデマンド課金は無効のままにします。 |
| 検証 | キックオフ前にサブスクリプションの利用資格、クレジット、残高、有効期限を確認します。評価グループ、可能な場合はユーザー、機能ごとに使用量を毎週監視します。 |
| 監査と終了処理 | 承認 Issue のリンクとクレジット付与の結果を POV レコードに記録します。毎回のトライアル後、再トライアルを申請する前に、トライアル後アンケートと DAP のフィードバックを完了します。 |
| 管理者による変更 | 正式に許可されたエスカレーションを通す場合を除き、承認済みの CustomersDot ワークフロー外でクレジットを手動変更しないでください。 |
