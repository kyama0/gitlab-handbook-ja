...
lastmod: "2026-09-22T15:37:30-05:00"
...
- コメント
  - [x] メッセージ作成欄をデフォルトでパブリックチャネルに設定する
  - [x] メール経由のエージェントコメントをデフォルトでパブリックにする
  - [ ] チケットの最初のコメントを非公開にできるようにする
  - [ ] デフォルトで下書きモードにする
  - [ ] CC されたエンドユーザーからのメールコメントをパブリックにできるようにする（非推奨）
  - [x] スペルチェックと文法チェックを有効にする
  - [x] 絵文字のテキスト置換を許可する
  - [x] エージェントによるテキストの色の変更を許可する
  - [x] エージェントが会話を翻訳できるようにする
    - AWS Translate で翻訳する
      - [x] すべてのチャネルで AI 翻訳を有効にする
      - [x] AI 翻訳である旨を表示する
  - 文字起こしをチケットに追加する方法を選択する: `As a public reply`
  - URI をハイパーリンクとしてレンダリングする: `["[]"]`
  - [x] エージェントが音声メッセージを送信できるようにする
- 添付ファイル
...
#### ポータルの問題 {#portal-issues}

これらはサポートポータル内の問題に関する報告です。それぞれの問題には固有の課題がある場合がありますが、ユーザー向けの一般的なトラブルシューティングガイドは次にあります。
...
サポートポータルアカウントに予備のメールアドレスを追加したいという問い合わせを顧客から受けることがあります。予備メールアドレスは、送信されたチケットを特定のアカウントに紐付けるために使用しますが、送信者として使用される（したがって通知を受け取る）のはプライマリメールアドレスのみです。
