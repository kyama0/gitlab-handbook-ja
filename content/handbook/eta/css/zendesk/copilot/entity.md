---
title: 'エンティティ'
description: 'Zendesk Copilot エンティティに関するドキュメント'
upstream_path: /handbook/eta/css/zendesk/copilot/entity/
upstream_sha: aa050cd4c77778c3f3ebc5d01bf39c92d7da06be
lastmod: "2026-07-27T13:51:40-05:00"
translated_at: "2026-07-28T06:31:03+09:00"
translator: codex
stale: false
---

## エンティティとは

[Zendesk](https://support.zendesk.com/hc/en-us/articles/6711181959194-Classifying-unique-information-in-tickets-with-entities) によると:

> インテリジェントトリアージの一部であるエンティティ分類では、受信チケットおよびメッセージングの会話から、製品名、注文番号、所在地の詳細など、特定の情報を自動的に取得し、対応可能な詳細でチケットを充実させます。

## 現在の設定

- Zendesk Global
  - サンドボックス: 現在使用していません
  - 本番環境: 現在使用していません
- Zendesk US Government
  - サンドボックス: 現在使用していません
  - 本番環境: 現在使用していません
