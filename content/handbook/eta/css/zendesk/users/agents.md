...
私たちは、エージェント同期（Customer Support Systems および Support チーム向け）と[プロビジョニング/デプロビジョニング](/handbook/eta/css/zendesk/users/provisioning)プロセスを通じて、エージェントを大部分管理しています。
...
エージェント同期は、Zendesk における Customer Support および Customer Support Systems チームメンバーのメタデータを管理します。その他のエージェントアカウント（統合、ボットなど）は手動で管理されます。
...
- GitLab.com ユーザー ID（カスタム[ユーザーフィールド](/handbook/eta/css/zendesk/users/fields)）
- GitLab.com ユーザー名（カスタム[ユーザーフィールド](/handbook/eta/css/zendesk/users/fields)）
- Zendesk グループメンバーシップ
- Manager タグ（カスタム[ユーザーフィールド](/handbook/eta/css/zendesk/users/fields)）
- 名前
- Out of Office ステータス（カスタム[ユーザーフィールド](/handbook/eta/css/zendesk/users/fields)）
  - US Government のみ
- 署名
- 選択されたユーザータグ（詳細は[動作の仕組み](#how-does-it-work)を参照）
- ユーザーリージョン（Zendesk Global のみのカスタム[ユーザーフィールド](/handbook/eta/css/zendesk/users/fields)）
...
- [プロビジョニング/デプロビジョニング](/handbook/eta/css/zendesk/users/provisioning)で指定された場合を除き、これは決して行わないでください。
