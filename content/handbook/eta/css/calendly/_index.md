...
upstream_path: /handbook/eta/css/calendly/
...
Calendly Events to gCal Events に変更を加えるには、プロジェクトリポジトリでマージリクエストを作成する必要があります。実際の変更内容は、リクエスト自体によって異なります。

ピアレビューと承認の後、マージリクエストをマージできます (これにより、次回のスケジュール実行時に変更が適用されます)。
