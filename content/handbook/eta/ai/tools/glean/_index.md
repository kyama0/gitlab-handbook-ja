...
- **機能全体への展開。** 特定の同僚に直接ではなく、機能全体でエージェントを共有することは、その機能の ATO および Enterprise AI と連携して行います。
...
はい、すべての GitLab チームメンバーが独自に構築できます。2 つの構築モード（自然言語とステップバイステップのアドバンスト）、出発点としてのテンプレート、引き続き Enterprise AI を経由する部分（検証、機能全体への展開、カスタムアクションおよびコネクターの変更、公開モード）については上の [Glean エージェントを構築する](#build-a-glean-agent) を参照してください。
