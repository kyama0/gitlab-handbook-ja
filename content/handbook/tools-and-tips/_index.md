...
ハンドブックのアーキテクチャ、構造、およびデスクトップでローカルに編集する方法については、[Handbook Development セクション](https://handbook.gitlab.com/docs/development/) を参照してください。ブラウザでハンドブックを編集する方法については、[the editing handbook page](/handbook/about/editing-handbook/) を参照してください。
...
セキュリティのない Wi-Fi を使用する場合は、個人用 VPN を検討してください。
私たちには [企業 VPN はありません](/handbook/security/product-security/security-platforms-architecture/security-architecture/zero-trust/#why-we-dont-have-a-corporate-vpn) が、GitLab のために旅行する場合や未知のネットワークを頻繁に使用する場合は、[個人用 VPN を購入](https://internal.gitlab.com/handbook/finance/expenses/#vpn-subscription) することを検討してください。
...
YubiKey にはいくつかの [セキュリティ Issue](https://www.yubico.com/support/security-advisories/) があり、通常は迅速に解決されます。彼らはセキュリティ勧告のための専用ページを持っています。
...
[Google Calendar](/handbook/tools-and-tips/#google-calendar) の招待は Zoom リンクの真実の唯一のソースです。Google Document アジェンダで Zoom リンクをリンクすることを避けてください。これは、すぐに最新ではなくなる可能性があります。会議の開始頃に Zoom リンクが変更された場合、一時的にそこに置いておくことは大丈夫です。
...
1. Chrome で Google ドキュメントを素早く作成する: Chrome のアドレスバーに「docs.new」を入力します。
同様に、Google Sheet には「sheets.new」など…
1. 多くの他の編集者がいる文書内では、文書の上部にあるユーザーの画像またはアイコンをクリックして、彼らのカーソルと文書に何を入力しているかにフォーカスを移動できます。
...
1. Google Document の番号付きリストに追加の行を追加するには、`Return` を押して文書内の既存の番号付きリストの下に追加スペースを追加し、それらのスペースをハイライトし、フォーマットバーの番号付きリストアイコンをクリック（または Mac で `Command ⌘ + Shift + 7` を押す）してスペースから番号付きリストを作成します。クイックチュートリアルとして [番号付きリストに行を追加する方法](https://www.youtube.com/watch?v=dgyttEJi-ZQ) のビデオをご覧ください。
...
オプション: Google で[プロフィール写真を設定する](https://myaccount.google.com/personal-info) ことで、Google ドキュメントで自分がどこにいるかを写真で表示できます（最初の文字だけ表示されるのではなく）。これにより、会議の参加者が文書内で動き回るときに、他の人がディスカッションをより簡単にフォローできます。
...
このカレンダーで Company Calls、Group Conversations、101s、その他のチームのミーティングの詳細を見つけることができるので、別のチームのミーティングに参加して質問したり、彼らが何に取り組んでいるかを学んだり、GitLab の他の部門とチームについて知ったりできます。
...
##### GitLab Team Meetings カレンダーへのイベントの追加
...
これにより、連続したイベントを持つ参加者がトイレに行ったり、コーヒーを飲んだりする機会を持つことができ、次の機能に遅れることなくなります。
