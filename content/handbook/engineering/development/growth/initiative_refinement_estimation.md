...
translator: codex
stale: false
lastmod: "2026-07-10T13:07:44+05:30"
---
...
- 毎日トリアージボットは `~"workflow::refinement"` 列のすべての Issue をチェックし、Issue に必要最低数の見積もり投票がある場合（現在の設定については[こちら](https://gitlab.com/gitlab-org/quality/triage-ops/-/blob/master/lib/growth_refine_automation_helper.rb)の `MIN_REACTIONS` 定数を参照）、ボットは Issue の重みを最も多く投票された値に設定し、refinement thread に ✅ リアクションを付け、Issue を `~"workflow::scheduling"` に移動します。
...
- エンジニアが `Ready for Development` から Issue を引き取り、ステータスを `In Dev` に変更します。すると、トリアージボットが現在のマイルストーンを Issue に割り当てます。
