...
Provision チームはトライアル作業に関して Growth チームとコラボレーションしています。Growth はトライアルの入口、つまり GitLab 内でトライアルを開始する画面と、CustomersDot の薄いトライアル入口エンドポイントおよび基本的なコントローラーを所有します。一方、Fulfillment/Monetization は、より深いトライアルコードベース（プロビジョニング、エンタイトルメント、課金、ライフサイクル、共有サービス）に加え、CustomersDot のすべての運用サポートとオンコールを所有します。詳細な所有権の境界とコラボレーションガイドラインについては、[トライアルの所有権とコラボレーションフレームワーク](/handbook/engineering/development/growth/trials-ownership/) を参照してください。Growth エンジニアがトライアル作業のために CustomersDot に貢献する必要がある場合は、そこに記載されている意思決定フレームワークに従う必要があります。質問やコラボレーションリクエストについては、[#s_fulfillment](https://gitlab.slack.com/channels/s_fulfillment) または [#s_growth](https://gitlab.slack.com/channels/s_growth) Slack チャンネルに連絡してください。
...
- [トライアルの所有権とコラボレーションフレームワーク](/handbook/engineering/development/growth/trials-ownership/) - Growth と Fulfillment/Monetization にまたがるトライアル作業の概念的な所有権モデルと意思決定フレームワーク
