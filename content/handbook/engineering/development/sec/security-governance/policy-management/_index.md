---
title: Policy Management グループ
description: "GitLab の Policy Management チームは、スキャンを強制し、脆弱性が検出された場合にセキュリティ承認を必須とするソリューションの作成を担っています。"
upstream_path: /handbook/engineering/development/sec/security-governance/policy-management/
...
Security Policies グループのミッションは、セキュリティチームとコンプライアンスチームに組織全体でセキュリティコントロールを適用する手段を提供することです。私たちのチームが開発したツールにより、お客様はセキュリティスキャンを適用・自動化し、リポジトリへの提案された変更についてセキュリティ承認を必須とすることで、セキュリティリスクを防止できます。私たちのビジョンと方向性を包括的に理解するには、https://about.gitlab.com/direction/security_risk_management/security_policies/security_policy_management/ の方向性ページをご覧ください。
...
完了したアイテムは、[エピックの完了定義](/handbook/engineering/development/sec/security-governance/policy-management/planning/#definition-of-done-for-epics)の条件がすべて満たされた後にテーブルから削除されます。エピックはこの時点でクローズされます。
...
機能の取り組み方に関する現在のプロセスは[機能プロセスページ](/handbook/engineering/development/sec/security-governance/policy-management/feature_process/)にあります。
