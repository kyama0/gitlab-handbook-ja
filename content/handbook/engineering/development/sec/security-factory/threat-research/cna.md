---
title: Threat Research - CNA プロセス
upstream_path: /handbook/engineering/development/sec/security-factory/threat-research/cna/
...
1. 提出者が提供した CVSS スコアをレビューします。

   - CVSS スコアが CWE と説明に基づいて期待されるものと大きく乖離している場合は、提出者にスコアが妥当かどうかを確認します。
   - 予期しない CVSS メトリクスに明確な理由がある場合は、その旨を説明に注記します。例：_「… ユーザーが自分のアカウントにのみ影響を与えられるため、全体的な影響は限定的です」_
   - **注意**：CVSS スコアは、CVE の説明と CWE のみにアクセスした場合でも、外部の視点から意味をなすものでなければなりません。

## GPG キー

`cve@gitlab.com` メールには、Threat Research チームが CNA プロセスで使用する [GPG キー](https://about.gitlab.com/security/cve/#cve-public-gpg-key)があります。

## 公開キーの延長

GitLab の `cve@gitlab.com` メールの公開 GPG キーは、6 か月ごとに有効期限が切れるよう設定されています。有効期限をさらに 6 か月延長するには、以下の手順を行ってください。

1. `cve@gitlab.com` の公開キーと秘密キーの両方をローカルに持っていることを確認します。
1. `gpg --list-keys` でキーを一覧表示します。
...
1. `gpg --edit-key AAAABBBBCCCCDDDDEEEEFFFFGGGGHHHHIIIIJJJJ` でキーを編集します。
1. `expire` 対話コマンドでキーの有効期限をさらに `6m` に設定します。
...
- [cves-workflow（内部）](https://gitlab.com/gitlab-org/secure/vulnerability-research/advisories/cves-workflow/-/blob/master/lib/state_machine.rb#L546)
