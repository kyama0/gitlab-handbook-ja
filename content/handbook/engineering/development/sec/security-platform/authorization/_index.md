...
upstream_path: /handbook/engineering/development/sec/security-platform/authorization/
...
Authorization グループは [Security Platform ステージ](/handbook/engineering/development/sec/security-platform/)の一部です。作業は `group::authorization` ラベルで追跡します。
...
| `~workflow::refinement` | エンジニアリングによるさらなる調査が必要。リファインメント後、Issue にはウェイトが割り当てられ、workflow ラベルは `~workflow::scheduling` に更新されるべき。 |
