...
upstream_path: /handbook/engineering/development/sec/oncall/rotation-leader/
...
2. チームメンバーの地域（APAC、EMEA、AMER）と主要ドメインを特定する
...
- 8 時間シフトあたり各エンジニアが何回呼び出されているか？
- 呼び出し通知はチームと地域全体で公正に分配されているか？
- 過剰な呼び出し通知（アラート疲れ）を生成しているドメインはあるか？
- 呼び出し通知が少なすぎる（潜在的なカバレッジギャップ）ドメインはあるか？
...
- [ローテーションへの参加と離脱](/handbook/engineering/development/sec/oncall/joining-and-leaving-rotation) — チームメンバーの追加/削除を管理する
- [カバレッジとスケジューリング](/handbook/engineering/development/sec/oncall/coverage-and-scheduling) — スケジュールを管理および公開する
- [休暇と祝日](/handbook/engineering/development/sec/oncall/time-off-and-holidays) — チームの祝日カバレッジを管理する
- [成功の測定](/handbook/engineering/development/sec/oncall/measuring-success) — ローテーション健全性メトリクスを追跡する
- [コミュニケーションと文化](/handbook/engineering/development/sec/oncall/communication-and-culture) — ローテーションで責任追及のない文化を育む
