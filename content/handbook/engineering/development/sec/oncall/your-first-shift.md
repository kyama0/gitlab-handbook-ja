---
title: 初めてのシフト
upstream_path: /handbook/engineering/development/sec/oncall/your-first-shift/
...
呼び出し通知を受信したときに何をすべきか分からない場合、誰に連絡すればよいですか？
...
呼び出し通知を受信しても慌てないでください。以下の手順で対応します：

### 直ちに行うこと

1. Incident.io で **アラートを承認する**（通常 5 分以内）
2. **アラートの詳細を読む** — どのサービスか？どのメトリクスか？しきい値はいくつか？
3. Slack の **インシデントチャンネルに参加する**（呼び出し通知にリンクが含まれているはずです）
...
5. **継続的に報告する** — 5 〜 10 分ごとに Slack のインシデントチャンネルに進捗を更新する

### 行き詰まった場合

- Slack で助けを求める
- そのドメインに不慣れな場合は、ドメイン固有の Slack チャンネルに呼び出し通知を送る  
- 30 分間沈黙したまま調査を続けないこと — 早めにエスカレートする
...
- [基本事項の理解](/handbook/engineering/development/sec/oncall/understanding-the-basics) — オンコール初心者はここから始める
- [ツールとアクセス](/handbook/engineering/development/sec/oncall/tools-and-access) — シフト前にツールをセットアップする
- [コミュニケーションとカルチャー](/handbook/engineering/development/sec/oncall/communication-and-culture) — 呼び出し通知受信時のコミュニケーション方法を確認する
