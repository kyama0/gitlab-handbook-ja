---
title: "オーナーシップと境界 - AI Clients"
description: このページは、AI Clients のシステムで機能を作成・保守するすべての関係者に明確さをもたらし、期待値を明確にします。
aliases:
  - /handbook/engineering/ai/editor-extensions-vscode/ownership/
  - /handbook/engineering/ai/editor-extensions-multi-platform/ownership/
  - /handbook/engineering/ai/ai-clients/developer-clients/ownership/
upstream_path: /handbook/engineering/ai/ai-clients/ownership/
...
複数のグループが、AI Clients を通じて公開される機能に貢献しています。これによりオーナーシップとメンテナンス責任に関する曖昧さが生じる可能性があります。本ドキュメントは、一貫したオーナーシップの帰属、関係者全員の期待値の整合、効率の改善のために合意されたアプローチを概説します。

システムの一部を所有するということは、影響を受ける部分の機能開発、バグ修正、一般的なメンテナンスに責任を持つことを意味します。理想的には、所有するシステムの一部にメンテナーを配置し、AI Clients ステージからより独立した形にするのが望ましいですが、これは厳密な要件ではありません。
...
* 機能の実装
  ...
  * Language Server オーナーシップモデル
    * Language Server の変更が完了したら、機能チームは **AI-Powered:AI-Clients** ステージに通知し、ステージが Language Server の変更をエディター拡張機能に統合します。
