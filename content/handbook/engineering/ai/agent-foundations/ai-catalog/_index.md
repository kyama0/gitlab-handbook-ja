...
3. デリバリーの[健全性ステータスを監視し伝える](#status-updates)
...
Issue の分解と[重み付け](#weighting)は、マイルストーンが始まる前の計画週に完了する必要があります。分解と重み付けが完了していない Issue やエピックを `~Deliverable` としてコミットすることはできません。
...
### マイルストーン内のエピック {#epics-in-a-milestone}
...
* エピックが主要な `~Deliverable` であり、その子 Issue がすべて小さい（作業期間が 3 日未満）場合は、
  エピック自体に週次アップデートを投稿します。
