...
translator: claude
stale: false
lastmod: "2026-06-18T09:07:10+02:00"
---
...
ステークホルダーに情報を共有し、非同期コラボレーションを支援するため、**現在のマイルストーン** の Deliverable に関するすべてのステータス更新は、以下の標準テンプレートを使って Deliverable の Issue に直接報告する必要があります。

更新を Issue に集約することで、各 Deliverable の進捗に関する信頼できる唯一の情報源を確保できます。Slack スレッドやスタンドアップにステータスが散在するのを避け、誰でも非同期で状況を追いやすくなり、マイルストーン全体を通じて Deliverable がどのように変化したかを追跡可能な履歴として残せます。

#### 更新を投稿するタイミング

作業を開始したとき、リスクを特定したとき、スコープ変更を検討しているとき、またはマイルストーン中の定期チェックイン時など、ステータスに意味のある変化があったときに更新を投稿します。最低でも週に 1 回は更新を投稿してください。

#### テンプレート

Issue のコメントとして更新を投稿する際は、以下のテンプレートを使用してください。
...
- **Current status** — 現在の状況を簡潔に説明します。完了したこと、進行中のこと、デリバリーに影響する可能性のあるブロッカーや依存関係を含めます。
- **Shipping this milestone** — Deliverable が現在のマイルストーン内に出荷できる見込みであれば `yes`、遅れる場合は `no` を示します。回答が `no` の場合は、current status フィールドに短い説明を追加します。
- **Scope reduction opportunities** — マイルストーン期限に間に合わせるためにスコープから外せる部分がある場合は `yes`、現在のスコープが最小限のデリバリーである場合は `no` を示します。`yes` の場合は、何を減らせるかを簡潔に説明します。
- **`/health_status`** — Deliverable が期待どおり進んでいる場合は `on_track`、期限内または完全なスコープで出荷できるか不確実な場合は `at_risk` を適用します。
- **`/cc`** — この更新について通知すべきチームメンバーやステークホルダー（例: PM、EM、デザイナー、下流チーム）をタグ付けします。
