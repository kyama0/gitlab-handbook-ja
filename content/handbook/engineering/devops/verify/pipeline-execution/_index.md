...
translator: codex
stale: false
lastmod: 2026-06-26T12:28:14-04:00
...
Pipeline Execution には、明示的な SLO に結びついた迅速なトリアージと解決を必要とする、計画外でリアクティブな作業の特別なワークストリームがいくつかあります:
...
- Pipeline Execution Flaky Tests: これについてはまだ明文化されたプロセスがありませんが、今後注意深く監視し、積極的に管理する必要がある、新たな高優先度のワークストリームです。

これらの Issue は、トリアージされた重大度に基づいていつでも作業予定に入れることができます。S1 Issue は直接開発に取り込まれる場合があり、私たちが行わない変更を求める Issue は、バックログに入れられるか、そのままクローズされることもあります。トリアージ結果にかかわらず、これらの Issue は迅速に_対応_される必要があり、バックログに放置してはいけません。Issue に直接アサインされた開発者がいる場合、その開発者がその Issue の解決に関する最初の窓口であり権限者であるべきです。一般に、EM と PM が、これらが目標期間内に解決されるようにする責任を持ちます。

### 計画された作業 {#planned-work}

- [Pipeline Execution Current Milestone](https://gitlab.com/groups/gitlab-org/-/boards/1372896?milestone_title=Started&label_name[]=group%3A%3Apipeline%20execution): 計画済みか計画外かを問わず、すべての作業はその Issue を提供する予定のマイルストーンにアサインされます。このボードには現在進行中の Issue が一覧表示されます。アサインされている人がいない場合、その Issue は Ready For Development としてマークされ、いつでも着手できる状態であるべきです。Issue が Ready For Development でない場合は、それが本当にそうであるかを確認するためにレビューするか、マイルストーンから外す必要があります。
- [Pipeline Execution Upcoming Milestone](https://gitlab.com/groups/gitlab-org/-/boards/1372896?label_name[]=group%3A%3Apipeline%20execution&milestone_title=Upcoming): 一般に、任意のマイルストーン中に EM と PM が次のマイルストーン向けの作業をキューに入れます。私たちが取り組むプロジェクトの一般的な長さのため、ここには、開発者が現在の作業のフォローアップとして作成した新しい Issue が含まれることがよくあります。
- [Next 1-3 Releases](https://gitlab.com/groups/gitlab-org/-/boards/1372896?label_name[]=group%3A%3Apipeline%20execution&milestone_title=Next%201-3%20releases): これは比較的優先度の高い Issue のプールであり、今後のマイルストーンに向けた計画作業の主なソースであるべきです。定期的にこのリストをレビューし、優先度が最新であり、開発用に Issue を取り出せる健全なバックログ上位があることを確認する必要があります。
- [Net 4-6 Releases](https://gitlab.com/groups/gitlab-org/-/boards/1372896?label_name[]=group%3A%3Apipeline%20execution&milestone_title=Next%204-6%20releases): Next 1-3 Releases プールと似ていますが、緊急度は低めです。適切であれば開発に取り込むことも、1-3 リリースプールから Issue が予定されるにつれて、そのプールを補充するために使うこともできます。

FY27Q2 時点では、Backlog マイルストーンの Issue は定期的に能動レビューされていません。長期的な戦略計画を進める際に、関連する Issue をバックログから検索し、6 か月の計画ビューに取り込む場合があります。バックログ内の Issue に注意やより高い優先度が必要だと思う場合は、その Issue 内で PM または EM に連絡してください。

#### エンジニアリングが Issue をリファインする方法 {#how-engineering-refines-issues}

Issue がまだ **Ready for Development** とマークされていない場合、EM または PM は、その Issue が [Upcoming Milestone](https://gitlab.com/groups/gitlab-org/-/boards/1372896?label_name[]=group%3A%3Apipeline%20execution&milestone_title=Upcoming) に移る際に、エンジニアにレビュー、検証、リファインメントを依頼します

- 計画された作業は、完全なリファインメントと技術実装の提案なしに [Current Milestone](https://gitlab.com/groups/gitlab-org/-/boards/1372896?milestone_title=Started&label_name[]=group%3A%3Apipeline%20execution) に存在すべきではありません。**Ready for Development** であるべきです。
- 計画された作業は、**Refinement** 中の状態で [Upcoming Milestone](https://gitlab.com/groups/gitlab-org/-/boards/1372896?label_name[]=group%3A%3Apipeline%20execution&) に存在できます。この状態では、リファインメントを明示的に任されたチーム内の IC にアサインされているべきです。
- 計画外作業は、その予測しづらい性質上、現在および upcoming のマイルストーンに任意の状態で存在する場合があります。私たちは迅速に進めるよう最善を尽くしますが、アサインされたエンジニアが進行中の重大な問題を緩和または解決するために素早く動く必要がある場合、正式なプロセスはより軽量で見えにくいことがあります。
...
1. Issue の説明に提案がありますか？_もしあれば:_
    ...
    1. 実装に意図しない副作用はありますか？
1. Issue には、提供される価値に合った適切なラベル付けがされていますか？

    - Issue タイプ: bug、feature、maintenance。これらのラベルは、内部の可視性と企業コンプライアンス機能の両方にとって重要です。
    - 重大度ラベル付け: すべての Issue は、現在の正確性について重大度を慎重に評価する必要があります。一部の Issue、特にフォローアップ Issue は、元のソース Issue が解決された後も高めの優先度と重大度ラベルを持ち続けることがあります。リファインメント中は、リファインメントの範囲で提供される価値を検討することが重要です。
...
1. `~workflow::` ラベルを適切なステータスに更新する。例:
   ...
   - ~"workflow::blocked" : さらなる調査と研究が必要な場合。[リファインメント "~spike" Issue](https://gitlab.com/gitlab-org/gitlab/-/issues/new?issuable_template=Pipeline%20Execution%20Refinement%20Spike) を作成し、PM と EM にメンションする。
...
1. チームメンバーがコミットした作業を完了するために助けが必要かどうか確認する。[WIP を減らすために右から左へ作業する](#working-right-to-left-to-reduce-wip) を参照
2. ピックアップが必要なものがないか、[計画外作業の追跡 Issue](#unplanned-work) を確認する。確信が持てない場合は、注意が必要なものがあるか、またはないことを確認するため、EM または PM に確認してください。
3. 新しい[計画された作業](#planned-work)アイテムをピックアップする。まず Current Milestone ボードを確認し、アサインされていないものや助けられるものがなければ、Upcoming Milestone ボードのアイテムへ進みます。
...
優先事項に従って、使用されるリストがキュレーションされ、各チームメンバーがキャパシティに応じて、これらのリストからアイテムを引き出すことができます。
...
- エピック、Issue、またはマージリクエストをオープンするか、それで作業を開始するときに、すべてのステークホルダーがこれを認識しているか、または更新する必要があるかを検討してください。確信が持てない場合は、黙っているよりも更新する側に倒してください。
...
私たちにとっての最優先事項はユーザビリティであり、[JTBD](/handbook/engineering/devops/verify/pipeline-execution/jtbd/) を効果的に評価する 1 つの方法は、定期的な [UX スコアカード](/handbook/product/ux/ux-scorecards/) です。機能的なクラスタや `.gitlab-ci.yml` ファイルを持つプロビジョニングされた環境などのインフラストラクチャサポートを必要とする技術的なタスクについては、Product Designer とプロダクトマネージャーが、Engineering Manager および Quality の Stable Counterparts と協力して、JTBD のテストシナリオに基づいたプロジェクトを作成できます。この場合に協力するためのいくつかのガイドライン:
...
1. Issue のスケジューリング。エンジニアリングマネージャーは [PE Technical Debt issue board](https://gitlab.com/groups/gitlab-org/-/boards/3567075?scope=all&label_name[]=group%3A%3Apipeline%20execution&label_name[]=technical%20debt&assignee_id=None) をレビューし、Issue をマイルストーンの計画 Issue に追加するか、Product と来るべきマイルストーンで Issue をスケジュールする必要性についてコミュニケートすることで、今後のマイルストーンにスケジュールする Issue を推奨します。チームは、スプリントのキャパシティの 20% を技術的負債 Issue で埋めるよう努めます。
