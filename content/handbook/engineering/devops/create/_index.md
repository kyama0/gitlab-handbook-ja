...
月次マイルストーン計画Issueは、次のマイルストーンが始まる前の週に作成されるようトリガーされます。PM（プロダクトマネージャー）、EM（エンジニアリングマネージャー）、PD（プロダクトデザイナー）が一緒にマイルストーンを計画します。プロダクトデザイナーは以下の責任があります:

- **隔週でUXバグをトリアージ:** CreateのProduct Design Managerは、2週間ごとに未スケジュールのバグリストを確認するよう各チームのPDに連絡します。この演習を最大30分にタイムボックスし、1つずつ開く代わりに、リストをスキャンして何か目立つものがあるかどうかを確認することから始めることをお勧めします。デザイナーは、トリアージされたIssueに`severity`と`bug::ux`ラベルを割り当てます。デザイン提案の追加など、その他の作業はこのプロセスの範囲外です。
- **各マイルストーンで少なくとも5つのUXバグおよび/またはUX拡張を推奨する:** 通常のIssueバックログをレビューすることに加えて、UXバグとIssue拡張をレビューするためにこのGLQL [Wikiページ](https://gitlab.com/groups/gitlab-com/create-stage/-/wikis/Create-UX-Bugs-Dashboards)の使用を検討してください。マイルストーンに至る週中、PDは、優先順位付けに最も影響的で重要な少なくとも5つのバグおよび/またはUX拡張のリストを計画Issueに追加する必要があります。PDはこれらの項目を擁護し、優先順位付けされるためにコラボレートする必要がありますが、最終的な決定はチームのキャパシティ、実現可能性/ブロック状態、その他の優先事項（重大度の高いバグとInterlock項目）の対象となります。
