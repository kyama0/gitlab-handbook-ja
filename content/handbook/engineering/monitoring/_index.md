...
[各ステージグループ](/handbook/product/categories/#devops-stages) 向けに設計された一連のモニタリングダッシュボードがあります。これらのダッシュボードは、特定の機能カテゴリで働くすべての人に、コードが GitLab.com スケールでどのように動作するかについてのインサイトを提供するように設計されています。ステージグループごとにグループ化され、機能／コード変更、デプロイ、フィーチャーフラグの切り替えの影響を示します。
