...
[各ステージグループ](/handbook/product/categories/#devops-stages) 向けに設計された一連のモニタリングダッシュボードがあります。これらのダッシュボードは、特定の機能カテゴリーで働くすべての人に、コードが GitLab.com スケールでどのように動作するかについてのインサイトを提供するように設計されています。ステージグループごとにグループ化され、機能／コード変更、デプロイ、フィーチャーフラグの切り替えの影響を示します。
...
アラートルールを作成すると、グループが自身の機能をモニターし、問題をプロアクティブにキャッチするのに役立ちます。これにより、エラー予算 SLO を侵害する前に問題を修正でき、結果として GitLab.com のサービス可用性を高く保つのに役立ちます。
...
1. グループの機能カテゴリーに合わせて必要な条件を設定します。
