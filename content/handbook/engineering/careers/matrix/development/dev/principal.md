---
title: "Dev キャリアフレームワーク: プリンシパルエンジニア"
upstream_path: "/handbook/engineering/careers/matrix/development/dev/principal/"
upstream_sha: "3480299851f7e2243d4f08b75dac452f89929636"
translated_at: "2026-04-27T00:00:00Z"
translator: claude
stale: false
---

## プリンシパルエンジニア


<!-- include omitted: includes/engineering-careers/dev-career-matrix-nav.md (no localized version under content/ja/) -->


**GitLab のプリンシパルエンジニアには、以下のコンピテンシーを示すことが求められます:**

- [リーダーシップコンピテンシー](#leadership-competencies)
- [テクニカルコンピテンシー](#technical-competencies)
- [バリューへの整合](#values-alignment)

---

### リーダーシップコンピテンシー {#leadership-competencies}


<!-- include omitted: includes/engineering-careers/principal-leadership-competency.md (no localized version under content/ja/) -->


<!-- include omitted: includes/engineering-careers/development-principal-leadership-competency.md (no localized version under content/ja/) -->


- 任意の API エンドポイントについて GitLab コードベースを深く調査し、その機能を理解して他者に説明できる
- 他チームと頻繁に協力して、効率的なソリューションにつながる大規模な変更を調整する
- GitLab コードベース全体にわたるコードレビューを定期的に実施する

### テクニカルコンピテンシー {#technical-competencies}


<!-- include omitted: includes/engineering-careers/principal-technical-competency.md (no localized version under content/ja/) -->


<!-- include omitted: includes/engineering-careers/development-principal-technical-competency.md (no localized version under content/ja/) -->


- 複数のプロダクトにまたがる低速・非効率なコードを特定する
- サーバーサイドプログラミング言語とそのフレームワークに関する幅広い知識を持つ
- エンジニアリングの標準・ツール・プロセスを改善する

### バリューへの整合 {#values-alignment}


<!-- include omitted: includes/engineering-careers/values-competency.md (no localized version under content/ja/) -->


<!-- include omitted: includes/engineering-careers/development-principal-values-competency.md (no localized version under content/ja/) -->

