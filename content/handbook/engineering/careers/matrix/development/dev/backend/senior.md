---
title: "Dev キャリアフレームワーク: Senior バックエンドエンジニア"
upstream_path: /handbook/engineering/careers/matrix/development/dev/backend/senior/
upstream_sha: 3480299851f7e2243d4f08b75dac452f89929636
translated_at: "2026-04-27T00:00:00Z"
translator: claude
stale: false
lastmod: "2024-05-18T05:06:05+00:00"
---

## Dev Senior バックエンドエンジニア (BE)


<!-- include omitted: includes/engineering-careers/dev-be-career-matrix-nav.md (no localized version under content/ja/) -->


**GitLab の Dev Senior バックエンドエンジニアは、以下のコンピテンシーを発揮することが期待されます:**

<!-- markdownlint-disable MD051 -->
- [リーダーシップコンピテンシー](#leadership-competencies)
- [テクニカルコンピテンシー](#technical-competencies)
- [バリューズへの整合](#values-alignment)
<!-- markdownlint-enable MD051 -->

---

### Senior リーダーシップコンピテンシー {#leadership-competencies}


<!-- include omitted: includes/engineering-careers/senior-leadership-competency.md (no localized version under content/ja/) -->


<!-- include omitted: includes/engineering-careers/development-senior-leadership-competency.md (no localized version under content/ja/) -->


- 技術アーキテクチャの議論をリードし、技術的意思決定を推進する
- 複数のステージにまたがるコードレビューを定期的に実施する
- 必要に応じてワーキンググループに参加する

### Senior テクニカルコンピテンシー {#technical-competencies}


<!-- include omitted: includes/engineering-careers/senior-technical-competency.md (no localized version under content/ja/) -->


<!-- include omitted: includes/engineering-careers/development-senior-technical-competency.md (no localized version under content/ja/) -->


- ドキュメントの品質にかかわらず、サードパーティサービスと連携できる
- チームメンバーが作成した低速・非効率なコードを特定する
- サーバーサイドプログラミング言語とそのフレームワークに高度な専門知識を持つ
- DB の制約・リレーショナルと非リレーショナルの違い・潜在的な制限を理解している
- サービスや技術スタックの複数レベルにまたがるプロダクション問題をデバッグできる
- 既存の機能を拡張してアーキテクチャを発展させる

### Senior バリューズへの整合 {#values-alignment}


<!-- include omitted: includes/engineering-careers/values-competency.md (no localized version under content/ja/) -->


<!-- include omitted: includes/engineering-careers/development-senior-values-competency.md (no localized version under content/ja/) -->

