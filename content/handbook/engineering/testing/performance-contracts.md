...
### バックエンド／サービスコントラクト {#backend--service-contracts}
...
### フロントエンド／UI コントラクト {#frontend--ui-contracts}
...
## `performance.yml` コントラクト {#the-performanceyml-contract}
...
### コントラクト定義（必須） {#contract-definition-required}
...
### フロントエンド: SiteSpeed パフォーマンスバジェット {#frontend-sitespeed-performance-budgets}
...
### バックエンドエンドポイント {#backend-endpoints}
...
### 追加のサービスメトリクス {#additional-service-metrics}
...
#### LabKit v2 と SLI マッピング {#labkit-v2-and-sli-mapping}
...
## 採用ワークフロー {#adoption-workflow}

{{% alert title="マイルストーン 2 で対応予定" color="info" %}}
導入ワークフローとツールは Milestone 2（MVP）で利用可能になります。このセクションは、ツールの準備が整い次第、詳しい手順で更新されます。
{{% /alert %}}

### クイックスタート（予定） {#quick-start-planned}
...
### CI インテグレーション（予定） {#ci-integration-planned}
...
## LabKit にまだないメトリクスの扱い {#handling-metrics-not-yet-in-labkit}
...
## フィードバックと質問 {#feedback-and-questions}
