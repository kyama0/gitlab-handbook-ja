...
2. **`rack_request_protected_paths`** — protected-path レート制限のみ。これらは一般的なレート制限と重なります（protected API path への POST は両方を発火します）ので、別の limiter による独立した counter が必要です。
...
*解放されること:* 設定を通じたルールの追加と変更。ルール変更は、アプリケーション全体の build と deploy なしに展開されます。
...
*解放されること:* 顧客ごとに異なるルールと、デプロイなしのほぼ即時の変更の展開。
...
| 未認証リクエストの `<anonymous>` sentinel | 山括弧なしの sentinel が実際のユーザー名と衝突することを避け、ルールレベルの auth/unauth 区別を可能にする | [#28852](https://gitlab.com/gitlab-com/gl-infra/production-engineering/-/work_items/28852) |
