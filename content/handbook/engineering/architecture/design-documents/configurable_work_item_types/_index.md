...
| `configure_work_item_type` | Subgroup, project | Maintainer+ | Maintainer+ | サブグループおよびプロジェクトレベルでワークアイテムタイプ設定 UI にアクセスする。現在は委譲元から継承されているウィジェットのカスタマイズや階層のカスタマイズといった、将来のタイプごとの設定のためのエントリポイントとして予約されています。 |
