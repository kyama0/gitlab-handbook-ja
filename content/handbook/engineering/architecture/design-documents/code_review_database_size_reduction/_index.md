...
translator: codex
stale: false
lastmod: "2026-06-23T20:14:55+01:00"
---
...
1. 最も低リスクの変更で最大の節約を最初に達成し (クイックウィン)、その後より大きな構造的変更に進む。
1. ユーザー向け機能のリグレッションなしに、既存のアプリケーション動作との後方互換性を維持する。
1. 他のグループが自分の大きなテーブル (`issues` や `work_items` など) のために採用できる、繰り返し可能なパターン (HTML キャッシュクリア、保持ポリシーなど) を確立する。
1. 必要なときに、より大きな構造変更 (テーブル分解、列タイプ変換など) をサポートするため、アプリケーションとモデルコードを更新する。
1. 複数のマイルストーンにわたって変更を段階的に提供し、各変更が独立して価値があるようにする。
...
### スコープ外の機会 {#out-of-scope-opportunities}
...
#### 90 日間の保持ポリシー {#90-day-retention-policy}
...
詳細な設計と実装作業については、
[Code Review データベーステーブルのサイズを削減する (#17571)](https://gitlab.com/groups/gitlab-org/-/work_items/17571)
エピックを参照してください。

## 代替ソリューション

1. **[Cells](/handbook/engineering/architecture/design-documents/cells/)。** GitLab.com を複数の Cells に水平分割すると、データが独立したデータベースに分散され、単一の Code Review データベースのサイズを削減できます。ただし、Cells は長期的かつ組織全体のアーキテクチャ上の取り組みであり、これらのテーブルの短期的な成長に単独で対処するものではありません。
1. **データベース分解。** 既存の `main`/`ci` 分解パターンに従い、Code Review テーブルを専用データベースに移動できます。これはデータ自体を縮小しませんが、これらのテーブルを main primary から移すことで、この作業の動機となっている運用上の圧力を軽減します。ここで提案しているサイズ削減の代替ではなく補完であり、外部キー、結合、クロスデータベーストランザクションなど、重要な横断的労力を伴います。
1. **ClickHouse (カラム型ストア) へのオフロード。** GitLab はすでにアナリティクスとコントリビューションデータに ClickHouse を使用しており、監査イベントとエラートラッキングイベントを移行する提案も進行中です。`events` テーブルは自然な候補です。イベントを ClickHouse にストリーミングできます。ClickHouse はアナリティクスと履歴ユースケース向けにカラム型ストレージと高い圧縮率を提供し、一方で PostgreSQL は短いホットウィンドウのみを保持します。これは、[90 日間の保持ポリシー](#90-day-retention-policy)で説明したアーカイブステップの代替実装であり、テキスト量の多い `notes` や `merge_requests` テーブルではなく、`events` テーブルにスコープを限定します。
1. **[GCP の 64 TB ボリューム制限を緩和する (#2)](https://gitlab.com/groups/gitlab-com/gl-infra/data-access/dbo/-/work_items/2)。** LVM を使用して、アプリケーション変更なしで 1 ボリュームあたり 64 TB の上限を回避します。これにより即時の余裕が生まれ、`ci` データベースクラスタにはすでにデプロイ済みです。ただし、レイテンシ増加、パフォーマンス低下、より複雑な障害モードのリスクがあるため、長期的な修正ではなく、最後の手段のバックアッププランです。
