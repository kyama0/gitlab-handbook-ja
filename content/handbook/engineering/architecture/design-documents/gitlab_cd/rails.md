...
| description | text | |
| `last_rollout_iid` | integer | Application ごとの Rollout `iid` を割り当てるカウンター（[Rollout IID](#rollout-iids)を参照） |
...
|---|---|---|
| `organization_id` | bigint FK | シャードキー |
| `iid` | integer | ユーザー向け ID; `UNIQUE(application_id, iid)`; Application ごと（[Rollout IID](#rollout-iids)を参照） |
...
### Rollout IID {#rollout-iids}

Rollout には、グローバル主キーではなく、Issue の `#5` やパイプラインの `#123` と同様に Application ごとに連番となる、ユーザー向けの `iid`（UI の `#487`）があります。

`AtomicInternalId` は適合しません（project/namespace ごとにしかスコープされず、CD は Organization スコープであるため）。そのため、行ロックの下で Application ごとのカウンター（`cd_applications.last_rollout_iid`）から割り当て、`UNIQUE(application_id, iid)` で裏付けます。
