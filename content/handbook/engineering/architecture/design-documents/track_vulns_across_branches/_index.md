...
translator: claude
stale: false
lastmod: "2026-08-18T05:50:02-07:00"
---
...
1. **リリースのアテステーション** - 複数のサポート対象リリースブランチまたはタグ（たとえば `v1-stable`、`v2-stable`）を維持するお客様が、プロジェクトをフォークしたり GitLab ネイティブのトリアージツールを失ったりすることなく、「この特定のリリースにおける脆弱性の状況はどうなっているか」に答えられるようにします。
1. **開発者の行動に関するレポート** - デフォルトブランチに一度もマージされない脆弱性や、他のブランチより一貫して脆弱性が多いブランチといったデータを明らかにします。
1. **SDLC 全体にわたる脆弱性履歴** - デフォルトブランチだけを反映するのではなく、コードが開発、ステージング、本番ブランチを移動する際の脆弱性の状態をトラッキングします。
1. **ブランチ全体にわたるセキュリティポリシーの適用** - 新規と既存の検出結果の両方について、デフォルト以外のブランチ上の脆弱性をセキュリティポリシーで考慮できるようにします。

### 非目標

1. **すべてのブランチのトラッキングはサポートしません。** データベース容量を保護するため、トラッキング対象の ref はオプトインであり、[クォータによって制限](https://gitlab.com/groups/gitlab-org/-/epics/20475)されます。この制限が存在する理由については、[スケーラビリティ、ストレージ、パフォーマンス](#scalability-storage-and-performance)セクションを参照してください。
1. **複数ブランチの依存関係リストはスコープ内です。** [SBOM Across Contexts エピック](https://gitlab.com/groups/gitlab-org/-/epics/19809)を通じて、ブランチによる依存関係リストのフィルタリングと選択、およびブランチ対応の SPDX/CycloneDX/JSON エクスポートを扱います。
1. **個別の ID とアクセス管理はスコープ外です。** VAC はブランチごとの権限モデルを導入せず、プロジェクトの既存のアクセス制御を再利用します。このブループリントのスコープはデータレイヤー（脆弱性と検出結果のレコードがトラッキング対象の ref にどう関連するか）であり、ID や認可ではありません。
1. **複数ブランチを統合したレポートはスコープ外です。** SBOM と依存関係のエクスポートはアクティブなブランチフィルターに従いますが、複数のブランチにまたがる単一の脆弱性レポートを 1 つのドキュメントとしてエクスポートする計画はありません。エクスポートのスコープは、一度に 1 つのトラッキング対象 ref（またはデフォルトブランチ）に限定されます。
...
### スケーラビリティ、ストレージ、パフォーマンス {#scalability-storage-and-performance}
