...
stage: enablement
group: Tenant Scale
title: 'プロポーザル: ルート学習を使用したステートレスルーター'
status: rejected
toc_hide: true
upstream_path: /handbook/engineering/architecture/design-documents/cells/rejected/proposal-stateless-router-with-routes-learning/
...
このプロポーザルと[リクエストバッファリング](proposal-stateless-router-with-buffering-requests.md)を使用したものとの主な違いは、このプロポーザルがリクエストボディを正しい Cell にリダイレクトするためにプリフライト API リクエスト（`/api/v4/internal/cells/learn`）を使用することです。これにより、各リクエストが処理のために一度だけ送られますが、どの Cell に向けるべきかをデコードするために URI が使用されます。
...
1. ルーターが新しいリクエストを受け取ると、`/api/v4/internal/cells/learn?method=GET&path_info=/group-org/project` を送信してどの Cell が処理できるかを学習します
1. GitLab に Organization という新しいコンセプトが導入されます
1. すべての既存エンドポイントを URI でルーティング可能にするか、特定の Cell での処理に固定することが求められます。これには `/dashboard` のような曖昧なエンドポイントを `/organizations/my-organization/-/dashboard` のようなスコープ付きのエンドポイントに変更する必要があります
1. `/admin` のようなエンドポイントは常に `cell_0` のような特定の Cell にルーティングされます
1. 各 Cell は `/api/v4/internal/cells/learn` に応答でき、各エンドポイントを分類できます
...
リクエストを処理する際に URI がデコードされ、キャッシュされていない各エンドポイントに対してプリフライトリクエストが送信されます。

エンドポイントを問い合わせると、GitLab Rails はルーティング可能なパスについての情報を返します。GitLab Rails は `path_info` をデコードして既存のエンドポイントにマッチさせ、ルーティング可能なエンティティ（プロジェクトなど）を見つけます。ルーターはこれを短期間のキャッシュ情報として扱います。

1. プレフィックスマッチ: `/api/v4/internal/cells/learn?method=GET&path_info=/gitlab-org/gitlab-test/-/issues`
...
1. 一部のエンドポイントは完全一致が必要な場合があります: `/api/v4/internal/cells/learn?method=GET&path_info=/-/profile`
...
すべてのユーザーは、ユーザー設定で制御できる新しいカラム `users.default_organization` を取得します。私たちは `GitLab.com Public` Organization のコンセプトを導入します。これはすべての既存ユーザーのデフォルト Organization として設定されます。この Organization により、ユーザーは `Cell US0`（つまり元の GitLab.com インスタンス）のすべてのネームスペースのデータを表示できます。この動作は、`/dashboard` のようなグローバルページを表示するときにそれが Organization にスコープされていることを知らされない既存ユーザーには見えない場合があります。
...
1. URL が多種多様な場合（つまりロングテール）、ルーターキャッシュが非常に大きくなる必要があるかもしれません。そのような場合、ユーザークッキーに 2 番目のキャッシュレイヤーを実装して、頻繁にアクセスされるページが常に最初から正しい Cell に向かうようにする必要があるかもしれません。
...
1. すべての不明なルートは最新のデプロイ（`Cell US0` と仮定）に送られます。これは、新しく追加されたエンドポイントが最新の Cell によってのみデコード可能であるため必要です。`/internal/cells/learn` は軽量に処理でき、パフォーマンスへの影響はないはずなので、これはおそらく問題ではありません。
...
1. `/internal/cells/learn` が `Cell EU1` に送られ、リソースが `Cell EU0` に存在すると応答します
1. `Cell EU0` が正しいレスポンスを返します
...
1. `/internal/cells/learn` が `Cell EU1` に送られ、リソースが `Cell EU0` に存在すると応答します
1. `Cell EU0` はログインフローを通してリダイレクトします
1. ユーザーは `/users/sign_in` をリクエストし、`/internal/cells/learn` を実行するためにランダムな Cell を使用します
1. `Cell EU1` は固定ルートとして `cell_0` で応答します
1. ログイン後、ユーザーは `/my-company/my-project` をリクエストし、これは `Cell EU0` にキャッシュされ保存されています
1. `Cell EU0` が正しいレスポンスを返します
...
### ログイン中にプライベートなプロジェクトまたはグループ `/not-my-company/not-my-project` にアクセス
...
フローはログインユーザーと同様ですが、デフォルト Organization を選択できないため、`/dashboard` のようなグローバルルートはログインページにリダイレクトします。
...
1. `/admin` は `Cell US0` にロックされています
1. `/admin` の一部のエンドポイント（Admin の Projects など）は Cell にスコープされており、ユーザーはドロップダウンで正しいものを選択する必要があります。これにより `/admin/cells/cell_0/projects` のようなエンドポイントになります。
