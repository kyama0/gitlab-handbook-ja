...
AI Catalog のバックエンドアーキテクチャは、以下をサポートするため段階的に進化してきました（[用語集](https://docs.gitlab.com/development/ai_features/glossary/#agent-types) 参照）:
...
サービスアカウントは、フローまたは外部エージェントが **グループレベル** で有効化された時に自動的に作成されます。これらは [複合アイデンティティ](#agent-identity) 認証のためのマシンアイデンティティを提供します。
