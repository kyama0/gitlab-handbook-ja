...
しかし、大規模 Organization は [データベース負荷の大きな割合（internal link）](https://gitlab.com/gitlab-com/gl-infra/tenant-scale/staff/lstaff/-/work_items/3)を占めているため、レガシー Cell を最終的に十分に緩和するには、小規模 Organization だけでなく、それらも移動する必要があります。また、[データ量の大きな割合（internal link）](https://gitlab.com/gitlab-com/gl-infra/tenant-scale/tenant-services/team/-/work_items/417)も占めています。
...
2. **デプロイモデル: RunwayService と RunwayJobs。** Org Mover は永続的な状態データベースに支えられた長期稼働の [RunwayService](https://docs.runway.gitlab.com/welcome/introduction/) と、個々の移行フェーズを実行する RunwayJobs で構成されます。ジョブのみのモデルでは REST コントラクトを提供できず、リアルタイムのステータスもサポートできません。
...
5. **最初は人が go/no-go 判断を行います。** カットオーバーは、Org Mover が Organization のトラフィックを旧 Cell から新 Cell へ送る瞬間であり、移動が本番反映される時点です。カットオーバーの前に、SRE がレプリケーションと検証のステータスをレビューし、切り替えを承認します。メトリクスと確信度が高まれば、この判断は自動化できます。
...
9. **ルーティングは Topology Service に委譲されます。** Org Mover は Topology Service の 2 フェーズの claim update を通じてカットオーバーを呼び出し、ルートを検証します。Topology Service はルーティング権限を持つシステムです。
...
- **派生データストア（ClickHouse、Elasticsearch）の移行。** これらはレプリケーションされるのではなく、移行後にターゲット Cell 上で再構築されます。
...
1. **Org Mover サービスはコントロールプレーンであり、データはそれやそのジョブを通過しません。** Org Mover はコマンドを発行し、進捗を読み、判断します。調整されるシステムがバイト列を移動します。
...
- 移動状態を追跡する
- デプロイフリーズやアクティブな batched background migration の確認など、移動の遷移を事前検証する
- Organization Read-Only Mode を切り替える
- Siphon を設定および管理する
- Geo を設定および管理する
- 監査のために SRE の操作を Org Mover で追跡する
- 一時停止、再開、中止を調整する
...
- Org Mover がカットオーバーで呼び出す 2 フェーズの claim update、つまり後戻りできない時点
