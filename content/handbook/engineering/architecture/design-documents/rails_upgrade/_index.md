...
- Rails をドラフトマージリクエストで次のバージョンにバンプします
- 失敗した CI ジョブとテストを、マージリクエストにコミットをプッシュして修正します
- コミットをマージリクエストから別々の MR に抽出し、デフォルトブランチにマージします
  - 変更が次の Rails バージョンにのみ適用可能（変更/削除されたパッチなど）な場合は不可能なこともあります
