...
| 要件 | レベル | オーナー | 詳細 |
| ------------------------- | ------- | ------- | -------------------------------------------------------------- |
| Namespace 解決エンドポイント | MUST | AR | 指定された namespace UUID の namespace 詳細（スラグ、ステータス）を返します。Rails はプロビジョニング時に UUID を永続化し、スラグとステータスをキャッシュします（[ADR-022](../decisions/022_namespace_decoupling.md#slug-discovery)）。 |
