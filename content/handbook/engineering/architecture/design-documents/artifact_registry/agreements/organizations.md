...
組織 ID や属性は AR の公開 API や URL に出現しません。`organization_id` は厳密に内部的なもので、ビリング按分とスラグ検索に使われますが、クライアントには露出しません。

### AR が提供するもの

| 要件 | レベル | オーナー | 詳細 |
| ------------------------- | ------- | ------- | -------------------------------------------------------------- |
| スラグ検索エンドポイント | MUST | AR | 指定された Organization のスラグを返します。 |
| Namespace 作成 API | MUST | AR | 確認済み Organization と顧客選択のスラグにリンクされた AR namespace を作成する管理 API。 |
...
| 要件 | レベル | オーナー | 詳細 |
| ----------------------------------- | ------- | --------------- | -------------------------------------------------------------- |
| 不変な Organization ID | MUST | Organizations | Organization に割り当てられた `organizations.id` は変わっては **MUST NOT** いけません。AR はこれをアンカータプルに保存し、ビリング按分とスラグ検索に使用します。変更があると namespace が孤児化します。 |
