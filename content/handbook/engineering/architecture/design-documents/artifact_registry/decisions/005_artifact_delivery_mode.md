...
**却下理由:** 2 軸の形は、より小さく一貫したサーフェスで同じ範囲をカバーします。3 モードの形が節約する限界的なコストは、それが導入する運用上の曖昧さに見合いません。この代替案は元の決定であり、現在は置き換えられています。
...
**却下理由:** ネットワーク制約は通常、リポジトリレベルではなく、namespace/ネットワークの境界に適用されます。namespace 単位が適切な粒度です。具体的なユーザー需要が現れた場合、namespace の設定を継承しつつ個別に上書きできるリポジトリ単位の上書きを将来検討できますが、これは対象外です。
...
- [ADR-008: コンテンツアドレス可能ストレージ](008_content_addressable_storage.md)（読み取りパス）
- [ADR-009: API 設計](009_api_design.md)（ダウンロードエンドポイント）
- [ADR-022: ネームスペースの分離](022_namespace_decoupling.md)（設定境界としての namespace）
- [S01: HTTP サーバーとルーティング](https://gitlab.com/gitlab-org/ops/artifact-registry/-/merge_requests/34)（この決定の影響を受けるタイムアウトとレスポンスサイズの前提）
- [S06: ストレージレイヤー](https://gitlab.com/gitlab-org/ops/artifact-registry/-/merge_requests/108)（解決アルゴリズムを実装。`StorageConfig.delivery_mode` proto フィールド、`BlobStore.OpenBlob` の `WithDeliveryMode` オプション、`WithForceStream` の優先順位）
- [リダイレクト対プロキシに関する関連ディスカッション](https://gitlab.com/gitlab-org/ops/artifact-registry/-/merge_requests/34#note_3276797114)
- [2 軸への集約の根拠](https://gitlab.com/gitlab-org/ops/artifact-registry/-/merge_requests/108#note_3353135649)（この決定を導いた MR-108 のディスカッション）
