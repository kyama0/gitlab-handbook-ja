...
- **ライフサイクルイベント**（`deleted_at`、`purged_at`）: [ADR-015（内部）](https://internal.gitlab.com/handbook/engineering/architecture/design-documents/artifact_registry/decisions/015_slug_policy/#slug-lifecycle)で定義された slug ライフサイクルを実装します。`deleted_at` はソフト削除されたネームスペースを示し、[ADR-010](010_data_retention.md#subscription-expiration) のソフト削除期間内は回復可能です。`purged_at` はハード削除されたネームスペースを示し、slug は恒久的に廃止され、アンカーと課金カラムは監査とフォレンジックのため保持されます。この 2 つは一方向のイベント記録です。一度設定されると設定されたままになります。purge 済みの行では両方が設定されます。
- **サービス条件**（`blocked_at`、`disabled_at`、`suspended_at`）: それ以外は稼働中のネームスペースに対する可逆的な制限です。タイムスタンプは条件が課された時刻を記録し、`NULL` は有効でないことを意味します。繰り返しサイクルの履歴は行ではなく監査イベントストリームに保存されます。`blocked_at` はセキュリティブロックを示します（slug は予約され、リクエストは処理されません。ネームスペースはサブスクライブ済みのままです）。`disabled_at` は、所有 Organization がレジストリをオフにしたことを示します。リクエストは処理されず、データは保持されます。これは Rails 側の Organization レベル設定によって駆動され、内部 API を通じて設定および解除されます。`suspended_at` は、内部 API を通じて課されるビリング停止を示します。サービスは読み取り専用に劣化します。ダウンロードは処理され、書き込みは拒否されます。そのため支払いの失効が本番環境のコンシューマーを壊すことはありません。これらのカラムは独立しているため条件は共存でき、1 つを解除しても別の条件は解除されません。
- **サービス提供可否の述語**: 書き込みを処理する検索（push、ミューテーション）では 5 つすべてのカラムが `NULL` である必要があります。読み取りを処理する検索（ルーティング、認可、ダウンロード）では、`deleted_at`、`purged_at`、`blocked_at`、`disabled_at` が `NULL` である必要があります。停止中のネームスペースは読み取りを引き続き処理します。サブスクリプションライフサイクルのクエリ（課金、保持、スケジュールジョブ）は、ブロック済み、無効化済み、停止中のネームスペースもサブスクライブ済みのままであるため、`deleted_at` と `purged_at` のみを除外します。API は派生ステータス（優先順位: purged、deleted、blocked、disabled、suspended、active）を公開し、Rails は表示用にそれをキャッシュします（[ADR-022](022_namespace_decoupling.md#slug-discovery)）。
...
リポジトリをハード削除すると、その構造上の子に cascade されます。関係する外部キーには、次の参照アクションがあります。

- フォーマット子テーブル（`container_repositories`、`npm_repositories`、`maven_repositories`、およびそれぞれの virtual/remote バリアント）、`repositories` を参照する FK `(repository_id, namespace_id)`: `ON DELETE CASCADE`。
- `repository_collection_repositories`（コレクション結合テーブル）、`repositories` を参照する FK `(repository_id, namespace_id)`: `ON DELETE CASCADE`。
- アーティファクトテーブル（`container_images`、`npm_packages`、`maven_packages`）、それぞれのフォーマット子テーブルを参照する FK `(<format>_repository_id, namespace_id)`: `NO ACTION`。

したがって、単一の `DELETE FROM repositories` はフォーマット子行とすべてのコレクションリンクを削除しますが、まだアーティファクトを保持しているリポジトリは削除できません。アーティファクトから子への `NO ACTION` 外部キーが cascade を拒否し、ステートメント全体が中止されるためです。ルールは、子が純粋な構造（子行、コレクションリンク）である場合は cascade し、ユーザーデータ（アーティファクト）である場合は拒否することです。これは、上記のソフト削除パスと、アプリケーションが管理する `blob_storage_attachments` クリーンアップに対応する宣言的な仕組みです。

#### インデックス {#repositories-indexes}

- **`repositories`**: `(namespace_id, name)` に対するユニークインデックス — アクティブなリポジトリとソフト削除されたリポジトリの両方にまたがって名前の一意性を強制し、名前の衝突によって復元が失敗することがないようにします。名前の再利用にはまずハード削除が必要です。`(namespace_id, name) WHERE soft_deleted_at IS NULL` に対するインデックス — アクティブなリポジトリの検索と名前順の一覧表示のために最適化されたスキャンパスです。`(namespace_id, format) WHERE soft_deleted_at IS NULL` に対するインデックス — アクティブなリポジトリをフォーマットでフィルターします。`(namespace_id, kind) WHERE soft_deleted_at IS NULL` に対するインデックス — アクティブなリポジトリを種類でフィルターします。`(namespace_id, visibility) WHERE soft_deleted_at IS NULL` に対するインデックス — リポジトリを可視性レベルでフィルターします（可視性監査クエリ「このネームスペースで現在公開されているリポジトリはどれか?」を支えます）。ランディングページのソート可能なカラムごとに 1 つずつ、すべて `WHERE soft_deleted_at IS NULL` と末尾の `id DESC` keyset tiebreaker を持つインデックス: `(namespace_id, artifacts_count DESC, id DESC)`、`(namespace_id, downloads_count DESC, id DESC)`、`(namespace_id, size_bytes DESC, id DESC)`、および式インデックス `(namespace_id, COALESCE(last_updated_at, created_at) DESC, id DESC)`。`ROW(<col>, id)` keyset 境界の `id` 側はインデックス内に存在する必要があります。カウンターカラム（`artifacts_count`、`downloads_count`、`size_bytes`）はデフォルトが `0` でカーディナリティが低いため、インデックス内に tiebreaker がないと、planner はそれをスキャン後のソート/フィルターとして適用し、深いページではネームスペースに対して O(offset) に劣化します。式インデックスにも `id DESC` を含めますが、これはカーディナリティのためではありません（`created_at` は行ごとに一意なので、`COALESCE(last_updated_at, created_at)` はカーディナリティが高いです）。keyset に決定的なカーソルを与えるためです。新しさによるソートは、生の `last_updated_at` ではなく `COALESCE(last_updated_at, created_at)` で並べます。更新されたことがないリポジトリ（`last_updated_at` が NULL。新しいリポジトリでは一般的なケース）は、`NULLS LAST` の下に沈むのではなく、作成時刻で順位付けされ、「最近更新された」ものの上位に並びます。また、coalesce されたキーは決して NULL にならないため、keyset 境界は NULL 領域用の特別なケースなしに、インデックス範囲へ折り込まれる単純な `ROW(<col>, id)` 比較になります。シリアライズされる `last_updated_at` フィールドは nullable のままです。coalesce するのはソートキーだけです。`name` ソートには tiebreaker は不要です。`(namespace_id, name)` ユニークインデックスがすでに決定的な keyset だからです。`(namespace_id, format, name) WHERE soft_deleted_at IS NULL` に対するインデックス — フォーマットでフィルターされた名前順一覧（主要なブラウズビュー: フォーマットでフィルターし、名前でソート）用です。`(namespace_id, name)` ユニークインデックスを起点にして `format` をインデックス後フィルターにし、別フォーマットの行を読み飛ばすのではなく、`(format, name)` 範囲を直接 seek し、インデックスオンリーのままにします。インデックス化するのは format+name の組み合わせだけです。よりまれなカウンター/タイムスタンプのソートでは、ソートとフィルターの組み合わせごとに複合インデックスを増やすのではなく、`format`/`kind` はインデックス後フィルターのままにします。`(namespace_id, soft_deleted_at DESC) WHERE soft_deleted_at IS NOT NULL` に対するインデックス — このネームスペース内のソフト削除されたリポジトリを削除時刻順に一覧します（ゴミ箱一覧クエリ「ゴミ箱には何があり、いつ削除されたか?」を支えます）。この逆の部分述語は上記のアクティブ行の部分インデックスを反映したものです。このテーブルの他のすべての部分インデックスはゴミ箱を除外し、完全な `(namespace_id, name)` ユニークインデックスは `soft_deleted_at` をキーにしないため、ゴミ箱の一覧表示はそうでなければフィルターとソートのためにネームスペース内のすべての行を訪れなければならなくなります。GC の適格性は [ADR-010](010_data_retention.md) に従って `soft_deleted_at + retention_window` から導出されます。別個のカラムは不要です。
...
いくつかのカラムは、すべてのダウンロードまたはアップロードリクエストで更新されます。`repositories` のカウンターカラム（`artifacts_count`、`downloads_count`、`size_bytes`）、エンティティ数制限チェックに使用される `npm_packages` のパッケージごとのカウンター（`versions_count`、`tags_count`）、Maven と npm のバージョンテーブル（`maven_versions`、`maven_remote_versions`、`npm_versions`、`npm_remote_versions`）の `size_bytes` カウンター、そして `container_images`、`maven_packages`、`maven_versions`、`npm_packages`、`npm_versions` の `last_downloaded_at` タイムスタンプです。これらをリクエストパスで直接書き込むと、同じ行に対する同時リクエストがシリアライズされ（人気のパッケージでのホット行の競合）、リクエストレイテンシがデータベースの書き込みスループットに結合されます。
...
1. **オンデマンドの検証**: ユーザーが「このリポジトリは実際にどれだけのストレージを使っているか?」と尋ね、私たちはソース行から正確な値を導出する必要があります。
2. **ドリフトの修正**: 失敗したフラッシュ、部分的なバッファ損失、またはバックグラウンドジョブのバグによってカウンターが非同期化し、それを再計算する必要があります。

調整は `(repositories.format, repositories.kind)` で分岐します。各組み合わせは、異なるテーブルの連鎖を通じて blob ストレージへ到達するためです。各フォーマットでは、クエリがリポジトリ内のアーティファクトから参照されるすべての blob `sha256` を収集し、リポジトリ内の重複排除のために `DISTINCT` を適用し、サイズを得るために `blob_storage_blobs` に JOIN します。クエリは、中間層テーブルを `*_repository_id` カラムでフィルターします。これらのカラムは `repositories.id` ではなく、フォーマット固有のスタブテーブル自身の `id` を参照します。下記のクエリ例は、小さな CTE を介して `repositories.id` からスタブ `id` を解決するため、親識別子で呼び出せます。

- **Container**: `container_images`（`container_repository_id` でフィルター）→ `container_blobs` と `container_manifests`。両方のテーブルが `blob_sha256` を持ちます。union によってレイヤー blob と manifest payload をカバーします。
- **Maven**: `maven_packages`（`maven_repository_id` でフィルター）→ `maven_files`。単一のファイルテーブルが、パッケージのバージョン固有ファイルとパッケージレベルファイルをカバーします。
- **npm**: `npm_packages`（`npm_repository_id` でフィルター）→ `npm_versions` → `npm_files`。さらに `npm_metadata_files`（バージョンではなくパッケージレベルをキーにします）との union を取ります。
- **Remote バリアント**（`kind = remote`）は、キャッシュテーブル（`*_remote_*`）に対して同じ形に従います。キャッシュはリポジトリのフットプリントの一部です。

`SUM(DISTINCT bsb.size)` は誤りです。異なる blob が同じ `size` 値を持つことがあり（同じ長さの小さなファイルがつぶれてしまいます）、調整ではまず `SELECT DISTINCT blob_sha256` し、その後に `blob_storage_blobs` へ JOIN して合計する必要があります。

ソフト削除された行も含めることで、再計算された値が `repositories.size_bytes` が追跡するものと一致します。カウンターはリポジトリ内で重複排除されます（調整での `DISTINCT blob_sha256` と一致します）。つまり、ある `sha256` がリポジトリに初めてアタッチされたときにのみインクリメントし、その `sha256` の最後のアタッチメントをガベージコレクションがハード削除したときにデクリメントします。ソフト削除と復元は no-op であり、上記のネームスペースレベルの振る舞いと一致します。

調整コストはネームスペースの大きさではなく、リポジトリのアーティファクト数に比例します。フォーマット固有の file、blob、manifest テーブル（container の `container_blobs` と `container_manifests`、Maven の `maven_files`、npm の `npm_files` と `npm_metadata_files`、およびそれぞれの `*_remote_*` キャッシュバリアント）は `HASH(namespace_id)` でパーティショニングされるため、walk の各側は単一パーティションにプルーニングされます。これらのテーブル上の既存のフォーマット固有インデックス（`(namespace_id, container_image_id, digest)`、`(namespace_id, maven_package_id, file_name)`、`(namespace_id, npm_version_id, file_name)` と、それぞれの remote 版）は部分インデックス（`WHERE soft_deleted_at IS NULL`）であり、ソフト削除を含む walk を直接満たすことはできません。リポジトリごとのカーディナリティはデータモデルによって境界づけられ（アーティファクトごとにファイルまたは blob 参照 1 行、container では manifest が小さな追加要素）、調整は頻繁ではない（オンデマンドまたはドリフト修正であり、ホットパスではない）ため、境界のあるパーティションスキャンは許容できます。[ネームスペースレベルの調整](#namespace-level-storage-accounting-reconciliation)からの最後の `blob_storage_blobs_by_namespace` lookup は、単一パーティションのインデックスオンリースキャンです。

リポジトリレベルの調整には、追加の保険構造（カバリングインデックス、シャドウテーブル）は導入しません。本番メトリクスが非常に大きいリポジトリで遅すぎることを示した場合、最も安価な次のステップは、それらの各テーブルに非部分的な `(namespace_id, parent_id)` インデックスを追加することです。その先では、フォーマット固有テーブルでの attach/detach 時に維持される、`(namespace_id, repository_id, sha256, size)` をマップするリポジトリパーティショニングされたシャドウテーブルが、より細かいスコープで [オプション B](#namespace-level-storage-accounting-reconciliation) の形を反映します。

#### アーティファクトレベルのストレージ会計 {#artifact-level-storage-accounting}

アーティファクトレベルのストレージ使用量は、単一のアーティファクトバージョン（container manifest、Maven バージョン、npm バージョン）のバイトフットプリントです。これは、アーティファクトごとの UI 表示（例: 「このイメージは 142 MB」、「このパッケージバージョンは 4 MB」）を支え、サイズでフィルターまたはソートする [ライフサイクルルール](010_data_retention.md) とレポートクエリで使用されます。

会計モデルは、基礎となるアーティファクトの形が異なるため、フォーマットごとに異なります。

- **Container manifests**: `container_manifests.size` は push 時に事前計算され、不変です。manifest はコンテンツアドレス可能（[ADR-008](008_content_addressable_storage.md)）なので、バイトが変更されると新しい digest と新しい `size` を持つ新しい manifest が生成されます。このカラムが信頼できる唯一の情報源なので、調整は不要です。remote cache は `container_remote_manifests.size` でこれを反映します。この値は、子が遅延キャッシュされる manifest list に対して [progressive semantics](#container-remote-repositories) を持ちます。
- **Maven and npm versions**: 各 version 行は、バージョンのフットプリントに対する信頼できる唯一の情報源である、事前計算された `size_bytes` カラム（`maven_versions`、`maven_remote_versions`、`npm_versions`、`npm_remote_versions` 上）を持ちます。`container_manifests.size` と異なり、push 時に不変に設定することはできません。Maven と npm のバージョンはバージョンレベルではコンテンツアドレス可能ではなく、ファイルはバージョンのライフタイム中に追加または削除できるためです。そのため、`repositories.size_bytes` と同様に [バッファ書き込み/非同期書き込み](#buffered-and-asynchronous-writes) を介したバッファカウンターとして維持されます。バージョンに `blob_sha256` が初めてアタッチされたときにインクリメントし、そのバージョン内のその `sha256` の最後のアタッチメントをガベージコレクションがハード削除したときにデクリメントします（調整で使用する `DISTINCT blob_sha256` と一致するよう、バージョン内で重複排除します）。表示パスとバージョン一覧は、サイズでソートまたはフィルターする場合も含め、インデックス化されたカラムを直接読み取ります。

ソフト削除されたファイルは、ガベージコレクションがハード削除するまで、バージョンごとの `size_bytes` に寄与し続けます。ソフト削除と復元はカウンターに対して no-op であり、ネームスペースとリポジトリのセマンティクスと一致します。

読み取り時に導出するのではなくカラムを事前計算することで、バージョン一覧をサイズでソートおよびフィルターできます。単一バージョンのサイズを導出することはどちらの方法でも安価です（境界のある数ファイルの JOIN）が、サイズがバージョン一覧のソート可能またはフィルター可能なカラムになると、すべての行について導出する方法はスケールしなくなります。Cloud SQL PostgreSQL 17 インスタンス（`large` プロファイル、最も深いネームスペースに約 26K の Maven バージョンと約 26K の npm バージョン）で検証しました。

| サイズ上位 50 バージョン | Maven | npm |
| --- | --- | --- |
| 読み取り時に導出（各バージョンの blob を合計） | 58 ms | 29 ms |
| 事前計算された `size_bytes` カラム + インデックス | 0.06 ms | 0.08 ms |

これは 3〜4 桁の差であり、重複排除された読み取り時導出版はさらに遅く（Maven で約 190 ms）、バージョン数とともに差は広がります。ランディングページがすでにリポジトリを `size_bytes` でソートしているため、同じ期待値がバージョン一覧にも適用されます。したがって、スコープ全体の一貫性のためにカラムを事前計算します。

#### アーティファクトレベルのストレージ会計の調整 {#artifact-level-storage-accounting-reconciliation}

`container_manifests.size` は不変でコンテンツアドレス可能なので、調整は不要です。Maven と npm の `size_bytes` カウンターは、リポジトリレベルおよびネームスペースレベルのカウンターと同様に、失敗したフラッシュ、部分的なバッファ損失、またはバックグラウンドジョブのバグによってドリフトする可能性があるため、オンデマンド検証またはドリフト修正のためにソースデータから正確な値を再計算します。

1 つのバージョンに対する調整では、フォーマット固有のファイルテーブルからそのバージョンの distinct な `blob_sha256` 値を選択し、blob ごとのサイズを得るために `blob_storage_blobs` に JOIN します。これは、単一の `maven_version_id` または `npm_version_id` にスコープした [リポジトリレベル](#repository-level-storage-accounting-reconciliation) の調整と同じ walk です。ソフト削除されたファイルも含めることで、再計算値がカウンターの追跡対象と一致します。バージョンごとのカーディナリティはフォーマットプロトコルによって境界づけられるため（通常 Maven は 4〜15 ファイル、npm は 1〜3 ファイル）、再計算はサブミリ秒の単一パーティションスキャンです（検証では約 0.6 ms）。クエリ例は [blob ストレージのクエリ例](#blob-storage-query-examples) にあります。

カラム導入時のバックフィルは、バージョンでグループ化したセットベースの再計算です。現在のボリュームでは安価です（最も深いネームスペースの約 26K バージョンのバックフィルが約 284 ms なので、この規模ではテーブル全体でも数秒です）。これは、事前計算を先送りして本番ボリュームに対して後からバックフィルするよりもはるかに安価です。今実施することで、3 つすべてのフォーマットにわたってデータモデルと API の一貫性も保てます。
...
- [リポジトリレベルの調整](#repository-level-storage-accounting-reconciliation): フォーマット別に、単一リポジトリの正確なストレージを求める。各先頭 CTE は `repositories.id` からフォーマット固有のスタブ `id`（`container_repositories`、`maven_repositories`、または `npm_repositories`）を解決します。中間層の `*_repository_id` カラムは、`repositories.id` ではなく、スタブテーブル自身の `id` を参照します。

  - **Container**: images を walk し、`container_blobs` と `container_manifests` から blob refs を union し、`blob_sha256` で重複排除し、ネームスペースのシャドウテーブルを介して合計します。
  ...
  - **npm**: packages → versions → files を walk し、パッケージレベルのメタデータファイルを union し、`blob_sha256` で重複排除します。
...
- [アーティファクトレベル](#artifact-level-storage-accounting): 事前計算された container manifest のサイズを読み取る（不変、単一行検索）
...
- [アーティファクトレベル](#artifact-level-storage-accounting): 事前計算された Maven バージョンサイズを読み取る（信頼できる唯一の情報源、単一行検索）
...
- [アーティファクトレベルの調整](#artifact-level-storage-accounting-reconciliation): カウンターを検証または修正するために、ソースデータから Maven バージョンの正確なサイズを再計算する（通常 4〜15 ファイル。境界づけられたカーディナリティ）
...
- [アーティファクトレベル](#artifact-level-storage-accounting): 事前計算された npm バージョンサイズを読み取る（信頼できる唯一の情報源、単一行検索）
...
- [アーティファクトレベルの調整](#artifact-level-storage-accounting-reconciliation): ソースデータから npm バージョンの正確なサイズを再計算する（通常 1〜3 ファイル/バージョン。`npm_metadata_files` はパッケージレベルをキーにしており、単一バージョンのフットプリントには含まれません）
