...
識別子を導出します。リゾルバーは、Artifact Registry の認可失敗を not-available
エラーに、トランスポート失敗を service-unavailable エラーにマッピングし、
...
- サービス間の結合は依然としてフロントエンドに落ちる（Alternative 1 と同じ形）。
...
- サービス間の結合と CORS。`postMessage` パスは Alternative 3 と同じコストがかかる。
