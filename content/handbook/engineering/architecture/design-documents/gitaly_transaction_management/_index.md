...
{{% alert title="今後は推進せず、実装の削除を進めています" color="warning" %}}
ここで説明する先行書き込みログ、トランザクションマネージャー、Raft サービスは、
Praefect を使わない Gitaly Cluster という方向性に向けて構築されました。その方向性は取りやめとなったため、
この設計を今後推進することはありません。（利用可能なステータスでは「Rejected」が最も近いものです。
設計は大部分が実装済みで、レビューで却下されたのではなく、現在は実装を撤去している段階です。）

ステージングではトランザクションが無効化されており、Gitaly からコードを削除しています。この作業は
[Raft、WAL、トランザクションコードの削除](https://gitlab.com/groups/gitlab-org/-/epics/23151)で追跡しています。
Housekeeping の再ハイドレーションとオフロードはトランザクションを基盤に構築されているため、
削除が反映されると、これらの機能はサポートされなくなります。
{{% /alert %}}
...
Git はスナップショット分離を実装するためのツールをネイティブに提供していません。したがって、リポジトリスナップショットはファイルシステム上でリポジトリのディレクトリ構造を一時ディレクトリにコピーし、リポジトリの内容をハードリンクすることで実装されます。Git は参照やオブジェクトをその場で更新せず、常に新しいファイルを書き込むため、ハードリンクされたファイルはスナップショット内で変更されずに保たれます。読み取りスナップショットのための正しいバージョンのカスタムフックも所定の位置にリンクされます。パフォーマンスに関する懸念については、[パフォーマンスへの考慮事項](#performance-considerations)を参照してください。
...
pre-receive フックは書き換えられたリポジトリを `internal/allowed` の認可エンドポイントに送信します。エンドポイントから Gitaly へのフォローアップリクエストには、既にクォランティンが設定されたスナップショットリポジトリを指す相対パスが含まれています。トランザクションミドルウェアはこれを検出し、別のトランザクションを開始しません。
...
### パフォーマンスへの考慮事項 {#performance-considerations}
...
### クライアントへのトランザクションの公開 {#expose-transactions-to-clients}
...
### Raft レプリケーション {#raft-replication}
