...
2. スケジュールされた PCL テーブルを更新するMRがインフラストラクチャプラットフォームエンジニアリングディレクターによって承認される。
...
| 実装レベル | テックスタック内でコードネームを tenant_id にマッピングする必要がある場合 | <ul><li>このユースケースには内部コードネームを使用する。</li><li>Switchboard が内部コードネームの SSOT。</ul></li> |
