...
- 高次元フィールド名によって発生したインシデント数を削減することで、すでに**Self-Managed のアップグレード体験を改善**しました。
- 次の目標は、インシデント対応時の手間を削減するために GitLab のすべてのサービスで使用される一貫したフィールドスキーマを定義することです。これは**Self-Managed と SaaS の両方で GitLab 管理のための監視と可観測性機能の強化**に直接結びついています。
...
- これにより Observability チームが OTeL ロギングの採用に向けた取り組みを加速できるようになり、**Self-Managed と SaaS の両方で GitLab 管理のための監視と可観測性機能の強化**にもつながります。
