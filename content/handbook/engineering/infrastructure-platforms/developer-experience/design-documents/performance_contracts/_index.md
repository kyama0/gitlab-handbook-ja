...
### 現在 (マイルストーン 1-4) {#current-milestone-1-4}
...
### フロントエンド: SiteSpeed の構造的なビュー {#frontend-sitespeed-structural-view}
...
## スキーマ設計の決定 {#schema-design-decisions}

### 決定: エンドポイントのカテゴリを自由形式のラベルとする {#decision-endpoint-categories-as-free-form-labels}
...
### 決定: パフォーマンスティアをスキャフォールディングのデフォルトとする {#decision-performance-tiers-as-scaffolding-defaults}
...
### 決定: `resources` および `database` セクションを MVP ではオプションとする {#decision-resources-and-database-sections-are-optional-for-mvp}
...
### 決定: `sli_mapping` は LabKit v2 のメトリクス名を直接参照する {#decision-sli_mapping-references-labkit-v2-metric-names-directly}
...
### 決定: スキーマの正規の場所はツーリングの選定まで保留する {#decision-schema-canonical-location-deferred-pending-tooling-selection}
...
### 決定: フロントエンド設定を `performance.yml` の `frontend` 配下に名前空間化する {#decision-namespace-frontend-configuration-under-frontend-in-performanceyml}
...
## 環境とツーリングの決定 {#environment-and-tooling-decisions}

### 決定: 環境管理ツーリングの選定 {#decision-environment-management-tooling-selection}
...
### 決定: スキーマ検証のアプローチ {#decision-schema-validation-approach}
...
## AI 統合の決定 {#ai-integration-decisions}

### 決定: GitLab Skills リポジトリにパフォーマンスコントラクトのスキルを公開する {#decision-publish-a-performance-contract-skill-to-the-gitlab-skills-repo}
...
### 注記: コントラクトの種類と SiteSpeed フロントエンドバジェット {#note-contract-types-and-sitespeed-frontend-budgets}
