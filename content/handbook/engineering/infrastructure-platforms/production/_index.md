...
{{% alert color="warning" %}}
GitLab のチームメンバーで、GitLab.com の可用性に関する問題について Reliability Engineering に通知したい場合は、こちらのインシデント報告に関する簡易手順を参照してください: [インシデントの報告](/handbook/engineering/infrastructure-platforms/incident-management/#reporting-an-incident)。
{{% /alert %}}
...
すべての構成、デプロイ、フィーチャーフラグのイベントは、events インデックスを使って Elastic Search に記録されます。
イベントへのアクセス方法やログ記録方法の詳細は、[events ランブック](https://gitlab.com/gitlab-com/runbooks/-/blob/master/docs/events/README.md) を参照してください。

これらのイベントには、デプロイ、フィーチャーフラグ、構成（Chef、Terraform）、すべての Kubernetes 変更が含まれます。
