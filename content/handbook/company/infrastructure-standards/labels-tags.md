...
- `team` の曖昧さについてのフィードバックと、スプレッドシートで行った再マッピング（ほとんどの部門にはグループ/チームがあるものの、エンジニアリングを除いてサブグループを持つものはなく、エンジニアリングにはサブ部門/ステージ（例: `Dev` - `Plan`）の組織階層がある）に基づき、`gl_team` を `gl_dept_group` に名称変更しました。エンジニアリングのサブ部門はラベル/タグ構造では使用しません。
...
- 短期および長期の[ワーキンググループ](/handbook/company/working-groups/)が、`wg-` プレフィックスと、各グループが選んだ分かりやすい名称（コードネームは不可）を使って `gl_dept_group` ラベルを使用できるようになりました。
...
```terraform
gl_owner_email_handle: 'yourname'
```
