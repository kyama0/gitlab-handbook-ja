...
translated_at: "2026-07-16T07:57:44+09:00"
translator: codex
stale: false
lastmod: "2026-07-15T04:18:40+01:00"
---
...
- 認識と使用のため、Slack チャンネル (`#social_media_action`、`#sales`、`#security_discuss`、`#customer-success`) に最終コミュニケーション資料を投稿し、デベロッパーリレーションズへの認識のため `@devrel` をピングする
...
- **サポートチーム:** セキュリティエンジニアオンコールおよびコミュニケーションマネージャーオンコールから提供される背景情報と準備された応答を使用して、私たちのサポートチームはセキュリティインシデントに起因する顧客コミュニケーションをトリアージし応答します。Slack の `#support_leadership` 経由でオンコールマネージャーに連絡します。緊急の場合は、PagerDuty アラートをトリガーして [`Support Manager On-call` をページング](/handbook/support/on-call/#engaging-the-on-call-manager) します。Slack で `#support_leadership` に移動し、`/pd trigger` と入力して Title フィールドに要約を書き、影響を受けるサービスの一覧から **Support Managers** を選択して、Create をクリックします。このグループがセキュリティインシデントとイベントを早期に認識できるよう、彼らは security-external-incident-or-event-response テンプレートで FYI として自動タグ付けされます。

- **デベロッパーリレーションズ:** ソーシャルチャンネル経由で顧客や一般大衆に応答する必要がある場合があるため、公開資料がリリースされる前にエンゲージする必要があります。彼らのインタラクションを支援するために、準備された応答や FAQ を提供する必要があります。このグループには任意の Slack チャンネルで `@devrel` をピングして連絡します。このグループがセキュリティインシデントとイベントを早期に認識できるよう、彼らは `security-external-incident-or-event-response` テンプレートで FYI として自動タグ付けされます。
...
- [JiHu](/handbook/finance/jihu-support/) へのコミュニケーションは、JiHu Slack ワークスペース内の [#security-vulnerability](https://gitlab-jh.slack.com/archives/C039R937PAN) チャンネル経由で行われる必要があります。
