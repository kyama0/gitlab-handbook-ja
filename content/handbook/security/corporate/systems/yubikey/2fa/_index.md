...
IT がリセットして再登録できるソフトウェア認証器とは異なり、YubiKey 認証を使用するシステムについては、セキュリティ姿勢を低下させるバイパス 2FA ポリシーに対象者を追加しない限り、IT 側でもサインインさせることはできません。必要な場合は対応しますし、全員にセカンダリ／バックアップキーを配り終えるまでの YubiKey の初期展開期間中も対応しています。
