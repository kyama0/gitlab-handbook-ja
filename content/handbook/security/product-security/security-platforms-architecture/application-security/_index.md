...
このリストはこのページに含めるには動的すぎるため、現在は[GitLab AppSecインベントリ](/handbook/security/product-security/security-platforms-architecture/application-security/inventory/)で管理されています。

期待される構成のないプロジェクトは、[インベントリ違反リスト](https://gitlab.com/gitlab-com/gl-security/product-security/inventory/-/issues)(社内リンク)で確認できます。

## GitLabインベントリ

[GitLab AppSecインベントリ](/handbook/security/product-security/security-platforms-architecture/application-security/inventory/)についてさらに学んでください。
