...
| **部門**|`Department:[department-name]`| 企業（-com）発見事項のリスクを所有する部門を特定するため |
| **グループ**|`group:[group-name]`| プロダクト（-org）発見事項のリスクを所有するプロダクトを特定するため |
...
| `USRM Workflow::Remediation Plan` | Remediation Manager がステップ、オーナー、期日を含む詳細な修復計画を文書化している | 修復作業が開始され、Issue がアクティブなモニタリングに入る時（`USRM Workflow::Monitoring` ラベルを適用） |
| `USRM Workflow::Monitoring` | 修復作業がアクティブな進捗モニタリングと共に進行中 - 完了と検証を待っている | Finding Identifier が修復完了を検証する時（`USRM Workflow::Closed` ラベルを適用） |
...
USRM 発見事項の品質を改善し、エスカレーションを可能にするため、以下のチェックが自動化（トリアージボット）またはエージェントによって定期的に実施されます:

1. 必須ラベルの適用: すべての発見事項 Issue には、メトリクスとレポート作成に必要なラベルが必要です
2. 期日の徹底: すべての発見事項 Issue には、現実的なタイムラインが必要です
3. 期日超過 Issue の追跡: 期日超過の Issue には、即時の注意と正当化が必要です
4. アサイン欠落管理: すべての発見事項 Issue には、指定されたオーナーが必要です
5. 自動ナッジング: Issue が非アクティブのままの場合、自動ナッジングを実施します
...
Slack で `#security_help` に連絡するか、関連する Issue で finding coordinator をタグ付けしてください。USRM の更新は `#security_discuss` で公開されます。月次の録画は、より詳細な分析と共に [公開](https://drive.google.com/drive/folders/1uC1aVlSoy4-bfvc4MIpwtKSznvx1e8Up?usp=drive_link) されます（社内のみ）。Risk Source Owner との定期的なミーティングは、傾向のレビューとフィードバックの収集のために実施されます。
