...
Zendesk インテグレーションとは、Zendesk とサードパーティサービスとの永続的な接続です。簡単に取り消せる API トークンとは異なり、インテグレーションは Zendesk の設定に深く埋め込まれており、削除するのは著しく難しくなります。インテグレーションは、永続的なアクセスを必要とする本番システムに対してのみ利用してください。
...
### API トークンまたはインテグレーションをリクエストする

API トークンとインテグレーションはどちらも、次の承認プロセスに従います。
...
1. **判断**: 承認されたら、エンジニアがトークンまたはインテグレーションを作成します

**主な違い:**

- **API トークン**: Zendesk で直接作成し、1Password 経由で共有
- **インテグレーション**: Integration bot アカウントで作成し、プロセスはインテグレーションの種類によって異なる
...
- インテグレーションは API トークンよりも取り消しが著しく難しく、より高いセキュリティリスクをもたらします。

**要件:**

- インテグレーションは Integration bot エージェントアカウントで作成する必要があります。
- Zendesk US Government に対するインテグレーションリクエストは、セキュリティ要件の都合により現在サポートされていません。
...
インテグレーションに関するすべてのリクエストは、[access request issue](https://gitlab.com/gitlab-com/team-member-epics/access-requests/-/issues/new?issuable_template=API_Token_Request) 経由で行ってください。

access request が作成されたら、リクエスト者のマネージャーが承認する必要があります。

承認が済んだら、そのインスタンスのプロビジョナー (通例 Fullstack Engineer, Customer Support Operations) がリクエストをレビューします。

このレビューでは、インテグレーションが持つアクセスレベルの大きさを踏まえ、ビジネス上の理由とユースケースを慎重に分析します。インテグレーションはさらに高いリスクをもたらすため、可能な限り避けるべきです。API トークンは素早く簡単に取り消せますが、インテグレーションはそうではありません。

問題ないと判断されたら、Fullstack Engineer, Customer Support Operations がインテグレーションを作成します。

その具体的な手段はインテグレーションの種類によって異なります。セットアップ手順は、各インテグレーションのドキュメントを参照してください。すべてのインテグレーションは、追跡と取り消し能力を確保するため、その Zendesk インスタンスの Integration bot アカウントで作成しなければなりません。
...
具体的な手順はインテグレーションごとに異なるため、ここでは記載できません。
