...
必要なセットアップをすべて行ったら、実施すべきテストスイートを含むタスク項目を Issue 上に生成する必要があります。実施が必要な各テストについて、子タスク項目を作成してください。

各子タスク項目について:

- 件名/タイトルはテスト対象の名前にします (例えば、SLA ポリシー `Priority Support - FRT` をテストする場合、件名/タイトルは `Priority Support - FRT` にします)
...
<details>
<summary>「Support Readiness SLA」を使用したテスト項目の例</summary>
...
テストスイートのすべての子タスク項目を生成したら、親 Issue にそれをまとめたコメントを追加します。次のような内容です。
...
{{% alert title="GitLab Duo エージェントの利用" color="primary" %}}

私たちは `CustSuppOps Zendesk Test Suite Generator` という名前の GitLab Duo エージェントを開発しました。このエージェントは、作業中のマージリクエスト (および関連付けられた Issue) を使用して、テストスイートを生成します。

使用方法:

1. マージリクエストを作成します (説明に親 Issue へのリンクを含めるようにしてください)
...
- 何を実施・確認しているか (および使用しているロジック) を提示する
- 子タスク項目の内容についての承認を求める
- 親 Issue への要約コメントの追加についての承認を求める
- 実施したすべてのアクションを要約する

`CustSuppOps Zendesk Test Suite Generator` が作業中のプロジェクトで実行可能かどうかを判断するには、該当する項目の対応するハンドブックページを参照してください。

{{% /alert %}}

テストスイート全体を生成したら、テストを実施する (またはテストの実施について SIG チームに支援を依頼する) 必要があります。

テストが実施されるにつれ、テストの結果と状態を子タスク項目に更新します。テストが失敗した場合は、気づいたことや変更が必要かどうかをノートまたはコメントに追加します。テストが完了したら (成功・失敗を問わず)、子タスク項目をクローズします。他のメンバーがすばやく状況を把握できるよう、子タスク項目の件名/タイトルを `:white_check_mark:` または `:x:` で編集して最終ステータスを示すと役立ちます。

{{% alert title="注意" color="warning" %}}

テストが失敗した場合、修正がマージリクエストにプッシュされた後で、テストスイート全体 (以前に行ったテストを含む) を新たに実施する必要があります。これにより、テストスイートの完了後に変更を行う必要が生じても、すべてが正常に動作することを保証できます。
