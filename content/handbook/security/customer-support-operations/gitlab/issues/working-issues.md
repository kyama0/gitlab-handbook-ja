...
<details>
<summary>「Support Readiness SLA」を使ったテストアイテムの例</summary>
...
私たちは `CustSuppOps Zendesk Test Suite Generator` という名前の GitLab Duo エージェントを開発しました。このエージェントは、あなたが作業しているマージリクエスト（およびリンクされている Issue）を使用して、テストスイートを生成してくれます。

使用するには:

1. マージリクエストを作成します（説明に親 Issue へのリンクを含めることを確認してください）
...
エージェントの実行中、次のことを行います:

- 何を行い、確認しているか（および使用しているロジック）を説明します
- 子タスクアイテムの内容について承認を求めます
- 親 Issue へのサマリーコメントの追加について承認を求めます
- 実行したすべてのアクションを要約します

`CustSuppOps Zendesk Test Suite Generator` があなたが作業しているプロジェクトで実行できるかどうかを判断するには、該当するアイテムのハンドブックページを参照してください。
