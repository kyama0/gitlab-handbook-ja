...
このドキュメントは、GitLab Customer Support Operations チームが従うべきコーディング規約とベストプラクティスを定義しています。これらのガイドラインを活用することで、私たちはチーム内での一貫性、保守性、シンプルさ、そしてより良いコラボレーションを確保します。私たちのプロジェクトへのすべてのコントリビューターは、特定の例外が合意されない限り、これらのガイドラインに従うことが期待されます。

## 一般原則

私たちは、[GitLab の基本的価値観](/handbook/values/) がすべてのコーディングに関する意思決定の指針となるべきだと考えています:

- Collaboration（コラボレーション）
  - チームメイトと協力し、お互いのコードをレビューし、知識を共有します。
  - 常にフィードバックを受け入れ、それに基づいてコードを改善します。
  - あなたのコントリビューションがチームの他のメンバーにとって読みやすく理解しやすいものであることを確保します。
- Results（結果）
  - コードの影響と有効性に焦点を当てます。
  - 完璧な解決策よりも動作する解決策を優先します（ただし品質を損なう近道は避けるようにします）。
- Efficiency（効率性）
  - 明確で保守しやすいコードを書きます。
  - 機能を犠牲にすることなくシンプルさを追求します。
- Transparency（透明性）
  - コードは理解しやすく、すべての人がアクセスできるものであるべきです。
  - 必要に応じて、前提、選択、設計上の決定を明確に文書化します。
- Iteration（イテレーション）
  - コードは柔軟でリファクタリングしやすいものであるべきです。
  - 私たちは継続的な改善を目指しており、必要なときにコードを更新したり作り直したりすることをためらいません。

## 命名規則

- 変数
  - 変数の目的を明確に伝える説明的な名前を使用します。変数の名前が目的を明確に伝えていない場合は、それをより分かりやすく説明するコメントをコードに追加します。
  - すべての変数名はスネークケースを使用するべきです:
    - 例: `sum_of_pairings`、`gitlab_user`
- 定数
  - 定数の目的を明確に伝える説明的な名前を使用します。定数の名前が目的を明確に伝えていない場合は、それをより分かりやすく説明するコメントをコードに追加します。
  - 定数は大文字のスネークケースで書くべきです:
    - 例: `MAX_RETRIES`、`DEFAULT_TIMEOUT`
- 関数とメソッド
  - 関数名とメソッド名は、それらが実行する動作を表すべきです。関数／メソッドの名前が目的を明確に伝えていない場合は、それをより分かりやすく説明するコメントをコードに追加します。
  - 関数名とメソッド名はスネークケースで書くべきです。
    - 例: `my_new_function`、`check_gitlab_user`
- クラスとモジュール
  - クラスとモジュールはパスカルケースで書くべきです:
    - 例: `AccountBlocked`、`SupportSuperFormProcessor`
- ファイル名
  - すべてのファイル名はスネークケースを使用するべきです:
    - 例: `process_account`、`compare_only`

## コードのフォーマット

- インデント
  - 可能な限りインデントレベルごとに2スペースを使用します。
    - 例:
...
- 行の長さ
  - コードの行は最大80〜120文字に保ちます（80を目指し、最大は120です）。これにより可読性が確保され、ほとんどの環境でコードが快適に収まります。
- 空行
  - 関数、クラス、論理的に関連するコードブロックを区切るために空行を使用します。
  - メソッド定義の間には1行の空行を使用します。
- スペース
  - 演算子（例: `+`、`<`、`=` など）の間には1つのスペースを使用します。

## コメントとドキュメント

- インラインコメント
  - インラインコメントは控えめに使用します。複雑あるいは自明でないコードを明確にするためだけに使用します。大量のコメントが必要な場合、おそらくコードを過度に複雑にしています!
  - コメントは最新の状態に保ち、それが説明するコードに関連したものにします。

## コードの複雑さ

- 深くネストされたコードを避けます。ネストが複雑になりすぎる場合は、ヘルパー関数や早期リターンを使用するようにコードをリファクタリングします。
- 関数のサイズを制限します。各関数は明確で単一の責任を持ち、比較的小さく（20行未満）するべきです。
...
- コミットメッセージ
  - 行われている変更を詳しく説明します。
  - あなたが取り組んでいる issue に関連付けるテキストを追加します。
  - 例:
...
- ブランチ
  - ブランチの形式は `USERNAME-PROJECT-IID` の形式に従うべきです。
    ここで:
    - `USERNAME` はあなたの gitlab.com ユーザー名です
    - `PROJECT` はプロジェクトのスラッグです
    - `IID` は issue の IID です
    - 例:
      - `jcolyer` が issue `https://gitlab.com/gitlab-com/support/support-ops/support-ops-project/-/issues/1963` の作業のために作成するブランチの場合、ブランチ名は `jcolyer-support-ops-project-1963` となるべきです

## セマンティックバージョニングを使用する

{{% alert title="Note" color="primary" %}}

- これは一般論です。バージョニングに関するより具体的な情報については、あなたが取り組んでいる項目のドキュメントページを参照してください。

{{% /alert %}}

バージョン番号を使用する必要があるときはいつでも、`MAJOR.MINOR.PATCH` の形式であるセマンティックバージョニングを使用するよう努めるべきです。

バージョン番号を増やすときは、どの番号を増やすかを判断するために以下を使用するべきです:

- 大規模なリファクタリングを行う場合は `MAJOR` を増やします
- 機能を追加または削除する場合は `MINOR` を増やします
- 小さな変更（文言の変更、バグ修正など）を行う場合は `PATCH` を増やします

値を増やすときは、以下を覚えておきましょう:

- `MAJOR` を増やす場合、`MINOR` と `PATCH` の新しい値は 0 になります
- `MINOR` を増やす場合、`PATCH` の新しい値は 0 になります（`MAJOR` は変更なし）
- `PATCH` を増やす場合、`MAJOR` と `MINOR` の値は変更なしのままです
...
### 2つの数値しか許可されない場合のセマンティックバージョニングの使用

`1.01` や `9.8` のように2つの数値しか許可されないものを扱っている場合は、代わりに2番目の値について `MINOR` と `PATCH` の定義を組み合わせます。これにより必要な形式 `xx.yy` となり、セマンティックバージョニングに近い形を維持できます。

したがって、バージョン番号を増やすときは、どの番号を増やすかを判断するために以下を使用するべきです:
...
可能な限り、コードがエラーを適切に処理するようにします。これが不可能な場合は、エラーが何が問題なのかを明確に説明するようにします（さらに、それについて何をすべきかを伝えてくれればボーナスポイントです）。

## パフォーマンスに関する考慮事項

- パフォーマンスにはコードが機能的で保守可能になった後にのみ焦点を当てます。早すぎる最適化を避けます。
- プロファイリングツールを使用してパフォーマンスへの影響を測定し、最適化する前にボトルネックを特定します。
- 常識を働かせて、私たちのツールへの API 呼び出しを減らします。
  - 例: Zendesk からすべてのチケットを1つずつ手動で取得することもできますが、Readiness Gem が `list` メソッドを使用して効率的にこれを行ってくれるのに、そうする論理的な理由はありません。

## セキュリティ

- インジェクション攻撃（例: SQL インジェクション、XSS）を防ぐためにすべての入力を検証します。
- パスワード、トークンなどを平文で保存してはいけません。CI/CD 変数を使用します。

## コードレビュー

- すべてのコードはマージ前にピアレビューを受けなければなりません。レビューが徹底的に行われ、品質、一貫性、セキュリティに対処していることを確保します。
- 建設的なフィードバックを受け入れ、協力してコードを改善します。

## 言語固有のガイドライン

現在のところ、私たちの主要言語である ruby に焦点を当てています。これは生きたセクションであり、将来私たちが使用する可能性のある他の言語についても今後追加されるべきです。

### Ruby

- 可能な限り、[rubocop](https://rubygems.org/gems/rubocop) リンターで実行したときにコードが問題を示さないようにするようにします。
...
これにより、次の行からすぐにコーディングを開始でき、作業を進めるための良い出発点が得られます。

#### リクエストを行う

[出発点のコード](#writing-a-ruby-script) を使用すると、以下のように外部リクエストを行うことができます:
...
より完全な例として、ここでは Zendesk インスタンスのすべてのオートメーションを取得するコードを示します:
...
Zendesk に接続する必要がある場合は、[出発点](#writing-a-ruby-script) を使用して、以下を追加します:
...
GitLab に接続する必要がある場合は、[出発点](#writing-a-ruby-script) を使用して、以下を追加します:
...
Slack に接続する必要がある場合は、[出発点](#writing-a-ruby-script) を使用して、以下を追加します:
...
これを使ってリクエストを行うことができます。その例:

<details>
<summary>受信 Webhook を使用して Slack に投稿する</summary>
...
Salesforce に接続する必要がある場合は、[出発点](#writing-a-ruby-script) を使用して、以下を追加します:
...
Mailgun に接続する必要がある場合は、[出発点](#writing-a-ruby-script) を使用して、以下を追加します:
...
Google Sheets に接続する必要がある場合は、[出発点](#writing-a-ruby-script) を使用して、以下を追加します:
...
Google Calendars に接続する必要がある場合は、[出発点](#writing-a-ruby-script) を使用して、以下を追加します:
