...
translated_at: "2026-09-20T03:38:42+00:00"
translator: claude
stale: false
---
...
必要であれば、`#mr-buddies` Slack チャンネルなど、社内の誰にでもヘルプを求めてください。ウェブサイトを更新する**4 つの**方法があります：
...
1. [エージェントに支援を依頼する](#ask-an-agent-for-help)
...
1. 詳細を更新します：
   ...
   - 現在、一時的なチーム異動のリクエストに基づいて別のチームで働いている場合、`borrow` を追加し、以下のように `to` と `end_date` キーを設定してください：
...
### 方法 2：「Web インターフェース」を使って GitLab.com で情報を追加する {#method-2-add-your-info-on-gitlabcom-using-the-web-interface}
...
1. 次に、経歴情報を編集します。チームページに表示されるすべての経歴情報はデータファイルから取得されます。`data/team_members/person/FIRST_LETTER_OF_YOUR_SLUG/SLUG_REPLACE.yml` に移動します（あなたの名前またはスラッグを指定するファイルを探しています）。
...
1. お好みのエディタで `data/team_members/person/FIRST_LETTER_OF_YOUR_SLUG/SLUG_REPLACE.yml` を開き、特に自分の名前またはスラッグのファイルを探します。
1. フィールドのリストとその記入方法については、上記の[方法 1：Web IDE を使って GitLab.com で情報を追加する](#method-1-add-your-info-on-gitlabcom-using-web-ide)のポイント 5 を参照してください。
1. 編集した `data/team_members/person/FIRST_LETTER_OF_YOUR_SLUG/` のファイルに変更を保存します。
...
### エージェントに支援を依頼する {#ask-an-agent-for-help}

[www-gitlab-com プロジェクト](https://gitlab.com/gitlab-com/www-gitlab-com)で
[GitLab Duo Agentic Chat](/handbook/about/editing-handbook/#ask-an-agent-in-gitlab)を開き、
変更したい内容を説明してください。例：
...
名前と経歴を自分のものに置き換えてください。エージェントに YAML の書式を確認し、
重複するフィールドを特定し、提案した編集内容を整理するよう依頼することもできます。
[チームメンバーデータスキーマ](https://gitlab.com/gitlab-com/www-gitlab-com/-/blob/master/data/team_members/person/README.md)を
参照し、矛盾する値や Workday が管理するフィールドを指摘するよう依頼してください。

ドラフト MR を依頼する前に差分を確認してください。無関係なプロフィールの詳細はそのままにし、
承認を求める前に[変更を確認する](#reviewing-your-changes)の手順に従ってください。

## 変更を確認する {#reviewing-your-changes}
...
## ペットをチームペットページに追加する {#add-your-pets-to-the-team-pets-page}
