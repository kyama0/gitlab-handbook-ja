...
- `Net ARR` - ATR と Amount の差分。正の NetARR はグロースを、負の NetARR はチャーンまたは縮小を反映します。
- `ATR` - リニューアル率の計算に使用。これは Available to Renew (ATR) の収益です
- `Won ATR` - リニューアル率の計算に使用。これはディールの予測リニューアル収益です。このフィールドは ATR フィールドの 100% を上限とし、リニューアル率が 100% を超えないようにしています。
