...
translator: claude
stale: false
lastmod: "2025-10-29T07:27:16-04:00"
---
...
1. 以下のコマンドで GitLab パッケージをインストールします。完全修飾ドメイン名の代わりに、トレーニングシステムに割り当てられたパブリック IP アドレスを使用してください。<> 記号は含めないでください。
    ...
    > この例では、URL の前に `https://` を付けました。このラボのセットでは `http` プロトコルを使用しています。本番環境に GitLab をインストールする場合は、`https` プロトコルを使用するため `https://` を使用することを推奨します。
