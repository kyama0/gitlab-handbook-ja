...
まず、グループレベルで基盤フローを有効化する必要があります。この設定により、GitLab Duo がグループ内のプロジェクトで、ブランチの作成やマージリクエストの生成などのワークフローを自動化できるようになります。
...
1. **Allow foundational flows** の下で、各フローの横にあるチェックボックスをチェックしてすべてのフローを有効化します。7 個のフローがリストされているはずです。
