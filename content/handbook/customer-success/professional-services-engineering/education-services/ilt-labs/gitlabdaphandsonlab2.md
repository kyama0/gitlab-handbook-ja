...
Agentic Chat では、あなたが直接コントロールします。すべての書き込みアクションは実行前に承認が必要です。ラボ 1 が基盤フローを独自に実行できることを示したのに対して、このラボではあなたが主導権を握ります。
...
- **fix: improve text contrast** マージリクエストにコメントが 1 つ投稿されている。
