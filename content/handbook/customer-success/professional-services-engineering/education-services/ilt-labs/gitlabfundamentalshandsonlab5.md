...
1. 以下のコードを `.gitlab-ci.yml` ファイルにコピーします:
      ...
      > **build** という 1 つのステージを定義しました。このステージには、`go build` という 1 つのスクリプトを実行する単一のジョブがあります。その結果、Go アプリケーションがコンパイルされます。`./array` はコンパイルされたアプリケーションを実行します。`image` キーワードは、GitLab Runner でジョブが Golang Docker コンテナ内で実行されるようにするために使用されます。
