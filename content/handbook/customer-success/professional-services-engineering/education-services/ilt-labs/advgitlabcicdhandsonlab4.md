...
1. **Commit changes** を選択し、適切なコミットメッセージを追加して、**Commit to a new branch** を選択します。ブランチ名を `feature-flag` にします。**Commit to branch** を選択し、**Create a merge request for the new branch** が選択されていることを確認します。**Commit changes** を選択します。

1. すべてをデフォルト値のままにして、**Create a merge request** を選択します。

    > このコードは継続的に実行され、フィーチャーフラグトグルが有効か無効かを確認しようとします。50% のユーザーに対して有効になっているため、このコードを実行するとおよそ半分の確率で有効と表示されます。

## タスク B. フィーチャーフラグをテストする {#task-b-test-the-feature-flag}

1. これをテストするために、スクリプトの実行テストを行います。

1. **Build > Pipeline Editor** を選択します。

1. **feature-flag** ブランチにいることを確認して、`.gitlab-ci.yml` ファイル全体を次のコードに置き換えます:
