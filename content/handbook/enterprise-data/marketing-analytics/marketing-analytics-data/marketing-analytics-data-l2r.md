...
Lead to Revenueレポートは、主要なライフサイクルオブジェクト/マートの統合です: Person、Opportunity、Account、および両タイプのBizibleタッチポイント。結合されたオブジェクトに関するほぼすべてのセールスとマーケティングの指標とデータのワンストップショップです。ファネルの進行分析、データポイントと指標のコーティング、そしてリード作成からオポチュニティのクローズまでの真のファネルを1つのテーブルから作成することができます。
...
現在WIPのTableau固有のテーブルで、ファネルステージ間の速度とラグタイムを事前計算してrpt_lead_to_revenueデータをコーティングします。コーティングされたコンバージョン率とKPIカウントの決定に完全に焦点を当てています。

### ユースケース

1. コーティングされた速度、コンバージョン率、KPIカウントの計算
