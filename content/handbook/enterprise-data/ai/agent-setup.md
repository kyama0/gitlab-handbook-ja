...
### ステップ 4: AI プロバイダとして GitLab Duo を設定する
...
> **注:** `opencode.json` ではなく `~/.config/opencode/opencode.jsonc` を使用してください。`.jsonc` 拡張子はコメントを許容するので、設定への注釈付けに便利です。

### ステップ 6: 既定のエージェントを `plan` に設定する
...
詳細なプロンプトを入力するのは遅いです。[MacWhisper](https://goodsnooze.gumroad.com/l/macwhisper) は macOS 向けの音声テキスト変換アプリで、コンテキストの説明や問題を口頭で解いていくのが大幅に速くなります。特に、考えるよりも入力に時間がかかるような長いプロンプトで効果的です。
