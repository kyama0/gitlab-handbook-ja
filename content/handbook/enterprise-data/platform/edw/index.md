...
**ビッグテーブルの作成:**
ビッグテーブルモデルは、できるだけ多くの関連属性を単一の幅広いテーブルに提供することを目的とします。複数のデータソースをまたいで使う必要がない場合や、データが非常に大きく下流の結合を再設計する場合に有用です。
...
エンティティ関係（ER）図は、システム内で `entities` または `objects` がどのように相互に関連するかを表すフローチャートの一種です。`ER diagrams` は、論理とビジネスルールの観点（論理データモデル）および実装する特定のテクノロジーの観点（物理データモデル）から、リレーショナルデータベースをモデル化・設計するために使います。

以下の手順は、`Lucidchart's ERD import functionality` を使用して `ER Diagram`（論理・物理データモデル）を作成する方法を示します。データベースまたはスクリプトからデータモデルを作成するこのプロセスは、`Reverse engineering` とも呼ばれます。
...
1. ベロシティ - データがどれだけ速く変化、取り込み、または利用されるか
