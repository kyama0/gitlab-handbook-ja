...
EDW の本番データベースは、GitLab のデータコンシューマーによるレポートと分析に使用されます。主要な 5 つのスキーマ`COMMON_`、`SPECIFIC`、`LEGACY_`、`WORKSPACE_`、`SEMANTIC`で構成されています。以下に各スキーマの説明を示します。

データウェアハウスのアーキテクチャは、5 つの異なるスキーマで構成されています。
...
1. **SEMANTIC スキーマ:** エージェント型ツールおよび AI ツールによるセマンティックレイヤーへの主要なエントリポイントとして機能し、データを正しくクエリおよび解釈するために必要なすべてのコンテキストを含む埋め込み Agent Skills を備えた、キュレーション済みで文書化されたビューを提供します。
...
スキーマ設計は、初期取り込みから最終消費までのデータの自然な進行を反映しています。生データは専用スキーマを通じて入り、準備スキーマで変換され、最終的には人間によるレポートと分析のために次元モデルを支える消費可能なスキーマ、または同じデータへのキュレーション済みでエージェントがアクセスできるインターフェースを提供する AI Semantic スキーマに配置されます。このレイヤー化されたアプローチにより、データ処理の異なる段階間で明確な引き渡しを維持しつつ、パイプライン全体でデータ品質と一貫性を確保できます。各スキーマはデータの移動におけるチェックポイントとして機能し、データウェアハウスの完全性を維持するのに役立つ明確な責任とガバナンスルールを備えています。
...
AI Semantic スキーマは、エージェント型ツールおよび AI ツール向けに特別に設計された、データウェアハウスへのキュレーション済みで制御されたインターフェースを提供します。これらのスキーマは、データウェアハウスのイントロスペクションやメタデータのクロールに依存するのではなく、エージェントが発見して信頼性高く使用できる、安定して意図的に設計されたビューのセットを提供します。

この目的には、次の 2 つのスキーマがあります。

- **`prod.SEMANTIC`** — エージェント型ツール向けに公開されているビュー
- **`prod.RESTRICTED_SAFE_SEMANTIC`** — 機密データを公開するビューに対するアクセス制御付きの同等物

### 主な設計原則

これらのスキーマ内のすべてのオブジェクトは、非マテリアライズドオブジェクト（ビュー、関数、セマンティックビュー）です。これにより、完全なモデルの再ビルドを必要とせずに変更を直ちにデプロイできます。ビューは既存のレポートテーブルへのエントリポイントとして機能し、ビジネスロジックの重複を回避します。保守性、テスト容易性、スケーラブルな開発のため、すべてのオブジェクトは dbt で管理されます。

### ビューの種類

| 種類 | 説明 |
|------|-------------|
| Verified Query | 対象分野の専門家がキュレーションします |
| Discovered Query | ユーザークエリのパターンを評価して構築します |
| Semantic View | データの関係から構築します |
| User Defined Table Function | 厳格なパラメータが必要です |

### ドキュメント化のパターン

各ビューは、エージェントが利用するように設計された次のパターンに従って文書化されます。

- **スキーマの説明** — 個々のビューの説明と並んでエージェント検索に参加する、スキーマ自体の説明
- **短い説明** — ビューが提供する内容の簡潔な要約。エージェントのコンテキストウィンドウを圧迫せずに候補ビューを特定するため、段階的な詳細開示検索で使用されます
- **完全な使用方法の説明** — ビューが選択されると読み込まれる包括的な説明。これは Agent Skill として構造化され、機能します。使用方法のガイダンス、パラメータの期待値、結果の解釈など、ビューを正しくクエリおよび解釈するために必要なすべてのコンテキストが含まれます
- **カテゴリと優先度** — 検索結果をランク付けしてフィルタリングするために使用されるメタデータ

### エージェントによるこれらのスキーマの使用

エージェントは、段階的な詳細開示パターンを使用して、これらのスキーマと対話します。

1. エージェントはスキーマの説明とビューの短い説明を使用してスキーマを検索し、関連するビューを特定します。すべての結果の詳細を読み込むことなく、ランク付けされた一致を返します
2. エージェントは最も関連性の高いビューを選択し、その完全な使用方法の説明を読み込みます。これは完全なクエリコンテキストを提供する、用途に特化した Agent Skill として機能します
3. エージェントは埋め込みガイダンスを使用してビューを直接クエリするか、基礎となる SQL を読んで十分な情報に基づくアドホックな変更を行います
...
Tableau は主要な可視化ツールであるため、人間によるレポートを目的とするすべての EDW モデルは、Tableau の互換性を念頭に置き、一貫した命名規則に従い、移行を慎重に計画して設計する必要があります。このガイダンスは、COMMON、COMMON_MART、SPECIFIC、および関連するスキーマのモデルに適用されます。AI Semantic スキーマ（`SEMANTIC`および`RESTRICTED_SAFE_SEMANTIC`）のオブジェクトはエージェント型ツールによる利用向けに設計されており、[AI Semantic スキーマ](#ai-semantic-schemas)セクションで説明する別の設計パターンに従います。
...
1. **AI SEMANTIC VIEWS:** `SEMANTIC`および`RESTRICTED_SAFE_SEMANTIC`スキーマのオブジェクトでは、オブジェクトの種類に基づくプレフィックスの後に、そのオブジェクトが回答するよう設計されたビジネス上の質問を反映する平易な言葉の名前を使用します。
