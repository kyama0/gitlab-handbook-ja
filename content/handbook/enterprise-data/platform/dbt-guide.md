...
#### dbt の実行時に適切な Snowflake ウェアハウスを選択する
...
##### ローカルユーザー DB へのクローン（python スクリプト - `dbt clone` 以前）
...
#### data-tests プロジェクトで新しく導入された dbt テストの実行
...
### `dbt snapshot` でスナップショットテーブルを作成する
...
### スナップショットテーブルを prod データベースで利用可能にする
