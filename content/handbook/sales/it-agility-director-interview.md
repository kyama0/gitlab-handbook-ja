---
title: "IT アジリティディレクターとのインタビュー"
description: "Fortune 500 のエネルギー企業の IT アジリティディレクターへの 2020 年 4 月のインタビューからのクリップ"
upstream_path: /handbook/sales/it-agility-director-interview/
upstream_sha: 1e195b58b9f249ff10bd0e705106c320fee86141
translated_at: "2026-05-11T00:00:00Z"
translator: claude
stale: false
lastmod: "2024-06-27T22:14:31+00:00"
---

## イントロダクション

以下は、Fortune 500 のエネルギー企業の IT アジリティディレクターへの 2020 年 4 月のインタビューからのクリップです。

### 役割と責任

質問: *過去数年間のあなたの役割と責任は何でしたか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/ygpdeVUSPYA" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### DevOps プロジェクトの目標

質問: *あなたの組織の DevOps プログラムの目標は何でしたか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/xzVOSRuiK1U" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### DevOps プロジェクトの注力領域と組織

質問: *あなたの組織の DevOps プロジェクトの注力領域は何で、チームはどのように組織されていましたか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/rQvT_iDs5rU" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### DevOps ロールアウト戦略

質問: *あなたの DevOps ロールアウト戦略はどのようなものでしたか？フォーカスする特定のユースケースや機能はありましたか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/6s0iXfi70dQ" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### 断片化された DevOps ツールチェーンアプローチに伴う課題

質問: *あなたの組織はなぜ、どのようにベスト・オブ・ブリードの DevOps ツールチェーンアプローチを選び、その決定によりどのような課題に直面しましたか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/SmlBt1Rn2LA" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### DevOps プログラムの課題と学んだ教訓

質問: *あなたが DevOps プログラムで経験した課題と学んだ教訓は何でしたか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/vFM5DnswXD4" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### DevSecOps

質問: *DevSecOps の重要性についてあなたの見解はどのようなものですか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/pYfVc7qXrE4" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### IT ディレクターの 1 日

質問: *IT ディレクターの 1 日はどのようなものですか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/SBO35Q5U2YE" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### B2B セールスパーソンへのアドバイス

質問: *優れたベンダーやセールスパーソンと平均的なベンダーやセールスパーソンを分けるものは何ですか？B2B セールスパーソンに 1 つだけアドバイスを送るとしたら、それは何で、なぜですか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/HyIkHOvzRgM" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### ツール選定プロセス

質問: *あなたの組織は、新しいツールやサービスを購入する評価と意思決定を通常どのように行いましたか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/InGBYPK0u00" frameborder="0" allowfullscreen="true"> </iframe>
</figure>

### COVID-19 アドバイス

質問: *COVID-19 のパンデミック中の B2B セラーへのアドバイスは何ですか？*

<figure class="video_container">
  <iframe src="https://www.youtube.com/embed/kCARKq8YZUM" frameborder="0" allowfullscreen="true"> </iframe>
</figure>
