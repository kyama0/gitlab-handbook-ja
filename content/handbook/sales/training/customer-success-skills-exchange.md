---
title: "Technical Skills Exchange"
description: "GitLab のテクニカルなフィールドチームメンバー向けの継続教育ウェブキャストシリーズ"
upstream_path: /handbook/sales/training/customer-success-skills-exchange/
upstream_sha: 1e195b58b9f249ff10bd0e705106c320fee86141
translated_at: "2026-05-11T00:00:00Z"
translator: claude
stale: false
---

Technical Skills Exchange（旧称 Customer Success Skills Exchange）は、テクニカルなフィールドチームメンバー向けトピックに焦点を当てた隔週ウェビナーです。このシリーズは主に Customer Success と SA 組織を念頭に企画されていますが、フィールドチームメンバーであれば誰でも参加歓迎です。

- 各トレーニングセッションには [スピーカー](#speaker-responsibilities) と [モデレーター](#moderator-responsibilities) がいます
- セッションは約 50 分間です

## 今後のトレーニング

近日公開予定のトレーニングは、[Technical Skills Exchange Issue Board](https://gitlab.com/gitlab-com/sales-team/field-operations/enablement/-/boards/7792277?label_name[]=Technical%20Skills%20Exchange) で確認できます。

### 毎週のセッション招待を受け取る

Technical Skills Exchange のカレンダー招待を受け取るには、sales-cs メールグループに追加されていることを確認してください。

メールグループへのアクセスを依頼するには [アクセスリクエストを提出](/handbook/security/corporate/end-user-services/access-requests/access-requests/) できます。これらのグループはミーティングに招待されているので、個人ごとに招待は不要です。

### トレーニングプレイリスト

過去の Technical Skills Exchange セッションは、以下の場所で確認できます:

- [Technical Skills Exchange Highspot Page](https://gitlab.highspot.com/items/6554f9b2501316c929028227)
- GitLab Unfiltered YouTube の [Technical Skills Exchange Youtube playlist](https://www.youtube.com/playlist?list=PL05JrBw4t0KorkxIFgZGnzzxjZRCGROt_)
- [Technical Skills Exchange Q&A Doc](https://docs.google.com/document/d/1Zyj8u4JfiBi4gUSn9xAbRZ5H1erY9qpb8j0NzvFwwWg/edit)

### 新しい Technical Skills Exchange セッションを依頼するには

- [Technical Skills Exchange Issue Board](hhttps://gitlab.com/gitlab-com/sales-team/field-operations/enablement/-/boards/7792277?label_name[]=Technical%20Skills%20Exchange) にある現四半期のプランニング Issue にコメントして、私たちのチームと連絡を取ってください

### スピーカーの責任

- セッション開始予定の少なくとも 2 分前に準備を整え、定刻通りに開始できるようにしてください。
- スライドを Customer Success and SA Enablement プログラムマネージャーに送り、[ノートドキュメント](https://docs.google.com/document/d/1Zyj8u4JfiBi4gUSn9xAbRZ5H1erY9qpb8j0NzvFwwWg/edit) に追加してください
- その他の関連リンクも [ノートドキュメント](https://docs.google.com/document/d/1Zyj8u4JfiBi4gUSn9xAbRZ5H1erY9qpb8j0NzvFwwWg/edit) に含めてください

### モデレーターの責任

#### Technical Skills Exchange の全体管理

Technical Skills Exchange のモデレーターとして、四半期のトピック、スピーカー、日程の特定を担当します。

#### ウェビナーモデレーターの責任

**ウェビナー前**

- ウェビナーの 1 週間前に、スピーカーがセッションの準備ができ、参加可能かを確認してください。これにより直前のキャンセルを減らせます。
- ウェビナー前日に #customer-success、#tim-tams、#solutions-architects の Slack チャンネルでアナウンスを送ります。

**ウェビナー中**

各ウェビナーのモデレーターは、コールのホストおよびプレゼンターの全体的なウィングパーソンとして機能する必要があります。モデレーターはチャットを監視してプレゼンターに質問を伝え、コール中に言及されたリンクを検索して、それらがトレーニングのハンドブックページにリンクされていることを確認します。

- Zoom に 5 分前にログインします
- ビデオは録画されていない状態にし、録画されている場合は一時停止します
- すぐに全員を歓迎し、コールが録画されることを伝えます。機微な情報をどのように扱うかについては、毎回のイントロで具体的に言及してください。開始時刻にできるだけ近いタイミングでミーティングを開始してください。
- コールを録画します。
- 時間を監視します。

**ウェビナー後**

- セッション後、録画を [Technical Skills Exchange Highspot](https://gitlab.highspot.com/items/6554f9b2501316c929028227) にアップロードします。
- #customer-success、#tim-tams、#solutions-architects の Slack チャンネルでアナウンスを投稿し、[Highspot ページ](https://gitlab.highspot.com/items/6554f9b2501316c929028227) へのリンクを記載します。

アナウンスのサンプル:

> 昨日の Technical Skills Exchange セッション（<プレゼンター> が <セッションの概要> について話したもの）に参加できなかった方、録画が Highspot で公開されています！
>
> [Highspot ページ](https://gitlab.highspot.com/items/6554f9b2501316c929028227) で過去のすべての録画セッションへのリンクを確認でき、セッションの [ノートドキュメント](https://docs.google.com/document/d/1Zyj8u4JfiBi4gUSn9xAbRZ5H1erY9qpb8j0NzvFwwWg/edit) でチームの Q&A を確認できます。
