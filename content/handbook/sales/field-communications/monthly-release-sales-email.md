---
title: "GitLab 月次リリース セールス向けメール"
description: "毎月の GitLab リリースの後、Field Communications は Field 組織のメンバーが顧客や見込み顧客に送信する用途に再利用できるメールテンプレートを送付します"
upstream_path: /handbook/sales/field-communications/monthly-release-sales-email/
upstream_sha: 6329e70e21b29c3cf88be90bd4abee6489b39003
translated_at: "2026-05-21T11:45:00Z"
translator: claude
stale: false
lastmod: "2026-05-20T09:32:02-07:00"
---

## 概要

[毎月の](https://docs.gitlab.com/development/documentation/release_notes/) GitLab リリースの後、Product Marketing は Highspot でピッチテンプレートを作成し、Field 組織のメンバーが顧客や見込み顧客に送信する用途に再利用できるようにします。このピッチテンプレートの目的は、チームメンバーが最新リリースの技術的な詳細を素早く共有し、顧客や見込み顧客との意味のある接点を作りやすくすることです。

### フォーマット

クイックフォーマットには以下が含まれます:

1. [GitLab の価値ドライバー](/handbook/sales/command-of-the-message/)の概要
1. リリースの 3〜4 個の主要な、または注目すべき機能
1. リリースブログ記事および [GitLab Releases ページ](https://about.gitlab.com/releases/)

### プロセス

1. Product Marketing は[毎月の](/handbook/engineering/releases/)リリース日にピッチテンプレートを作成し、Highspot の `Company Pitch (Email) Templates Spot` にテンプレートを格納します。
1. Product Marketing は最新のリリースブログ記事を Highspot の `Customer Outreach` スポットにアップロードし、その後そのコンテンツを[リリースページ](https://gitlab.highspot.com/items/64be9d6dc960ff31cd9041c6)とともにピッチテンプレートに追加します。
1. Product Marketing はピッチテンプレートと最新のリリースブログ記事を、[GitLab Releases Overview ページ](https://gitlab.highspot.com/items/624626054e2472aa8611b5bf)の該当セクションに追加します。
1. Product Marketing は、上記ステップが完了した時点で、リリース記事ハイライト issue でピッチテンプレートへのリンクを Field Communications に通知します。
1. Field Communications は #sales でピッチテンプレートを共有し、[Field Update Tickers](/handbook/sales/field-communications/field-update-tickers/) および [Field Flash](/handbook/sales/field-communications/field-flash-newsletter/) に追加します。

### 役立つリソース

- [Highspot での External Shares (Pitching and Digital Rooms)](/handbook/sales/field-communications/gitlab-highspot/#key-functionality)
- [Highspot からメールピッチを送信する](https://help.highspot.com/hc/en-us/articles/213580306-Send-an-Email-Pitch-from-Highspot)
- [Highspot Pitching - Deep Dive Training](https://docs.google.com/document/d/1mu9itIGJPmT8onA19b36kzSTHEudUDAGQ5vSWt7ZRl0/edit#bookmark=id.oq1sf0leuadt)
- [GitLab Releases Overview ページ](https://gitlab.highspot.com/items/624626054e2472aa8611b5bf?lfrm=srp.0)

ご質問は #field-enablement-team までお問い合わせください。
