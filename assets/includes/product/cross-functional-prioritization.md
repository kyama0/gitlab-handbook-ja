...
Our backlog should be prioritized on an ongoing basis. Prioritization will be done via team planning (collaboration between product, development, UX) with PM as the DRI for the milestone plan. PMs, EMs, and UX will provide the following:

1. Product Manager (PM) provides prioritized `type::feature` issues
1. Engineering Manager (EM) in development provides prioritized `~"type::bug"` based on [bug prioritization](/handbook/engineering/infrastructure-platforms/developer-experience/bug-prioritization/) and `~"type::maintenance"` issues, including [reducing technical debt](/handbook/engineering/workflow/#technical-debt)
...
Future plans for what features we will implement must be planned for implementation within the next (rolling) 18-month period.

Any feature planned for implementation within that period must be labeled with [`~"backlog::prospective"`](https://gitlab.com/groups/gitlab-org/-/epics/18639). If there are no such plans, the issue must be labeled with [`~"backlog::no-commitment"`](https://gitlab.com/groups/gitlab-org/-/epics/18639), and a justification included.

#### Milestone planning and prioritization

Teams should follow the standard milestone planning process as described in our [Product Development Flow](/handbook/product-development/how-we-work/product-development-flow/#build-track). PMs will keep the overall prioritized backlog following the agreed-upon ratios and guidance from their EM, and UX counterparts.
