...
リポジトリ `/home/yamazaki/projects/gitlab-handbook-ja` で、upstream の pending ファイルを翻訳して `content/handbook/` と `translation-state/manifest.json` を更新し、`main` 向け PR を作成する。
...
`*.html.md` alias が必要な場合は `.agents/skills/translate-batch/scripts/add_html_md_aliases.py` の `MAPPINGS` を編集して実行する。
