...
{{- if .IsNamedParams }}
    {{- with (.Get "slack-channel") }}
        {{- $slackChannel = . }}
    {{- else }}
        <h2>Requires the slack-channel argument</h2>
    {{- end }}
    {{- with (.Get "slack-url") }}
        {{- $slackURL = . }}
    {{- else }}
        <h2>Requires the slack-url argument</h2>
    {{- end }}
    ...
    {{- with (.Get "group-issues-url") }}
        {{- $groupIssuesURL = . }}
    {{- else }}
        <h2>Requires the group-issues-url argument</h2>
    {{- end }}
    {{- $groupSubgroup = .Get "gitlab-subgroup" }}
{{- else }}
    <h2>Requires arguments for <code>slack-channel</code>, <code>slack-url</code>, <code>group-label</code>, and <code>group-issues-url</code></h2>
{{- end }}
...
{{- if $groupSubgroup }}
If you need to call the attention of the entire group, you can tag `@gitlab-com/{{ $groupSubgroup }}`.
{{- end }}
