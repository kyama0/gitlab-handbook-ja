...
{{- range site.Data.public.stages.stages }}
  {{- range .groups }}
    {{- if eq .name $section_group }}
      {{- $eng_ic_team_tag := .eng_ic_team_tag }}
      {{- $pm := .pm }}
      {{- $ux := .ux }}
      ...
      {{- range $k, $v := site.Data.public.team }}
        {{- if in .departments $eng_ic_team_tag }}
          {{- if not (eq (lower .name) (lower $em)) }}
            {{- if not (eq (lower .name) (lower $fem)) }}
              {{- $engTeamDict = merge $engTeamDict (dict (lower .gitlab) . ) }}
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>{{- with (index $pmData 0) }}{{ partial "member/with-gitlab-link" . }}{{- end }}</td>
      <td>{{- len $pmData }}</td>
    </tr>
    <tr>
      ...
      <td>{{- if gt (len $uxTeamDict) 0}}
          {{- range $uxTeamDict }}
            {{ partial "member/with-gitlab-link" . }}
          {{- end }}
        {{- end }}</td>
    ...
    <tr>
      ...
      <td>{{- with (index $emData 0) }}{{ partial "member/with-gitlab-link" . }}{{- end }}</td>
      <td>{{- len $emData }}</td>
    </tr>
    <tr>
      <th><abbr title="Frontend Engineering Manager">FEM</abbr></th>
      <td>{{- with (index $femData 0) }}{{ partial "member/with-gitlab-link" . }}{{- end }}</td>
      <td>{{- len $femData }}</td>
    </tr>
    <tr>
      <th><abbr title="Engineering">Engineering</abbr></th>
      <td>{{- if gt (len $engTeamDict) 0 }}
            {{- range $engTeamDict }}
              {{ partial "member/with-gitlab-link" . }}
            {{- end }}
          {{- end }}</td>
      <td>{{- len $engTeamDict }}</td>
    </tr>
    <tr>
      <th>Total</th>
      <td></td>
